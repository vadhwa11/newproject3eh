...
<div class="titleBg">
<h2>Vendor Analysis Report(Supplier Wise)</h2>
</div>
<div class="clear"></div>
<div class="Block">
<label><span>*</span> From Date </label> 
<input	type="text" name="<%= FROM_DATE %>" value="<%=currentDate%>"	class="date" maxlength="30" tabindex=1 validate="fromDate,date,yes"/> 
<img id="calendar"	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"	validate="Pick a date"	onclick="setdate('<%=currentDate%>',document.vendorAnalysis.<%= FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"	name="<%= TO_DATE %>" value="<%=currentDate%>" class="date"	maxlength="30" tabindex=1 validate="toDate,date,yes"/> 
<img id="calendar"	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"	validate="Pick a date"	onclick="setdate('<%=currentDate%>',document.vendorAnalysis.<%= TO_DATE%>,true);" />
<div class="clear"></div>
<label>Supplier</label> 
<select name="supplierId" id="supplierId"	validate="Supplier,metachar,no" tabindex=1>
...
<select name="categoryId" id="categoryId"	validate="Category,metachar,no" tabindex=1>
	<option value="0">Select</option>
	<% 
...
<label>Manufactuerer</label> 
<select name="manufactuererId"	id="manufactuererId" tabindex=1 validate="Manufacturer,metachar,no">
	<option value="0">Select</option>
	<% 
...
<input type="text" id="nameItem" name="nameItem" class="bigcaption" tabindex="1" validate="nameItem,metachar,no"/>
...
<div class="clear"></div>
<input type="hidden" name="reportName" value="VendorAnalysisReport_Supplier" validate="reportName ,metachar,no"/></div>
<div class="clear"></div>
