...
<%--<div class="Block"><label><%=prop.getProperty("com.jkt.hms.registration_no")%></label>
 <select id="hinId" name="<%=HIN_NO%>" readonly="readonly"
	onchange="submitProtoAjaxWithDivName('search','lab?method=getVisitNoForQC&flag=lab','testDiv')"
	  tabindex=1 > 
	<option value="<%=RequestConstants.UHID_FOR_QUALITY_TESTING%>"><%=RequestConstants.UHID_FOR_QUALITY_TESTING%></option> 
</select> --%> 
<input type="hidden" id="hinId" name="<%=HIN_NO %>" value="<%=RequestConstants.UHID_FOR_QUALITY_TESTING%>" />
...
<%} %>
<script type="text/javascript" language="javascript">
	window.onload=submitForm('search','lab?method=getPatientDetailsForQC');
</script>
