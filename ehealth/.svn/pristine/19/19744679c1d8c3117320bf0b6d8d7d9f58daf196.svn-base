...
<label>CHC</label>
<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveyBasicCenterListchc&chcphc='+this.value,'testDiv');" >
<%	if(chcList.size() >0){%>
...
<label>PHC</label>
<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveyBasicCenterListphc&chcphc='+this.value,'testDiv');" >
<%	if(phcList.size() >0){%>
				<option value="0">Select</option>
				 <%
