...
function investigation(obj){
    ...
    url="investigation?method=showSubInvestigationJsp&mainChargecodeId="+document.getElementById('mainChargecodeId').value+'&subChargeCodeId='+document.getElementById('subChargeCodeId').value+'&chargeCodeId='+document.getElementById('chargeCodeId').value+'&investigationName='+document.getElementById('investigationName').value+'&confidential='+document.getElementById('confidential').value+'&investigationType='+document.getElementById('investigationType').value+'&dischargeSummary='+document.getElementById('dischargeSummary').value+'&sampleId='+document.getElementById('sample_id').value+'&quantity='+document.getElementById('quantity').value+'&collectionCenterId='+document.getElementById('collectionCenterId').value;
		...
		url="/hms/hms/investigation?method=showTemplateJsp&mainChargecodeId="+document.getElementById('mainChargecodeId').value+'&subChargeCodeId='+document.getElementById('subChargeCodeId').value+'&chargeCodeId='+document.getElementById('chargeCodeId').value+'&investigationName='+document.getElementById('investigationName').value+'&confidential='+document.getElementById('confidential').value+'&investigationType='+document.getElementById('investigationType').value+'&dischargeSummary='+document.getElementById('dischargeSummary').value;
		obj.action=url;
		obj.submit();
...
Investigation Name</label> <input type="text" id="searchField"
	name="<%= SEARCH_FIELD%>" value=""
	validate="Investigation Name,string,no" MAXLENGTH="10" tabindex=1 /> <label><span>*</span>
Investigation Type</label> <select name="investigationTypeSearch"
	id="investigationTypeSearch" validate="Investigation Type,string,No">
...
onclick="submitForm('search','investigation?method=searchInvestigation','checkSearch')" tabindex=1 />
<input type="button" name="Report" value="Generate Report" class="buttonBig"
onclick="submitForm('search','investigation?method=printDiagonstics')"accesskey="g" tabindex=1 />
...
<%
 ...
 if(searchInvestigationList.size()==0 && map.get("search") != null)
	  {
	 %> <h4><a href="investigation?method=showInvestigationJsp">Show All
Records</a> </h4><%
    }
	%> <script type="text/javascript">
	formFields = [
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"><input
	type="hidden" name="<%= POJO_NAME %>" value="DgMasInvestigation"><input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="InvestigationName"><input
	type="hidden" name="title" value="Investigation"><input
	type="hidden" name="<%=JSP_NAME %>" value="investigation"><input
	type="hidden" name="pojoPropertyCode" value="InvestigationCode"><input
	type="hidden" name="deptId" value="<%=deptId%>" />
<div class="Block">
<label><span>*</span> Main Charge Code</label>

<select id="<%=MAIN_CHARGECODE_ID %>" name="<%=MAIN_CHARGECODE_ID %>"
...
<label><span>*</span> Sub Charge Code</label>
<select	id="<%=SUB_CHARGECODE_ID %>" name="subChargeCodeId"	validate="Modality ,string,yes" tabindex=1	onChange="populateCharge(this.value,'chargeCode');getLionClass(this.value)">
<option value="">Select</option>
	<% 
...
<label><span>*</span> Test Name</label>
<select	id="<%=CHARGE_CODE_ID%>" name="<%=CHARGE_CODE_ID %>"	validate="Test Name,string,yes" tabindex=1	onchange="fillInvestigation(this.value)">
<option value="">Select</option>
	<% 
...
<input	type="text" name="<%= INVESTIGATION_NAME %>" value="<%=investigationName %>" id=<%=INVESTIGATION_NAME %>	validate="Investigation Name,string,yes" maxlength="30" tabindex=1 />
<%}else{ %>
...
<select name="<%=RARE_COMMON %>"	tabindex=1>
	<option value="c">Common</option>
	<option value="r">rare</option>
</select>

 <label><span>*</span> Investigation Type</label> 
 <input type=text name="" id=""	validate="Investigation Type,string,yes" value="Template"/>
 <input type=hidden name="<%=INVESTIGATION_TYPE %>" id="<%=INVESTIGATION_TYPE %>"	validate="Investigation Type,string,yes" value="t"/>
...
<div class="clear"></div>
<%--
 <label>Collection</label> <select id=<%=COLLECTION_CENTER_ID %>
	name="<%=COLLECTION_CENTER_ID %>" validate="Collection,string,no"
	tabindex=1>
...
<label>Unit Name</label> <select id="unitName"
	name="<%=UNIT_OF_MEASUREMENT_ID %>" validate="Unit Name,string,no"
	tabindex=1>
	<option value="0">Select</option>
	<% 
...
<label> Sample</label> <select id=<%=SAMPLE_ID %>
	name="<%=SAMPLE_ID %>" validate="Sample,string,no" tabindex=1>
	<option value="0">Select</option>
	<% 
...
<div class="clear"></div>
<label>Sample Quantity</label> <input type="text" name="<%= QUANTITY%>"
	value="1" id="quantity" class="text" maxlength="10"
	validate="Sample Quantity,string,no" tabindex=1 />

	 <label>Normal
Value</label> <input type="text" name="<%= NORMAL_VALUE%>" value=""
	id="<%= NORMAL_VALUE%>" validate="Normal Value,string,no"
	maxlength="20" tabindex=1 onblur="disableValues();" /> 
	
	<label>Min
Value</label> <input type="text" name="<%= MIN_NORMAL_VALUE%>" value=""
...
<label>Max Value</label> <input type="text"
	name="<%= MAX_NORMAL_VALUE%>" value="" id="<%= MAX_NORMAL_VALUE%>"
	validate="Max Value,string,no" maxlength="20" tabindex=1 />
 --%>
<div id="testDivItem">
...
<input type="checkbox"	name="<%= CONFIDENTIAL%>" id="<%= CONFIDENTIAL%>" value="y"	class="radioCheck" tabindex="1" />
<label class="auto"> Appear in Discharge Summary</label>
<input type="checkbox"	name="<%=DSICHARGE_SUMMARY%>" id="<%= DSICHARGE_SUMMARY%>" value="y"	class="radioCheck" tabindex="1" />
	<%-- <label class="auto">Variation Required </label> 
	<input type="checkbox" name="variation" id="variation"
	value="y" class="radioCheck" tabindex="1" /> --%>
<label class="auto">SMS Required </label>
<input type="checkbox" name="sms" id="sms" value="y"	class="radioCheck" tabindex="1" />
<div class="clear"></div>
<%--<label class="common">Blood Screen test</label> <input type="checkbox"
	name="bloodBankScreen" id="screenTest" value="y" class="radioCheck"
	tabindex="1" /> <label class="common">Blood Reaction Test</label> <input
	type="checkbox" name="bloodReactionTest" id="reactionTest" value="y"
	class="radioCheck" tabindex="1" /> --%>
...
<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button"	onClick="submitForm('chargeCode','investigation?method=editInvestigation')"	accesskey="u" tabindex=1 />
<input type="button" name="Delete"	id="deletebutton" value="Activate" style="display: none;"	class="button"	onClick="submitForm('chargeCode','investigation?method=deleteInvestigation&flag='+this.value)"	accesskey="d" tabindex=1 />
<input type="reset" name="Reset"	id="reset" value="Reset" class="buttonHighlight" accesskey="r"	onclick="resetCheck();" tabindex=1 />
		 ...
		 while(itrGridChargeCodeList.hasNext())
            {
			 ...
			 if(dgmasInvestigation.getChargeCode().getId().equals(chargeCodeGrid.getId()) && chargeCodeGrid.getStatus().equals("y"))
			 {
			 %>
			 ...
			 while(itrGridSampleList.hasNext())
            {
             ...
             if(dgmasInvestigation.getSample() != null && !dgmasInvestigation.getSample().equals("")){
            	if(dgmasInvestigation.getSample().getId().equals(sampleGrid.getId()) && sampleGrid.getStatus().equals("y"))
			 {
			 %>
				data_arr[<%= counter%>][5] = "<%=sampleGrid.getSampleDescription()%>"
			<%
			}
			 else if(dgmasInvestigation.getSample().getId().equals(sampleGrid.getId()) && sampleGrid.getStatus().equals("n"))
			{
			%>
           ...
           <%}}}
			 ...
			 while(itrGridCollectionList.hasNext())
            {
             ...
             if(dgmasInvestigation.getCollection() != null && !dgmasInvestigation.getCollection().equals("")){
            	if(dgmasInvestigation.getCollection().getId().equals(collectionGrid.getId()) && collectionGrid.getStatus().equals("y"))
			 {
			 %>
				data_arr[<%= counter%>][14] = "<%=collectionGrid.getCollectionName()%>"
			<%
			}
			 else if(dgmasInvestigation.getCollection().getId().equals(collectionGrid.getId()) && collectionGrid.getStatus().equals("n"))
			{
			%>
           ...
           <%}}}
			...
			<%-- <%if(dgmasInvestigation.getLionCodeId()!=null){%>
				data_arr[<%= counter%>][15] = "<%=dgmasInvestigation.getLionCodeId().getComponent()%>"
			<%}else{ %>
			...
			<%}%>
			<% 
			if(dgmasInvestigation.getBloodBankScreenTest() !=null && dgmasInvestigation.getBloodBankScreenTest().equals("y")){%>
			data_arr[<%= counter%>][20] = "<%=dgmasInvestigation.getBloodBankScreenTest()%>"
			<%}else{%>
			data_arr[<%= counter%>][20] = ""
			<%}%>
			<%if(dgmasInvestigation.getBloodReactionTest() !=null && dgmasInvestigation.getBloodReactionTest().equals("y")){%>
			...
			<%if(dgmasInvestigation.getStatus() !=null && dgmasInvestigation.getStatus().equals("y")){ %>
