...
<%
	...
	if(map.get("masDietTypeList")!=null)
	{
		masDietTypeList = (List<MasDietType>)map.get("masDietTypeList");
	}
	String userName = "";
...
<input type="radio" name="<%=SELECTED_RADIO %>" value="2" class="radiobutMargin" />
 <input type="text" id="searchField" name="<%= SEARCH_FIELD%>"
	value="" validate="Diet,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'canteen?method=searchDiet')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','canteen?method=searchDiet','checkSearch')"
...
<input type="button" name="Report" value="Generate Report"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_diet">
		...
		if(searchDietList.size()>0)
		 {
			...
			{
	%> <a href="canteen?method=showDietJsp">Show All Records</a> <%
			}
		 }
...
<form name="diet" method="post" action=""><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
<div class="clear"></div>
<div class="Block">
<label><span>*</span> Diet Code: </label> <input
	id="codeId" type="text" name="<%= CODE%>" value=""
	validate="Diet Code,string,yes" class="textbox_size20" MAXLENGTH="8"
	/ tabindex=1><label id=biglabel><span>*</span>Diet
Name:</label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Diet Name,string,yes" class="textbox_size20" MAXLENGTH="30"
	/ tabindex=1
	onkeypress="return submitenter(this,event,'canteen?method=addDiet')"><script>
		document.diet.<%=CODE%>.focus();
	</script> 
	<div style="display: none">
	<label>
	<span>*</span> Type: </label> <select
	name="<%=DIET_CATEGORY_NAME %>" validate="Type,string,no">
	<option value="">Select</option>
	<%for(MasDietType dietType : masDietTypeList){ %>
...
<input type="button" name="add" id="addbutton" value="Add"
	class="button" onClick="submitForm('diet','canteen?method=addDiet');"
          ...
          while(itr.hasNext())
           {
            ...
            /*  MasDietType dietType = new MasDietType();
             dietType =(MasDietType)masDiet.getDietType(); */
%>
...
<%-- <% if(dietType.getDietTypeName()!=null){ %>
data_arr[<%= counter%>][3] = "<%=dietType.getDietTypeName()%>"
<%}else{%> --%>
data_arr[<%= counter%>][3] = ""
<%-- <%}%> --%>
data_arr[<%= counter%>][4] = "<%= masDiet.getLastChgBy()!=null?masDiet.getLastChgBy().getId():"" %>"
