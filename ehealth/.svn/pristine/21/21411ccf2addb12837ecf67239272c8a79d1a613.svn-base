...
<input type="hidden" name="colCode" value="relation_code">
<input type="hidden" name="colName" value="relation_name">
...
<input type="hidden" name="pojoPropertyCode" value="RelationCode">
<div class="paddingTop5"></div>
<div class="clear"></div>
<div class="Block">
<label><span>*</span> Relation Code  </label> 
<input id="codeId" type="text" name="<%= CODE%>" value="" validate="Relation Code,string,yes" class="textbox_size20" MAXLENGTH="8" / tabindex=1> 
<label><span>*</span> Relation Name </label> 
<input type="text" name="<%= SEARCH_NAME %>" value="" validate="Relation Name,string,yes" class="textbox_size20" MAXLENGTH="30" / tabindex=1 onkeypress="return submitenter(this,event,'generalMaster?method=addRelation')">
<script>
...
<div class="clear"></div>
<div id="edited"></div>
<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('relation','generalMaster?method=addRelation');" accesskey="a" tabindex=1 /> 
<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button" onClick="submitForm('relation','generalMaster?method=editRelation')" accesskey="u" tabindex=1 /> 
<input type="button" name="Delete" id="deletebutton" value="Activate" style="display: none;" class="button" onClick="submitForm('relation','generalMaster?method=deleteRelation&flag='+this.value)" accesskey="d" tabindex=1 /> 
...
data_header[0][0] = "Relation Code"
data_header[0][1] = "data";
data_header[0][2] = "25%";
data_header[0][3] = "<%= CODE%>"

data_header[1] = new Array;
data_header[1][0] = "Relation Name"
...
data_arr[<%= counter%>][1] = "<%=masRelation.getRelationCode()%>"
data_arr[<%= counter%>][2] = "<%= masRelation.getRelationName()%>"

data_arr[<%= counter%>][3] = "<%= masRelation.getLastChgBy() %>"
data_arr[<%= counter%>][4] = "<%= HMSUtil.convertDateToStringWithoutTime(masRelation.getLastChgDate()) %>"
