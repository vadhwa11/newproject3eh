...
<div class="clear"></div>
<form name="opd_OP_Clinic_Waiting_PatientList_new" method="post" action=""  onload="javascript:submitProtoAjaxWithDivName('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/investigation?method=showLabResultDetailAlert','alertdiv');">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
<input type="hidden" name="hospitalId" id="hospitalId" value="<%=hospitalId %>" />
<input type="hidden" name="deptId" id="deptId" value="<%=deptId %>" />
<input type="hidden" id="doctorId" name="doctorId" value="<%=userId %>">
...
function openDetail(visitId,rowVal,skipped){
	...
	if(rowVal==0  && searchFlag=='0'){
		submitFormForButton('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/opd?method=showOpdDetailJsp&visitId='+visitId);
	}else{
		   if(alertBeforeSubmit(skipped)){
			   submitFormForButton('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/opd?method=showOpdDetailJsp&visitId='+visitId+'&skipped='+skipped.value);
		   }
	}
}
var bReleaseClick='N';
function openOpdLite(visitId,rowVal){
	var skipped = ''
	if(bReleaseClick == 'N'){
	var searchFlag = 	document.getElementById('searchFlag').value;
	if(rowVal==0 && searchFlag=='0'){
		submitFormForButton('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/opd?method=showOpdLiteJsp&visitId='+visitId);
		   ...
		   if(confirm("You are Skipping the patient ?")){
			   submitFormForButton('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/opd?method=showOpdLiteJsp&visitId='+visitId+'&skipped=E&searchFlag='+searchFlag);
		   }
	}   
	}
}
...
var bReleaseClick='N';
	function openWindows(){
	    obj1 = eval('document.opd_OP_Clinic_Waiting_PatientList_new');
	    var url = "/hms/hms/opd?method=getOPClinicalReadViewList&"+csrfTokenName+"="+csrfTokenValue+"&opdType=opdLite";
	    listSizeAlert();
	    obj1.action = url;
		obj1.submit();
 ...
 */	}
	...
	function validate(){
		...
		if(!chkInputLength(patientName))
			...
			submitForm('opWaitingList','/hms/hms/opd?method=showNewOPWaitingList&'+csrfTokenName+'='+csrfTokenValue);
	}
						...
						if(msg.indexOf("success~~~true") != -1)
							{
									...
									bReleaseClick ='N';
									window.location ="opd?method=showNewOPWaitingList&selectedAppId=A2168";
	...
	function callWaitingList(){
	   ...
	   if(parkFlag=='p'){
		   submitProtoAjaxWithDivName('opWaitingList','/hms/hms/opd?method=getOPWaitingListRefresh&flag=p&newOP=yes','test');
	   }else{
		   submitProtoAjaxWithDivName('opWaitingList','/hms/hms/opd?method=getOPWaitingListRefresh&searchFlag='+searchFlag+'&tokenNo='+tokenNo+'&patientName='+pName+'&uhid='+hinNo+'&newOP=yes','test');  
	   }
	...
	function showParkPatient(status) {
		...
		document.opd_OP_Clinic_Waiting_PatientList_new.action = "/hms/hms/opd?method=showNewOPWaitingList&flag="
			+ status+'&opdType=opdLite';
	document.opd_OP_Clinic_Waiting_PatientList_new.submit();
	}
	...
	/* function bookCounterForDoctor(){
		...
		if(counterId!='0')
			submitProtoAjax('opd_OP_Clinic_Waiting_PatientList_new', action);
	} */
	...
	function checkForCounter(){
	 ...
	 function transferToCommonPool(employeeId,page){
		 ...
		 var url="/hms/hms/opd?method=showPlannedLeaveCommonPoolJsp&fromScreen=opd_OP_Clinic_Waiting_PatientList_new&"+csrfTokenName+"="+csrfTokenValue;
