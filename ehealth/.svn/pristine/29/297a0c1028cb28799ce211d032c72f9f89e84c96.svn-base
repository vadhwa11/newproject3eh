...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	id="voucherDetails" class="cmntable">
	...
	<td> <input id="accountNameId<%=i%>" type="text" size="30"  name="accountName<%=i%>" value="" validate="Account,metachar,yes" tabindex=1 onblur="validateAccountName(this.value,<%=i %>,'slId')" />
		...
		<script type="text/javascript" language="javascript" charset="utf-8">
					 function eventCallback(element, entry){
						return entry + "&amtType=" + document.getElementById('hiddenDrCrId').value+"&salesVoucherType=salesReturn";
					}
	...
	<td><select id="costCenterId<%=i %>"  name="<%=COST_CENTER_ID %><%=i%>" validate="Cost Center,metachar,no" class="medium" tabindex=1>
	<option value="0">Select</option>
...
<td> <input id="drAmountId<%=i %>" type="text"  size="10" name="<%=DR_AMOUNT%><%=i%>" value="" disabled="disabled" validate="DrAmount,float,no" onblur="totalDrCrAmount('dr');"  MAXLENGTH="12" tabindex=1 /></td>
	...
	<tr class="background">
		...
		<td><input id="totalDrAmountId" type="text" size="10" class="readOnlySmall" readonly="readonly" name="<%=TOTAL_DR_AMOUNT %>" validate="Total DrAmount,float,yes"  value=""   MAXLENGTH="15" tabindex=1 /></td>
	...
	<tr class="background">
		<td colspan=6 class="right">Total Sales Return Control Fig.</td>
...
<input type="button" name="add" id="addbutton" value="Add TO Template" class="buttonBig" onClick="addNarrationInAjax('salesReturnVoucher');" accesskey="a" tabindex=1 />
<div class="clear"></div>
<div class="clear"></div>
<div class="clear"></div>
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Submit" class="button" onClick="submitForm('salesReturnVoucher','account?method=submitSalesReturnVoucher','validateNarration','validateTotalDrCr');" accesskey="a" tabindex=1 />
...
function validateAccountName( strValue,inc,tdid ) {
 	...
 	if(strValue != "")
	{
		    ...
		    if(id!="")
		    {
		    	submitProtoAjaxWithDivName('salesReturnVoucher','account?method=getSubLedgerForAccount&accName='+encodeURIComponent(strValue)+'&rowVal='+inc,tdid+inc);
		    }
	}
}
