...
<%@page import="jkt.hms.masters.business.HesEquipmentCallLogEntry"%>
...
<div class="clear"></div>
<form name="eCallLogForm" method="post" action="">
<div id="edited"></div>
<label>&nbsp;</label>
...
<%		
			...
			{
				for(HesEquipmentCallLogEntry hesCallLogObj : searchList){
	        	int counter = 0;
%>					
					data_arr[<%= counter%>] = new Array();
					
					data_arr[<%= counter%>][0] = <%= hesCallLogObj.getId()%> 			
					data_arr[<%= counter%>][1] =   '<input type="radio"  name="parent" value="<%=hesCallLogObj.getId()%>" id="radio" />'
					data_arr[<%= counter%>][2] = "<%=hesCallLogObj.getEntryNo()%>"
					data_arr[<%= counter%>][3] = "<%= hesCallLogObj.getDate() %>"
					data_arr[<%= counter%>][4] = "<%= hesCallLogObj.getEquipmentMaster().getEquipmentName() %>"
					data_arr[<%= counter%>][5] = "<%= hesCallLogObj.getStatus() %>"
<% 
					if(hesCallLogObj.equals("y"))
					{
%>
