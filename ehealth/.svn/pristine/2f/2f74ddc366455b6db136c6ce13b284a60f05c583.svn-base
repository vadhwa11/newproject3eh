...
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onclick="setdate('',document.journalVoucher.<%=VOUCHER_DATE%>,event)" />
...
function validateAccountName( strValue,inc,tdid ) {
 	...
 	if(strValue != "")
	{
		...
		//	idx1++;
		    ...
		    if(id!="")
		    {
		    	submitProtoAjaxWithDivName('journalVoucher','account?method=getSubLedgerForAccount&accName='+encodeURIComponent(strValue)+'&rowVal='+inc+'&financialYearId'+=financialYearId,tdid+inc);
		    }
	}
