...
<div class="clear"></div>
	...
	<div id="labelsDiv">
		...
		<label id="Ecode"> <%=employee.getEmployeeCode() %> </label>
		<label>Employee Name</label>
		<label id="Ename"><%=employee.getFirstName()+" "+employee.getLastName()%></label>
		<label>Year</label>
		<select id="invYear" name="invYear"> 
		<%
		      for(HrMasFinancialYear obj : financialYearList)
			 
		   		{%>
		<option value="<%=obj.getId() %>"><%=obj.getFinancialYear() %></option>
		<%} %>
		 </select>
	</div>
	...
	<table id="secInvestList" width="100%" >
 	 <tr><th>Section</th>	<th>Investment</th> <th>Amount</th><th>Investment Date</th><th>Max. Amount</th><th>Proofs</th></tr>		
		<tbody id = "tableData12">
			...
			if(hrMasItaxSecInvestmentList!=null && hrMasItaxSecInvestmentList.size()>0)
			{
			  ...
			  if(i%2==0)
				...
				<td  width="10%" id="td3<%=i %>"><div align="center"> <input type="text" name="amount<%=i %>" id="amount<%=i %>" validation='Amount,int,yes'  onchange="chkIfEmpSelected(this)" onblur="setDateOfEntry(this.value,'<%=i %>')"></div></td>
				<td  width="20%" id="td4<%=i %>">
				<input type="text" id="date<%=i %>" name="date<%=i %>" value="" class="date" readonly="readonly" validate="Date,date,no"  MAXLENGTH="30" />
				<img src="/hms/jsp/images/cal.gif" onClick="javascript:setdate('',document.getElementById('date<%=i %>'),event)" width="16" height="16" border="0" validate="Pick a date" class="calender" /> 
	...
	<div class="clear"></div>
	<div class="paddingTop15"></div>
	<input type="button" name="save" value="Save" class="button" onclick="submitForm('employeeInvestment','/hms/hrms/incomeTaxMaster?method=saveEmpIndividualInvestment&'+csrfTokenName+'='+csrfTokenValue)">
