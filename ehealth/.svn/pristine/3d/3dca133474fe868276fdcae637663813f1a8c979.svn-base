...
import in.gov.uidai.authentication.common.types._1.Other;
import in.gov.uidai.authentication.common.types._1.Token;
...
import in.gov.uidai.authentication.uid_auth_request._1.Rc;
...
public class AuthRequestCreator {
	...
	public static Auth createAuthRequest(String aua, String sa, String licenseKey, Uses uses,
			Tkn token, AuthDataFromDeviceToAUA auaData, Meta metaData,String otpTxn) {
		...
		try {
			...
			Txn txn = new Txn();
			otpTxn="UKC:"+otpTxn;
			String txnVal =otpTxn;// createTxn(aua);//changed by govind 10-10-2017
			txn.setValue(txnVal);
			auth.setTxn(txn);

			SubAUACode subAUACode = new SubAUACode();
			subAUACode.setValue(sa);
			auth.setSubAUACode(subAUACode);
			
			ReqType reqType = new ReqType();
			reqType.setValue("auth");
			auth.setReqType(reqType);
			...
			System.out.println("ttttttt"+auaData.getSessionKeyDetails().getKeyIdentifier());
