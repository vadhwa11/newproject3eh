...
<div class="clear"></div>
<label>IP No.</label> <input type="text" name="ipno" id="ipno" value=""></input>

<label>Patient Name</label> <input type="text" name="pname" id="pname" value=""></input>
   
<div class="clear"></div>

<input type="button" name="Search" value="Search" class="button"
	onClick="submitForm('search','/hms/hms/ot?method=showMinorOtWaitingList');" />
 ...
 <table>
    ...
    %>
          ...
          <%--   <form name="servicRequest<%= counter%>" method="post"> --%>
            ...
            <tr onclick="submitForm('itemGrid', 'ot?method=showOTBookingJsp&surgeryHdId=<%=(Integer)object.getId()%>&otType=Minor')" style="cursor: pointer;">
            <%-- <td><%= counter%></td>
	...
	<%if(currentPage !=1){%>
		<a href='/hms/hms/ot?method=showMinorOtWaitingList&page=<%=currentPage-1%>'>Previous</a>
	<%}
	if(noOfPages>=1){%>
		<a href='/hms/hms/ot?method=showMinorOtWaitingList&page=<%=currentPage%>'><%=currentPage%></a>
	<%}
	if(currentPage <noOfPages){%>
		<a href='/hms/hms/ot?method=showMinorOtWaitingList&page=<%=currentPage+1%>'>Next</a>
	<%}%>
	<div class="Block">
...
function searchParticularPage(type){
	...
	submitForm('search','/hms/hms/ot?method=showMinorOtWaitingList&page='+curPage);
}
</script>