...
public class PatientSampleCollectionDataToCentralServerSchedular extends
		QuartzJobBean {

	private static final Logger logger = org.apache.log4j.Logger
			.getLogger(PatientSampleCollectionDataToCentralServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		String ack = null;
		List<CentralServerSampleData> centralServerSampleDatas = null;
		Map<String, Object> dataMap = labDataService
				.getPatientSampleCollectionDataForCentralServer();
		if (dataMap.get("centralServerSampleDatas") != null) {
			centralServerSampleDatas = (List<CentralServerSampleData>) dataMap
					.get("centralServerSampleDatas");
		}
		if (centralServerSampleDatas != null) {
			for (CentralServerSampleData centralServerSampleData : centralServerSampleDatas) {
	...
	private String hl7MessageSender(String msg, MasHospital hospital) {
		...
		try {
			...
			logger.debug("MalformedURLException in Patient Sample Collection Data To Central Server Schedular");
		} catch (ConnectException e) {
			logger.debug("Connection refused in Patient Sample Collection Data To Central Server Schedular");
		} catch (SocketTimeoutException e) {
			logger.debug("Connection timeout in Patient Sample Collection Data To Central Server Schedular");
		} catch (IOException e) {
			logger.debug("IOException in Patient Sample Collection Data To Central Server Schedular");
		}
