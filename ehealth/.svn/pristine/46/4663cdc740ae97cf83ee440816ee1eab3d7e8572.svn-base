...
<div class="titleBg">
<h2>Pending List For Cell Grouping</h2>
</div>
...
<input type="text" class="date"
	...
	validate="FDate,date,yes" MAXLENGTH="10" tabindex="1" /> 
	<img
...
<table id="" width="100%" border="0" cellspacing="0" cellpadding="0">
 <thead>
		...
		<%if(null !=breh.getSampleCollection()){ %>
		...
		<td></td>
		<td></td>
		<% }if(breh.getDonor() !=null){%>
		<td><%=breh.getDonor().getDonationNo() %></td>
		<%}else{ %>
		<td></td>
		<%} %>
		</tr>
...
function populateResultEntry(Id){
	...
	window.location.href = 'bloodBank?method=resultEntryFormContJsp&Id='+Id+'&'+csrfTokenName+'='+csrfTokenValue; 
} 
