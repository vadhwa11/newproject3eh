...
<%
	...
	List<InjAppointmentHeader>pendingInjectionAppList=(List<InjAppointmentHeader>)map.get("pendingInjectionAppList");
	%>
<div class="titleBg">
<h2><%=depName %></h2>
</div>
...
<script type="text/javascript" language="javascript">
	...
	try{
	String st="";
	if(pendingInjectionAppList!=null){
		Iterator iterator=pendingInjectionAppList.iterator();
		while(iterator.hasNext())
		{	
			InjAppointmentHeader injAppointmentHeader=(InjAppointmentHeader)iterator.next();
			int injHeaderId=injAppointmentHeader.getId();
			Visit visit=injAppointmentHeader.getVisit();
			//String patientStatus=visit.getVisitStatus();
			...
			String age="";
			String priority="";
			if(injAppointmentHeader.getStatus().equalsIgnoreCase("p")){
