...
<div class="Block">
<h2>Waiting List Of REFERRING A POSTMORTEM EXAMINATION</h2>
...
<tbody id="tableData">
	...
	%> 
			...
			<tr onclick="submitForm('mlc<%=counter+1%>', 'mlc?method=showFormatForReferring')">
			<td><%= counter+1%></td>
          ...
          <td><%= mlcDetail.getVisit().getHin()!=null ? mlcDetail.getVisit().getHin().getHinNo():""%></td>		    
		    </tr><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"></form>
<%		++counter;
				} 
 	%> 
 	</tbody>
