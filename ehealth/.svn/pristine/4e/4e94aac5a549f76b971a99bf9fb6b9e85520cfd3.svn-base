...
<%
	...
	List<MasSession> sessionList = null; // added by amit das on 29-09-2016
	...
	if(map.get("sessionList")!=null){
		sessionList=(List<MasSession>)map.get("sessionList");
	}
		...
		<tr>
			...
			<td>
				...
				<%if((listA.getFinalStatus()!=null && listA.getFinalStatus().equalsIgnoreCase("n")) && (listA.getAdministrator()!=null &&  listA.getAdministrator().equalsIgnoreCase("n"))) {%>
						...
						<td><input type="text" readonly="readonly" value="<%=listA.getPrescriptionDetails().getDosage() %>" /></td> 
			   	<%}else{%>
			   		  <%if(batchList.size()>0){ %>
						<td><input type="text" readonly="readonly" id ="availableStock<%=i %>"  name="availableStock<%= i%>" /></td>
						<td><input type="text" id ="issueStock<%=i %>" name="issueStock<%= i%>" value="<%=listA.getPrescriptionDetails().getDosage() %>" readonly="readonly" /></td>
					   <%}%>
			   	<%} %>
			...
			<!-- added by amit das on 29-09-2016 -->
			<%--commented by govind 21-08-2017<td>
			<select name="sessionId<%=listA.getId()%>" id="sessionId<%=listA.getId()%>">
				<% for(MasSession masSession : sessionList) {%>
					<option value="<%=masSession.getId()%>"><%=masSession.getSessionName()%></option>
				<% } %>
			</select>
			</td> commented by govind 21-08-2017 end--%>
			
			
			<td>
			<input type="hidden" name="sessionId<%=listA.getId()%>" id="sessionId<%=listA.getId()%>" value="<%=listA.getSession()!=null?listA.getSession().getId():"0" %>"/>
			<input type="text" name="remarks<%=listA.getId()%>" id="remarks<%=listA.getId()%>" value="<%=listA.getNursingRemark()!=null?listA.getNursingRemark():"" %>" tabindex="1" size="30"	 /></td>
			<%
			...
			else
			{
				...
				if(sd1.equalsIgnoreCase(HMSUtil.convertDateToStringWithoutTime(listA.getInjAppointmentDate()))&& listA.getStatus().equalsIgnoreCase("p")){
				%>
				
					<td><input type="button" class="button" readonly="readonly" name="save<%=listA.getId()%>" id="save<%=listA.getId()%>" value="Submit" onclick="saveNursingCareValues(<%=listA.getId() %>,<%=i %>);" />
				<%
				}
				else
				{
...
function saveNursingCareValues(Id,rowVal)
{
		...
		var AppointmentFlag = '<%out.print(AppointmentFlag);%>';
		 var AppointmentDate ="";
		if(document.getElementById('appDate'))
	    	 AppointmentDate = document.getElementById('appDate').value;
	    var remarks = document.getElementById('remarks'+Id).value;
	    var sessionId = document.getElementById('sessionId'+Id).value;
	    
	    //added by govind 21-01-2017  
	    if(document.getElementById("itemBatchStock"+rowVal)!=null){
	    var itemBatchStock1=document.getElementById("itemBatchStock"+rowVal).value;
	    if(itemBatchStock1==0){
	    	alert("Please select batch no");
			return false;
	    }
	    }
	    //added by govind 21-01-2017  end
	   ...
	   var url = "opd?method=submitNursingCare&"+csrfTokenName+"="+csrfTokenValue+"&immunizationInj="+document.getElementById("immunizationInj").value+"&vaccinId="+document.getElementById("vaccinId").value+"&hinId="+document.getElementById("hinId").value+"&sessionId="+sessionId;
	    ...
	    if(AppointmentFlag == 'y')
	    	{
	    		if(document.getElementById('appDate').value == "")
	    		 ...
	    		 //added by govind 21-01-2017 
	    		 if(document.getElementById("appointmentDate"+rowVal)!=null){
	    		var prevAppDate = (document.getElementById('appDate')+(rowVal-1)).value;
		    	 var curDate = new Date();
		 	    var apptDate = new Date(AppointmentDate.substring(6),
		 	    		(AppointmentDate.substring(3, 5)-1), AppointmentDate.substring(0, 2));
		 	    if(prevAppDate){
		 	   var prevAppDateNew = new Date(prevAppDate.substring(6),
		 	    		(prevAppDate.substring(3, 5)-1), prevAppDate.substring(0, 2));
		 	    
		 	   //alert("prevAppDateNew "+prevAppDateNew);
		 	   // alert("apptDate "+apptDate);
		 	    if(apptDate<=prevAppDateNew){
		 	    	alert("please select next correct date");
		 	    	return false;
		 	    }
		 	    }
	    		 }
