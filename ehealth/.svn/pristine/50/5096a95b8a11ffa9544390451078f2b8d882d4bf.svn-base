...
prop.load(in);
%>
<form name="RELReport" action="" target="_blank" method="post">
...
<% } %>		
	...
	<input type="button" value="Generate Report" class="button"
		onclick="submitForm('RELReport','/hms/hms/investigation?method=generateResultEntryLabReport&flag=1','checkFromNTodata','chkPatientTypeForBillno');"
		accesskey="a" id="submitButtonId" />
		
<!--//Note: Revert the changes for story NO :37
		 <input type="button" value="Generate Report HTML" class="button"
		onclick="submitForm('RELReport','/hms/hms/investigation?method=generateResultEntryLabReport&flag=2','checkFromNTodata','chkPatientTypeForBillno');"
		accesskey="a" id="submitButtonId1" /> -->
