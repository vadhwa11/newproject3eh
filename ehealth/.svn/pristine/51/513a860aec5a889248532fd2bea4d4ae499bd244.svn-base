...
<%
	...
	List<HesCylinderUsageEntryHeader> hesRoutineWorkList = new ArrayList<HesCylinderUsageEntryHeader>();
	...
	if (map.get("hesRoutineWorkList")!= null)
		hesRoutineWorkList = (List<HesCylinderUsageEntryHeader>)map.get("hesRoutineWorkList");
...
<%
		String dt = "";
		String deliveryDate = "";
		for (HesCylinderUsageEntryHeader usageEntryHeader : hesRoutineWorkList )
		{
			dt = HMSUtil.convertDateToStringWithoutTime(usageEntryHeader.getEntryDate());
			deliveryDate = dt;
%>
...
<input type="text" id="entryNo" name="entryNo" value="<%=usageEntryHeader.getEntryNo()%>" tabindex=1 readonly="readonly" />
...
<table border="0" cellspacing="0" cellpadding="0" id="chargeDetails">
	...
	<%
		...
		{
			Cdate = (Date)orderWork[1];
			dt_challan = HMSUtil.convertDateToStringWithoutTime(Cdate);
	...
	<tr>
		...
		<td>	
		<input type="text" id="<%=CHALLAN_DATE%><%=inc%>" name="<%=CHALLAN_DATE%>" value="<%=challanDate %>" class="date" readonly="readonly" 
		...
		<%
				...
				{
					if(orderWork[2].equals(cylinderTypeMaster.getId()))
					{
		%>
		...
		<td width="10%"><input id="qty<%=inc%>" name="qty" type="text" size="5" MAXLENGTH="5" value="<%=orderWork[3]%>" 
		...
		<%
			...
			{
				if(orderWork[4].equals(hesUsageMaster.getId()))
				{
		%>
					<option value=<%=hesUsageMaster.getId()%> selected="selected"><%=hesUsageMaster.getCylinderUsageName()%></option>
		<%
				}
				else
				{
		...
		<option value="0">Select</option>
		<%
			if(orderWork[5].equals("R"))
			{
		%>		
				<option value="R" selected="selected">RKS</option>
				<option value="V">VBCH</option>
		<%
			}
			else
			{
		%>
				<option value="V" selected="selected">VBCH</option>
				<option value="R">RKS</option>
		<%
			}
		%>			
		...
		<td>
			<input id="remarks<%=inc%>" name="remarks" type="text" size="20" value="<%=orderWork[6]%>"/>
		</td>
		...
		<%
			for(int b = 0; b < hesClosingList.size(); b++)
			{
				...
				if(orderWork[2].equals(Id[0]))
				{
		%>
		<td>
			<input id="closingStock<%=inc%>" name="closingStock" type="text" value="<%=Id[1]%>" size="5" readonly="readonly" />
			<input id="ItemID<%=inc%>" name="ItemID" type="text" value="<%=orderWork[2]%>" size="5" readonly="readonly" />
		</td>
	</tr>
	<%
				}
...
<input type="button" name="sss" align="right" class="button" value="Submit"
 onclick="if(checkFilledRow()){submitForm('purchaseGrid','hes?method=updateCylinderUsageFormEntry');}" />
