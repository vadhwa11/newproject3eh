...
<form name="salesVoucher" method="post" action="">
<%
...
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onclick="setdate('',document.salesVoucher.<%=VOUCHER_DATE%>,event)" />
...
<input type="hidden" value="dr" name="hiddenDrCr" id="hiddenDrCrId" />
<div class="clear"></div>
</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	id="voucherDetails" class="cmntable">
	...
	<td> <input id="drAmountId<%=i %>" type="text"  size="10" name="<%=DR_AMOUNT%><%=i%>" value="" onblur="totalDrCrAmount('dr');"  MAXLENGTH="12" tabindex=1 /></td>
	<td> <input id="crAmountId<%=i %>" type="text"  size="10" name="<%=CR_AMOUNT%><%=i%>" value="" disabled="disabled" onblur="totalDrCrAmount('cr');"  MAXLENGTH="12" tabindex=1 /></td>
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Submit" class="button" onClick="submitForm('salesVoucher','account?method=submitSalesVoucher','validateNarration');" accesskey="a" tabindex=1 />
...
function validateAccountName( strValue,inc,tdid ) {
 	...
 	if(strValue != "")
	{
		    ...
		    if(id!="")
		    {
		    	submitProtoAjaxWithDivName('salesVoucher','account?method=getSubLedgerForAccount&accName='+encodeURIComponent(strValue)+'&rowVal='+inc+'&financialYearId='+financialYearId,tdid+inc);
		    }
	}
}
