...
<select name="<%=OBJECT_HEAD_ID %>" id="objectHead" onblur="poppulateobject(this.value)" >
<option value="0">Select</option>
<%
for(BudEstimateEntry objectHead:searchEstimateEntryList){
%>
<option value="<%=objectHead.getId() %>">
<%=objectHead.getObjectHeadId().getObjectHeadName() %></option>
<%}%>
</select>
