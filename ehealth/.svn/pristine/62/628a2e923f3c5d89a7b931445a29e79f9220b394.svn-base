
package com.jkt.entities.indentkmcl;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for anonymous complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType>
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="IndentNo" type="{http://www.w3.org/2001/XMLSchema}string"/>
 *         &lt;element name="HospitalName" type="{http://www.w3.org/2001/XMLSchema}string"/>
 *         &lt;element ref="{http://www.jkt.com/entities/indentkmcl}ItemListType" maxOccurs="unbounded"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "indentNo",
    "hospitalName",
    "itemListType"
})
@XmlRootElement(name = "IndentType")
public class IndentType {

    @XmlElement(name = "IndentNo", required = true)
    protected String indentNo;
    @XmlElement(name = "HospitalName", required = true)
    protected String hospitalName;
    @XmlElement(name = "ItemListType", required = true)
    protected List<ItemListType> itemListType;

    /**
     * Gets the value of the indentNo property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getIndentNo() {
        return indentNo;
    }

    /**
     * Sets the value of the indentNo property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setIndentNo(String value) {
        this.indentNo = value;
    }

    /**
     * Gets the value of the hospitalName property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getHospitalName() {
        return hospitalName;
    }

    /**
     * Sets the value of the hospitalName property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setHospitalName(String value) {
        this.hospitalName = value;
    }

    /**
     * Gets the value of the itemListType property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the itemListType property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getItemListType().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link ItemListType }
     * 
     * 
     */
    public List<ItemListType> getItemListType() {
        if (itemListType == null) {
            itemListType = new ArrayList<ItemListType>();
        }
        return this.itemListType;
    }

}
