...
public class PatientVisitDataToLeanServerSchedular extends QuartzJobBean {
	
	private static final Logger logger = org.apache.log4j.Logger.getLogger(PatientVisitDataToLeanServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		Map<String,Object>	dataMap=registrationDataService.getPatientVisitDataForLeanServer();
		if(dataMap.get("leanServerVisitData")!=null){
			leanServerVisitDatas=(List<LeanServerVisitData>)dataMap.get("leanServerVisitData");
		} 
		...
		for(LeanServerVisitData lPatVisitData:leanServerVisitDatas){
			if(lPatVisitData.getHospitalId()!=null) { 
				...
				dataMap = registrationDataService.getHospitalData(objectMap);
				if(dataMap.get("hospital")!=null){ 
					hospital = (MasHospital)dataMap.get("hospital");
					ack=hl7MessageSender(lPatVisitData.getVisitData(),hospital); 
					if("success".equalsIgnoreCase(ack)){ 
						registrationDataService.updateLeanServerPatientVisitData(lPatVisitData);  
					}
				}	
			}
		   }
	...
	private String hl7MessageSender(String msg,MasHospital hospital) {
		...
		try {
			...
			logger.debug("EncodingNotSupportedException in Patient Visit Data To Lean Server Schedular");
		} catch (HL7Exception e) {
			logger.debug("HL7Exception in Patient Visit Data To Lean Server Schedular");
		} catch (ConnectException e) {
			logger.debug("Connection refused in Patient Visit Data To Lean Server Schedular");
		}catch (SocketTimeoutException e) {
			logger.debug("Connection timeout in Patient Visit Data To Lean Server Schedular");
		}catch (IOException e) {
			logger.debug("IOException in Patient Visit Data To Lean Server Schedular");
		} catch (DecodeException e) {
			logger.debug("DecodeException in Patient Visit Data To Lean Server Schedular");
		} catch (EncodeException e) {
			logger.debug("EncodeException in Patient Visit Data To Lean Server Schedular");
		}
