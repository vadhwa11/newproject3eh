...
<select id=""patientCatSearch	name="patientCatSearch" validate="Patient Category,string,no">
...
<select id="btSearchId"	name="billTypeSearch" validate="Bill Type,int,no" onchange="displayDrugList(this.id);">
	<option value="0">Select</option>
	<%
...
<input type="button" name="Search" value="Search"  class="button"	onClick="submitForm('search','billingMaster?method=searchDiscountList');" />
...
<form name="discountMaster" method="post" action="">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
<div class="paddingTop15"></div>
<div class="clear"></div>
<div class="Block">
<input type="hidden" value="true" name="localdiscount" />
<div id="testDiv">
<%-- <div class="clear"></div>
...
<label id="mainchargeId">Main Service Head </label> 
...
<select id="chargeCodeId"	name="<%= CHARGE_CODE_ID%>" validate="Service,string,no"	onchange="displayMainSubCharge(this.value)">
	<option value="0">Select</option>
	<%
for(Object[] masChargeCode :ChargeCodeList) {
%>
...
<script type="text/javascript">
		...
		<%
			...
			for (MasEmployee emp :employeeList)
			{
				...
				{
					if(emp.getId().equals(empDep.getEmployee().getId())){
								%>
...
<input 	id="varianceId" type="text" name="<%=DISCOUNT_PERCENTAGE %>" value=""	validate="Discount percentage,float,no" MAXLENGTH="3" tabindex=1	onblur="disableField(this.id,this.value);" />
...
<%
			...
			%>
data_arr[<%= counter%>][1] = "<font id='error'>*</font>Parent InActivated--<%=patientType.getPatientTypeName()%>";
		<%
...
<%
		if(discount.getInsuranceAmt() != null){
%>
		data_arr[<%= counter%>][3] = "<%=discount.getInsuranceAmt()%>"
...
<%
	for(MasEmployee employee: employeeList){
		if(discount.getEmployee() != null){
			if(discount.getEmployee().getId() == employee.getId() && discount.getStatus().equals("y")){
%>
data_arr[<%= counter%>][4] = "<%=employee.getFirstName()%>"
<%
...
<%
	...
	StringBuffer output_str = new StringBuffer();
	StringTokenizer s = new StringTokenizer(charge[1].toString(),"\"");
...
if(discount.getChargeCode().getId() == charge[0] && discount.getStatus().equals("y")){

	%>
	data_arr[<%= counter%>][13] = "<%= output_str.toString()%>";

<%
		}else if(discount.getChargeCode().getId() == charge[1] && discount.getStatus().equals("n")){
			%>
data_arr[<%= counter%>][13] = "<font id='error'>*</font>Parent InActivated--<%=charge[0]%>";
		<%}}else{%>
			data_arr[<%= counter%>][13] = "";
		<%}
		} %>
...
<%
}else if(discount.getBplStatus()!=null && discount.getBplStatus().equalsIgnoreCase("y"))
{%>
...
<%}%>
	data_arr[<%= counter%>][25] = "<%=discount.getOtherCategory()!=null?discount.getOtherCategory().getPatientTypeName():""%>"
		data_arr[<%= counter%>][26] = "<%=discount.getFromAge()!=null?(discount.getFromAge()+" Year"):""%>"
			data_arr[<%= counter%>][27] = "<%=discount.getToAge()!=null?(discount.getToAge()+" Year"):""%>"
...
function onChangeItemGRoup()
{
	...
	submitProtoAjaxWithDivName('discountMaster','/hms/hms/billingMaster?method=getItemCategoryDetails','divForItemGroup');
	getCxcludeItem();
}
...
{
submitProtoAjaxWithDivName('discountMaster','/hms/hms/billingMaster?method=getChargeForExclude','excludecharge');
}

function getCxcludeItem()
{
submitProtoAjaxWithDivName('discountMaster','/hms/hms/billingMaster?method=getItemForExclude','excludeitem');
}
...
function submitDiscountMaster()
{
	...
	if(billType == "<%=serviceId%>"){
		document.getElementById('mainChargeCodeId').setAttribute('validate','Main Service Head,int,yes');
	...
	if(discountType != ""){
		if(discountType == 'F'){
			...
			document.getElementById('varianceId').setAttribute('validate','Discount(%),int,no'); 
		}
		else {
			...
			if(discountType == 'D'){
				 ...
				 document.getElementById('varianceId').setAttribute('validate','Tariff(%),int,no'); 
				 if(document.getElementById('varianceId').value!="" && document.getElementById('disValId').value!="")
					{
	...
	submitForm('discountMaster','/hms/hms/billingMaster?method=saveLocalDiscount','chkDate');
}
</script>
