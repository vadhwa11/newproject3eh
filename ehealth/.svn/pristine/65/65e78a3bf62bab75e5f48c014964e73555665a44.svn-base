...
<select id="departmentId"	name="<%=DEPARTMENT_ID %>">
	...
	<input type="button" name="save" id="addbutton"
		onclick="submitForm('patientSearch','/hms/hms/lab?method=searchPatientForValidationForEmpanelled');"
...
<div id="searchresults" tabindex="2">
	...
	<form name="pendingSampleValidationForEmpanelled" method="post" action="">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
data_arr = new Array();
	...
	String hinNo = "";
	    int  counter=0;
	    //System.out.println("patientDeatilList=22="+patientDeatilList.size());
			...
			outerloop:	
				...
				boolean onePamentisDone=false;
				 for(DgOrderdt orderDt : set){
					...
					}else if(!"A".equalsIgnoreCase(orderDt.getOrderStatus()) 
							&& (!"IP".equalsIgnoreCase(orderDt.getOrderhd().getPatientType()))){
						onePamentisDone=true;
					} 
