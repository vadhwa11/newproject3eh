...
<input type="text" id="searchField" name="<%= SEARCH_FIELD%>" value=""
	validate="Bank Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'billingMaster?method=searchBank')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','billingMaster?method=searchBank','checkSearch')"
...
<input type="button" name="Report" value="Generate Report"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_bank">
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
<input type="hidden"
	name="<%= POJO_NAME %>" value="MasBankMaster"><input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="BankName"><input
	type="hidden" name="title" value="Bank"><input type="hidden"
	name="<%=JSP_NAME %>" value="bank"><input type="hidden"
	name="pojoPropertyCode" value="BankCode">
<div class="paddingTop5"></div>
<div class="clear"></div>
<div class="Block"><label><span>*</span> Bank Code </label> <input
	id="codeId" type="text" name="<%= CODE%>" value=""
	validate="Bank Code,string,yes" MAXLENGTH="8" / tabindex=1>
<label><span>*</span> Bank Name </label> 
...
<input type="button" name="add" id="addbutton" value="Add"
	class="button"
	onClick="submitForm('bank','billingMaster?method=addBank');"
	accesskey="a" tabindex=1 /> <input type="button" name="edit"
	id="editbutton" value="Update" style="display: none;" class="button"
	onClick="submitForm('bank','billingMaster?method=editBank')"
	accesskey="u" tabindex=1 /> <input type="button" name="Delete"
	id="deletebutton" value="Activate" style="display: none;"
	class="button"
	onClick="submitForm('bank','billingMaster?method=deleteBank&flag='+this.value)"
...
data_arr[<%= counter%>][2] = "<%= masBank.getBankName()%>"
	...
	data_arr[<%= counter%>][3] = "<%= masBank.getLastChgBy()!=null?(masBank.getLastChgBy().getId()!=null?masBank.getLastChgBy().getId():"0" ):"0"%>"
