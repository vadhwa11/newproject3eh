...
<form name="silDilStatusMessage" method="post" ><input
	type="hidden" name="parent" value="<%=andNo%>" /> <input type="hidden"
...
<div class="clear"></div>
<input type="button" name="yes" value="Yes" class="button"	onClick="submitForm('silDilStatusMessage','/hms/hms/discharge?method=showDischargeSummaryReport&<%=ADMISSION_NUMBER%>=<%=andNo%>&<%=HIN_NO%>=<%=hinNo%>');" />
