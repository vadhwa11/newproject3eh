...
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onclick="setdate('',document.paymentVoucher.<%=VOUCHER_DATE%>,event)" />
...
<label>Balance:</label>
 <input id="balanceId" type="text"   name="<%=BALANCE_AMOUNT%>" readonly="readonly" value=""   />


<label><span>*</span>Voucher No:</label>
 <input id="voucherNoId" type="text"   name="<%=VOUCHER_NO%>" value="<%= voucherNo %>" readonly="readonly"  validate="Voucher No,string,no"  tabindex=1 />
...
<input type="hidden" id="flagForSL" name="flagForSL" value=""/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	...
	<td id="slId<%=i %>"> <input  type="text"   name="<%=SUB_LEDGER_CODE%><%=i%>" value=""   tabindex=1 /></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	...
	<td> <input id="accountNameBankId<%=inc%>" type="text" size="20"  name="accountNameBank<%=inc%>" value=""   tabindex=1 onblur="validateAccountName(this.value,<%=inc %>,'bankSlId');" />
...
<!--  ====================================================End Bank voucher table==============================-->
	...
	<label>Total</label>
	<input id="totalAmountId" type="text"   name="<%=TOTAL_AMOUNT %>" value=""  validate="Total Amount ,string,yes" MAXLENGTH="100" tabindex=1 />
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Submit" class="button" onClick="submitForm('paymentVoucher','account?method=submitPaymentVoucher','validateNarration');" accesskey="a" tabindex=1 />
...
function validateAccountName( strValue,inc,tdid ) {
 	...
 	if(strValue != "")
	{
		    ...
		    if(id!="")
		    {
		    	...
		    	submitProtoAjaxWithDivName('paymentVoucher','account?method=getSubLedgerForAccount&accName='+encodeURIComponent(strValue)+'&rowVal='+inc+'&flagForSL+'&financialYearId'+=financialYearId);
		    }
	}
}
