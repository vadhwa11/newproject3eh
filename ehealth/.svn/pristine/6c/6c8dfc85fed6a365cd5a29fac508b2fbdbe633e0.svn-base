...
<%@page import="jkt.hms.masters.business.Inpatient"%><script type="text/javascript" language="javascript"
...
prop.load(in);
		...
		Inpatient visit=(Inpatient)patientDataList.get(0);
		String patientName="";
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"><input type="hidden"
	...
	<input type="hidden"
	name="inpatientId" value="<%=visit.getId() %>" />
...
<label class=""><%=prop.getProperty("com.jkt.hms.opd_no") %></label> 
...
<input type="button" name="priv" id="priv" value="Patient Previous Details"	class="buttonBig3" onclick="showPreviousVisit(<%=visit.getAdNo()%>)"/>
<div class="Block">
<div class="clear"></div>
<label>
<span>*</span> Technician  Name</label> <select name="physiotherapistNameId" id="physiotherapistNameId">
...
<input type="text" id="dobId" name="<%=DATE_OF_BIRTH %>" tabindex="13" value="<%=consultationDate %>" class="date"  readonly="readonly" class="date" MAXLENGTH="30" />
<img id="" src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" onclick="seztdate('<%=consultationDate %>',document.opdMain.<%=DATE_OF_BIRTH%>,event)" validate="Pick a date"  tabindex="14" />
	
	<label>Dialysis Time </label>
	<input type="text" name="dialysisTime" value="<%=consultationTime.substring(0,5) %>" onKeyUp="mask(this.value,this,'2',':');"  maxlength="5" class="date"/>
...
//      document.opdMain.action="/hms/hms/opd?method=showPatientPhysiotheraypPreviousVisit&hinId=<%=visit.getHin().getId()%>&deptId=<%=visit.getDepartment().getId()%>&adNo=<%=visit.getAdNo() %>";
