...
<div id="searchresults" tabindex="2">
	...
	<script type="text/javascript">
	  formFields = [
			[0, "orderId", "id"],[1,"orderNo"],[2,"admissionDate"],[3,"ipNo"],[4,"orderDate"],[5,"orderTime"], [6,"hin"],[7,"patName"], [8,"age"], [9,"sex"],[10,"wardNo"],[11,"bedNo"], [12,"pType"],[13,"doctorName"],[14,"department"],[15,"departmentWard"],[15,"specialty"],[16,"Priority"]];
	  statusTd = 15;
	</script>
</div>
</form>
...
data_arr = new Array();
	...
	<%
		...
		for(DgOrderhd dgOrderhd: patientDetailList){
			...
			String flag = "";
			if(deptType.equalsIgnoreCase("WARD")){
...
//			}
			...
			%> 
					...
					data_arr[<%= counter%>][0] = <%= dgOrderhd.getHin().getId()%><%--//added by govind 27-07-2017--%>
					...
					data_arr[<%= counter%>][3] = "<%= HMSUtil.convertDateToStringWithoutTime(dgOrderhd.getInpatient().getDateOfAddmission())%>"
					<%String ipdNo="";
					String ward="";
					String bedNo="";
...
<%session.setAttribute("patientDetailList",patientDetailList);%>
	...
	{
	 var url="/hms/hms/lab?method=showPopupTokenJsp&flag=OP&"+csrfTokenName+"="+csrfTokenValue;
