...
<%int count=1; 
	for(MortuaryRegisterDetails mortuaryList : mortuaryDetails){ %>
		...
		<tr onclick="submitForm('mortuaryDetails<%=count %>', 'mlc?method=showPostmortemDetailNotesJsp')">
		    <td><%=count %></td>
			...
			<td><%=(mortuaryList.getCrimeDate()!=null)?HMSUtil.convertDateToStringTypeDateOnly(mortuaryList.getCrimeDate()):""%></td>
