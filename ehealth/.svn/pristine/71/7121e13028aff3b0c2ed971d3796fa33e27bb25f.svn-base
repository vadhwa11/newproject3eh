...
//var storeDepartmentId= document.departmentIndent.<%= ITEM_TYPE_ID%>.value;
var url="/hms/hms/stores?method=showAddDepartmentIndentJsp&internalIndentId="+internalIndentId + "&<%=FROM_WARD%>="+deptId+ "&<%=TO_WARD%>="+toDept+ "&<%=DEMAND_NO%>="+demandNo+ "&<%=DEMAND_DATE%>="+demandDate+"&<%=APPROVED_BY_EMPLOYEE_ID_DEPENDENT_INDENT%>="+approvedBy+"&<%=REQUEST_BY_EMPLOYEE_ID_DEPENDENT_INDENT%>="+requestBy +"&numOfRows=15&pageNo="+1;
...
function goPage(pidx) {
	...
	submitForm('departmentIndent','stores?method=getDepartmentIndentData&numOfRows='+numOfRows+'&currPage='+pidx+'');
}
...
document.departmentIndent.method="post";
submitForm('departmentIndent','stores?method=updateGridItemsInDepartmentIndent&status1='+status1);
}
...
function del()
{
	...
	{
	document.departmentIndent.method="post";
	submitForm('departmentIndent','stores?method=deleteGridItemsForDepartmentIndent');
	}
	else
	{
...
function jsDisplay() {
	...
	submitForm('searchPanel','stores?method=getDepartmentIndentData&<%=DEMAND_NO%>='+demandNo+"&status1="+status1+'&numOfRows=5&currPage=1');
}
...
<select name="<%=DEMAND_NO%>" validate="DEMAND NO,number,no">
...
<input type="hidden" name="<%=FROM_WARD %>" value="<%=deptId%>" validate="From Ward,alphanumeric,no" />
...
<div class="Block"><label>Demand No.</label> <input type="text"
	...
	MAXLENGTH="8" class="readOnly" validate="Demand No,alphanumeric,no" /> 
	
	<label>Demand Date</label> <input
	type="text" name="<%=DEMAND_DATE %>" value="<%=demandDate%>" 
	validate="Demand Date,date,no"
...
<select name="<%=FROM_WARD%>"	>
 ...
 <%
			...
			%>		
			</select> <label>To </label> <select name="<%=TO_WARD%>"
	validate="To Ward,String,no">
	<option value="0">Select</option>
	<%
				for (MasDepartment masDepartment :departmentList ) {
					
			if(masDepartment.getDepartmentType().getId()==3){
...
<%searchStoreInternalIndentMList.clear();
storeInternalIndentMDemandNoList.clear();
storeInternalIndentMList.clear();
demandNoList.clear();
departmentList1.clear();
%>
