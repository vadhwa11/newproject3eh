...
<div class="titleBg">
<h2>Search Patient For Update OP MLC</h2>
</div>
...
<div class="Block"><!--  	<label class="bodytextB">Service No.:</label>
			<input type="text" id="serviceNo." name="<%=SERVICE_NO%>" value="" class="textbox_date" MAXLENGTH="20" onblur="getHinNo('search','registration?method=getHinNoForUpdateAdt&flag=visit')"/>		
		-->
<div id="hinDiv"><label> <%=prop.getProperty("com.jkt.hms.registration_no") %></label> <input type="text"
	name="<%=HIN_NO%>" value="" MAXLENGTH="50"
	onblur="submitProtoAjax('search','adt?method=getMlcNo')"
...
<input type="button" name="Search" value="Search" class="button"
	onClick="if(checkMLC()){submitForm('search','adt?method=showUpdateMedicoLegalCaseDetails&flag=opMlc');}" />
