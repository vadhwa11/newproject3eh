...
prop.load(in);
		...
		System.out.println("pharmacyLabQueueId=jsppppppppp=="+pharmacyLabQueueId);
...
%>
	...
	<input type="hidden" name="dgOrderHdId" id="dgOrderHdId" value="<%=orderId%>" />
...
%>
		<label>Order Number</label> <input id="orderId" type=hidden
			name="<%=ORDER_NO %>" value="<%=orderNo %>" title="Order Number" />

		<label class="value"> <%=orderNo %>
		</label> <input type="hidden" id="dgOrderhdId" name="dgOrderhdId"
			value="<%= orderId%>" /> <input type="hidden" id="hinId"
...
<input	type="radio" id="rareCommon2" name="<%=RARE_COMMON  %>"	class="radioCheck" value="r" /> -->
	...
	%>
				<tr>
					...
					<td>
					<input type="hidden" value="<%=dgOrderdt.getOrderhd().getId() %>" name="dgOrderhdId" id="dgOrderhdId" />
						<input type="hidden" value="<%=dgOrderdt.getId() %>" name="<%=DG_ORDER_DETAIL_ID %><%=inc %>" id="dgOrderDtId<%=inc %>" />
					<input type="checkbox" value="<%=inc%>" name="selectedChrage" class="radioCheck" disabled="disabled" /></td>
					<td><input type="text" readonly="readonly" name="chargeCode<%=inc%>" value="<%=dgOrderdt.getChargeCode().getChargeCodeName() %>" id="chargeCode<%=inc%>" 
						tabindex="1" />
			...
			// }
						return entry + "&mainChargeCodeId=" + document.getElementById('mainChargeCodeId').value+"&subChargeCodeId="+document.getElementById('subChargeCodeId').value+"&orderHdId="+document.getElementById('dgOrderhdId').value;
				}
			    new Ajax.Autocompleter('chargeCode<%=inc%>','ac2update','lab?method=getChargeCodeForAutoComplete',{parameters:'requiredField=chargeCode<%=inc%>', callback: eventCallback});
					...
					<input type="hidden" value="<%=dgOrderdt.getChargeCode().getId() %>" name="<%=CHARGE_CODE_ID%><%=inc %>" id="chargeCodeId<%=inc %>" />
					<input type="hidden" value="<%=dgOrderdt.getMainChargecode().getId() %>" name="<%=MAIN_CHARGECODE_ID%><%=inc %>" id="mainChargeId<%=inc %>" />
					<input type="hidden" value="<%=dgOrderdt.getSubChargeid().getId()%>" name="<%=SUB_CHARGECODE_ID%><%=inc %>" id="subCharegId<%=inc %>" />
					<%-- <td><input type="text" id="chargeName<%=inc%>"
						name="<%=CHARGE_NAME%>" value="" class="readonly" /></td> --%>

					<td><input type="text" id="qty<%=inc%>" name="<%=QUANTITY %>"
						value="<%=dgOrderdt.getOrderQty() %>" readonly="readonly" validate="Qty,int,yes" MAXLENGTH="3" tabindex="1"
						onblur="if(this.readOnly== false){validateQty(this.value,this.id);calculateAmt(<%=inc %>);totalCost(<%=inc%>);}" />
					</td>

					<td id="rateVal<%=inc%>"><input type="text" value="<%=dgOrderdt.getChargeCode().getCharge() %>"
						...
						<%
						total= total.add(new BigDecimal(dgOrderdt.getOrderQty().floatValue()*(dgOrderdt.getChargeCode().getCharge()))); 
						%>
					<td><input type="text" id="amount<%=inc%>" value="<%=dgOrderdt.getOrderQty().floatValue()*(dgOrderdt.getChargeCode().getCharge()) %>"
...
<%}} %>
		...
		<div class="paddFltRight110">
			<label class="medium"><a>Total Cost</a></label> <input type="text" value="<%=total %>"
...
%>
			<tr>
				...
				<input type="hidden" value="<%=dgOrderdt.getOrderhd().getId() %>" name="dgOrderhdId" id="dgOrderhdId" />
								<input type="hidden" value="<%=dgOrderdt.getId() %>" name="<%=DG_ORDER_DETAIL_ID %><%=inc %>" id="dgOrderDtId<%=inc %>" />
				<input type="checkbox" value="<%=inc%>" name="selectedChrage" class="radioCheck" /></td>
				<td><input type="text" name="chargeCode<%=inc%>" value="<%=dgOrderdt.getChargeCode().getChargeCodeName() %>" id="chargeCode<%=inc%>"
					onblur="checkInvestigationItem(<%=inc %>);checkDuplicateTest(<%=inc %>);if(validateChargeCodeForAutoComplete(this.value, '<%=inc %>')){dsubmitProtoAjaxWithDivName('investigationOrderBooking','/hms/hms/lab?method=fillChargeCode&rowVal=<%=inc %>&hin=<%=hin%>','rateVal<%=inc %>');test(<%=inc %>)}"
					tabindex="1" />
					<div id="ac2update" style="display: none;" class="autocomplete"></div>
					<script type="text/javascript" language="javascript"
						charset="utf-8">
		...
		// }
		    ...
		    new Ajax.Autocompleter('chargeCode<%=inc%>','ac2update','lab?method=getChargeCodeForAutoComplete',{parameters:'requiredField=chargeCode<%=inc%>', callback: eventCallback});
				...
				<input type="hidden" value="<%=dgOrderdt.getChargeCode().getId() %>" name="<%=CHARGE_CODE_ID%>" id="chargeCodeId<%=inc %>" />
				<input type="hidden" value="<%=dgOrderdt.getMainChargecode().getId() %>" name="<%=MAIN_CHARGECODE_ID%>" id="mainChargeId<%=inc %>" />
				<input type="hidden" value="<%=dgOrderdt.getSubChargeid().getId()%>" name="<%=SUB_CHARGECODE_ID%>" id="subCharegId<%=inc %>" />
				<%-- <td><input type="text" id="chargeName<%=inc%>"
					name="<%=CHARGE_NAME%>" value="" class="readonly" /></td> --%>
		...
		<input type="hidden" value="<%=inc %>" name="hiddenValueCharge" id="hiddenValueCharge" />
			<input type="hidden" value="" name="selectPrevInvestigation" id="selectPrevInvestigation" />
	</div>
</div>
	<div class="clear"></div>
	<div class="paddFltRight110">
...
<%--Added by govind 27-07-2017 for IP Lab OrderBooking end--%>
	...
	<input type="button" class="button" value="Submit"
		id="submitForDisable"
		onclick="if(checkTestRow()){submitFormToDisableSubmit1('investigationOrderBooking','lab?method=submitOrderBookingForExistingInvestigation&patientType=<%=patientType %>');}" />
...
function addRow(){
	...
	e10.type = 'hidden';
	e10.name='<%=DG_ORDER_DETAIL_ID %>'+iteration;
	cell0.appendChild(e10);
	...
	e1.type = 'text';
	e1.onblur=function(){
		checkDuplicateTest(iteration);checkInvestigationItem(iteration);
	...
	e12.type = 'hidden';
	e12.name='<%=MAIN_CHARGECODE_ID%>'+ (iteration);
	e12.id = 'mainChargeId'+(iteration)
	e12.tabIndex="1";
	e12.readOnly = true;
	e12.size='20'; 
	...
	e13.type = 'hidden';
	e13.name='<%=SUB_CHARGECODE_ID%>'+ (iteration);
	e13.id = 'subCharegId'+(iteration)
	e13.tabIndex="1";
	e13.readOnly = true;
	e13.size='20'; 
	...
	e14.type = 'hidden';
	e14.name='<%=CHARGE_CODE_ID%>'+ (iteration);
	e14.id = 'chargeCodeId'+(iteration)
	e14.tabIndex="1";
	e14.readOnly = true;
	e14.size='20'; 
	cell1.appendChild(e14); --%>
...
function checkDuplicateTest(cnt){
var orderId =document.getElementById('dgOrderhdId').value;
var tbl = document.getElementById('chargeDetails');
var lastRow = tbl.rows.length;
var iteration = lastRow-1;
var chargeCodeName = document.getElementById("chargeCode"+cnt).value;
...
var chargeCodeId = chargeCodeName.substring(index1,index2);
	if(chargeCodeId !=0){
	var xmlHttp;
	  try {
	    // Firefox, Opera 8.0+, Safari
	    ...
	    xmlHttp.onreadystatechange=function()
	    {
	    	...
	    	for (loop = 0; loop < items.childNodes.length; loop++) {
	    	    ...
	    	    /*   var componentName  = item.getElementsByTagName("componentName")[0];
	    	       ...
	    	       document.getElementById("compI").value=msg.childNodes[0].nodeValue;
	    	       ...
	    	       if(msg !=undefined && msg.childNodes[0] !=undefined){
					   ...
					   document.getElementById("chargeCode"+cnt).value = "";
					   document.getElementById("qty"+cnt).value = "";
					   document.getElementById("resrate"+cnt).value = "";
					   document.getElementById("amount"+cnt).value = "";
					   
				}
	    	            
	    	}
	    }
	    var url="/hms/hms/lab?method=checkDuplicateInvestigation&chargeCodeId="+chargeCodeId+"&orderId="+orderId;
	 	url = url+'&'+csrfTokenName+'='+csrfTokenValue; // added by amit das on 07-07-2016
	    xmlHttp.open("GET",url,true);
	    xmlHttp.setRequestHeader("Content-Type", "text/xml");
	    xmlHttp.send(null);
