...
<%@page import="jkt.hms.masters.business.HesRefilledCylinderDeliveryHeader"%>
...
<%
	...
	List<HesRefilledCylinderDeliveryHeader> workParticularList = new ArrayList<HesRefilledCylinderDeliveryHeader>();
	List<HesCylinderTypeMaster> entryNoList = new ArrayList<HesCylinderTypeMaster>();
	String Entry_data = "";
	
	if (map.get("hesWorkList")!= null)
		workParticularList = (List<HesRefilledCylinderDeliveryHeader>)map.get("hesWorkList");
...
<form name="refilledCylinderDeliveryForm" method="post"">
<h2>Refilled Cylinder Delivery Entry</h2>
<div class="clear"></div>
...
<label>Date </label> <input type="text" name="<%=FROM_DATE%>"
	...
	onclick="setdate('<%=currentDate%>',document.refilledCylinderDeliveryForm.<%= FROM_DATE%>,event);" />
	
<label>Vehicle No :</label> <select name="searchVehicleNo">
	<option value="0" >--Select Vehicle No--</option>
<%
		for (HesRefilledCylinderDeliveryHeader hesRefilledCylinderDeliveryHeader : workParticularList )
		{
%>
			<option value=<%=hesRefilledCylinderDeliveryHeader.getVehicleNo()%>><%=hesRefilledCylinderDeliveryHeader.getVehicleNo()%></option>
<%
		}
%>
</select>
...
<%
		...
		{
%>
			<option value=<%=hesRefilledCylinderDeliveryHeader.getId()%>><%=hesRefilledCylinderDeliveryHeader.getEntryNo()%></option>
<%
		}
%>
</select> 
...
<%
		...
		{
%>
			<option value=<%=hesRefilledCylinderDeliveryHeader.getChallanNo()%>><%=hesRefilledCylinderDeliveryHeader.getChallanNo()%></option>
<%
		}
%>
</select> 
...
<input type="image" src="/hms/jsp/images/go.gif" class="button"
	onClick="submitForm('refilledCylinderDeliveryForm','hes?method=searchRefilledCylinderDeliveryForm');" />
...
<input type="text" id="entryNo" name="entryNo" value="CMGS/RCD-<%=Entry_data%>" tabindex=1 readonly="readonly" />
...
<input type="text" id="ChallanNo" name="ChallanNo" value=""  validate="Challan No,String,yes" tabindex=1 />

<div class="clear"></div>

<label><span>*</span>Challan Date</label>
<input type="text"
	name="<%=CHALLAN_DATE%>" value="<%=currentDate %>" class="date"
...
<input type="text" id="VehicleNo" name="VehicleNo" value=""  validate="Vehicle No,String,yes" tabindex=1 />
...
<input type="button" name="sss" align="right" class="button" value="Submit"
 onclick="if(checkFilledRow()){submitForm('purchaseGrid','hes?method=submitRefilledCylinderDeliveryEntry');}" />
