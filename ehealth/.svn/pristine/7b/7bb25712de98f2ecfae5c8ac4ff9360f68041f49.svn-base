...
public class PatientResultValidationDataToCentralServerSchedular extends
		QuartzJobBean {

	private static final Logger logger = org.apache.log4j.Logger
			.getLogger(PatientResultValidationDataToCentralServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		Map<String, Object> dataMap = investigationDataService
				.getPatientResultValidationDataForCentralServer();
	...
	private String hl7MessageSender(String msg, MasHospital hospital) {
		...
		try {
			...
			logger.debug("MalformedURLException in Patient Result Validation Data To Central Server Schedular");
		} catch (ConnectException e) {
			logger.debug("Connection refused in Patient Result Validation Data To Central Server Schedular");
		} catch (SocketTimeoutException e) {
			logger.debug("Connection timeout in Patient Result Validation Data To Central Server Schedular");
		} catch (IOException e) {
			logger.debug("IOException in Patient Result Validation Data To Central Server Schedular");
		}
