...
<input name="chargeCodeCodeForPerticularTestForMultiple"
	...
	<%
		if (dgCollection.getResultType().equalsIgnoreCase("r")
 ...
 %>
		...
		<input name="collectionId" value="<%=dgCollection.getId() %>" type="hidden">
		<input type="hidden" name="<%=RESULT_TYPE%>"
			id="<%=RESULT_TYPE%><%=inc%>" value="m"> <input type="text"
				...
				tabindex="1" maxlength="50" value="<%=(multResultVal!=null)?multResultVal:""%>" 
 ...
 %>
		...
		<td>
			...
			id="<%=ADDITIONAL_REMARKS%><%=inc%>" value="<%=dgDet.getRemarks()%>" tabindex="2"
			maxlength="200"> <%
 	} else {
 ...
 %> <input name="normalId" type="hidden" size="8"
			value="<%=nv.getId()%>" readonly /> <input name="normalValue"
			type="text" size="8" class="readOnly"
			value="<%=nv.getMinNormalValue()
													+ " - "
													+ nv.getMaxNormalValue()%>"
			readonly /> <%
