...
<div class="titleBg">
<h2>Department wise monthly summary </h2>
</div>
<form name="departmentWiseMonthly" target="_blank" action="" method="post">
<div class="Block"><label><span>*</span>From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.departmentWiseMonthly.<%=FROM_DATE%>,event)" />
...
<label><span>*</span>To Date</label> <input type="text" class="date"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.departmentWiseMonthly.<%=TO_DATE%>,event)" />
<div class="clear"></div>
<label>Sub Department:</label> <select name="<%= SUB_CHARGECODE_ID %>" tabindex=1>
...
<input type="button" name="OK" value="OK" class="button"
	onClick="submitForm('departmentWiseMonthly','lab?method=printDepartmentWiseMonthlySummaryReport','validateFromToDate');" />
...
<script type="text/javascript" language="javascript">
	...
	function validateFromToDate(){
		...
		obj1 = eval(document.departmentWiseMonthly.fromDate)
		obj2 = eval(document.departmentWiseMonthly.toDate)
