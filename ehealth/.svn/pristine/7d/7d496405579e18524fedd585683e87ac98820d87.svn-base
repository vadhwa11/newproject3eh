...
<h2 align="left" class="style1">Issues To Other Units on Surplus
(CIV)</h2>
<div id="contentspace">
		...
		<table width="20%" align="right" border="0" cellpadding="0"
			...
			<tr>
				...
				<td width="9%" background="/hms/jsp/images/toolbuttBack.gif"><input
					...
					class="toolbutton" onclick="openDeletePopupForIssueToOtherUnits();" />
				</td>
				<td width="4%" background="/hms/jsp/images/toolbuttBack.gif"><input
					type="button" name="print" type="submit" class="toolbutton"
					value="Print"
					onClick="submitForm('issueToOtherUnitsForm','stores?method=printIssueToOtherUnits&issueId='+<%=issueId%>);"></td>
...
<table width="330" border="0" cellpadding="2" cellspacing="1"
	...
	<tr>
		<td class="vbmenu_option" title="nohilite"><label
			...
			onClick="submitForm('issueToOtherUnitsForm', 'stores?method=searchIssueToOtherUnits');" />
		</td>
	</tr>
...
<input type="hidden" name="pageNo" value="<%=pageNo%>" id="pageNo" /> <label
	...
	value="<%=date %>" class="readOnly" id="isssueDate" /> <label
...
<label class="bodytextB"><font id="error"></font>Ref No:</label> <input
	...
	value="<%=box.get("docNo")%>" class="textbox_size20" id="docNo" /> <label
	class="bodytextB"><font id="error"></font>Demand Date:</label> <input
	type="text" name="<%=RequestConstants.ISSUE_DATE%>" readonly="readonly"
	value="<%=date %>" class="readOnly" id="isssueDate" /> <br />
...
<div style="float: left; padding-left: 15px;"><input type="button"
	...
	onclick="if(test()){{submitForm('issueToOtherUnitsForm','stores?method=addNextOrSubmitIssueToOtherUnits&buttonName=submit');}}" />
</div>
...
<input type="hidden" size="2" value="10" name="noOfRows" id="noOfRows" />
	...
	<tbody>
		...
		<tr>
			...
			<td width="10%"><input type="text" value="" tabindex="1"
				...
				onblur="if(fillSrNo(<%=inc %>)){checkForIssueToOTAFU(this.value, '<%=nameItem%>','<%=inc %>');}"
				name="<%=nameItem%>" />
			<div id="ac2update"
		  ...
		  new Ajax.Autocompleter('<%=nameItem%>','ac2update','stores?method=getItemListForIssueToDispensary',{parameters:'requiredField=<%=nameItem%>&issueId='+document.getElementById('issueId').value});
			...
			<input type="hidden" value="" id="<%=issuedItemId%>"
				class="bigcaption" />

			<td width="10%"><input type="text" value="" id="<%=lotNo%>"
				class="medcaption"
				onblur="fillGridIssueToOTAFU('issueToOtherUnitsForm','stores?method=fillIssueToOTAFUBasedOnLotNo&lotNo='+this.value , <%=inc %>);" /></td>
			...
			<td width="10%"><input type="text" value="" class="medcaption"
				...
				readonly="readonly" id="<%=qtyIssued%>" /></td>
			<td width="3%"><input type="button" class="morebutton"
				tabindex="1" onclick="openPopupForBrandForOtherUnits(<%=inc%>);"
				name="Submit2" /></td>
			<div id="ewe"></div>
		</tr>
		<% }   %>
