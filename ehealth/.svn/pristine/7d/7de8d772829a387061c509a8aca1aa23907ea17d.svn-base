...
<%
	...
	utilMap = (Map<String, Object>)HMSUtil.getCurrentDateAndTime();
		...
		if(map.get("hesEquipmentMasterList")!=null){
			hesEquipmentMasterList=(List)map.get("hesEquipmentMasterList");
		}	
...
<div class="clear"></div>	
     ...
     for (Iterator iterator = assessoryList.iterator(); iterator.hasNext();) {%>
			...
			<input type="hidden"  name="parent" value="<%= hesEquipmentMaster.getId()%>" id="radio" />
	...
	name="<%=serialNo %>"
	value="<%=hesEquipmentMaster.getSerialNo()%>" id="SerialNo"  class="textbox_size20"
	MAXLENGTH="10"/  >
	
	<label> Equipment Name: </label> 
	<input type="text"
	name="<%=euipmentName %>"
	value="<%=hesEquipmentMaster.getEquipmentName() %>" id=EquipmentName  class="textbox_size20"
	MAXLENGTH="10"/  >
	...
	name="<%=modelName %>"
	value="<%=hesEquipmentMaster.getModelName()%>" id="ModelName"  class="textbox_size20"
	MAXLENGTH="10"/ >
	
	<label> Make: </label> <input type="text"
	name="<%=make %>"
	value="<%=hesEquipmentMaster.getMake()%>" id="Make"  class="textbox_size20"
	MAXLENGTH="10"/  >
	
	<label> Contact Number Of Sales/Service Persons: </label> <input type="text"
	name="<%=contactNo %>"
	value="<%=hesEquipmentMaster.getContactNoSalesService()%>" id="ContactNoSalesService" class="textbox_size20"
	MAXLENGTH="10"/  >
	<div class ="clear"></div>
	
	<label>Warranty Details: </label> <textarea 
	name="<%=warrantyDetail %>"
	id="WarrantyDetails"  class="textbox_size20"
...
<table border="0" cellspacing="0" cellpadding="0" id="chargeDetails">
   ...
   <input type="text"
	name="assryName"
	value="<%=hesEquipmentAssr.getAssessoryName()%>" id="assryName"  class="textbox_size20"
	MAXLENGTH="10"/  ></td> 
	
	<td><input type="text"
	name="modNo"
	value="<%=hesEquipmentAssr.getModelNo()%>" id="modNo"  class="textbox_size20"
	MAXLENGTH="10"/  ></td> 
	
	<td> <input type="text"
	name="serialNo"
	value="<%=hesEquipmentAssr.getSerialNo()%>" id="serialNo"  class="textbox_size20"
	MAXLENGTH="10"/  ></td> 
	
	 <td> <input type="text"
	name="qunatity"
	value="<%=hesEquipmentAssr.getQuantity()%>" id="qunatity"  class="textbox_size20"
	MAXLENGTH="10"/  > </td>
...
<input type="button" name="sss" align="right" class="button" value="Update"
 onclick="submitForm('modifyMasterForm','/hms/hms/hes?method=updateEquipmentAssessories');" />
<div class="clear"></div>
<div class="paddingTop15"></div>
<div class="clear"></div>
<div class="division"></div>
