...
Ajax.Request = Class.create(Ajax.Base, {
  ...
  setRequestHeaders: function() {
    var headers = {
      ...
      '_csrf_header' : csrfTokenValue,
      /*'OWASP_CSRFTOKEN' : csrfTokenValue,*/
    ...
    xmlHttp.onreadystatechange=function()
    {
      if(xmlHttp.readyState==4){
		...
		obj1.length = 1;
      	...
      	{
	   	    var item = items.childNodes[loop];
	   	     var dipenseType = item.getElementsByTagName("dipenseType")[0];
	     
        	
            
            obj2 = document.getElementById("dispenseType"+rowVal);
            
           	obj2.length=1;
           	if(obj2.options[obj2.length-1]!=null){
			obj2.options[obj2.length-1].value = dipenseType.childNodes[0].nodeValue;
			obj2.options[obj2.length-1].text  = dipenseType.childNodes[0].nodeValue;
        	}
	        var id  = item.getElementsByTagName("id")[0];
	        var pvms  = item.getElementsByTagName("pvms")[0];
	        var au  = item.getElementsByTagName("au")[0];
	        ...
	        var manufacturerId = item.getElementsByTagName("manufacturerId")[0];
	        var manufacturerName = item.getElementsByTagName("manufacturerName")[0];
	        var quantityInHand = item.getElementsByTagName("quantityInHand")[0];
			var mrp=item.getElementsByTagName("mrp")[0];
        	document.getElementById('codeItem'+rowVal).value = pvms.childNodes[0].nodeValue
        	document.getElementById('idItem'+rowVal).value = id.childNodes[0].nodeValue
        	document.getElementById('idAu'+rowVal).value = au.childNodes[0].nodeValue
        	document.getElementById('quantityInHand'+rowVal).value = quantityInHand.childNodes[0].nodeValue
        	
        	document.getElementById("ratePerMdq"+rowVal).value=costPrice.childNodes[0].nodeValue
       		document.getElementById("unitRateVarTemp"+rowVal).value=costPrice.childNodes[0].nodeValue
       		document.getElementById("mrp"+rowVal).value=mrp.childNodes[0].nodeValue
      ...
      if(xmlHttp.readyState==4){
      	...
      	for (loop = 0; loop < items.childNodes.length; loop++) {
	       ...
	       return true
       		...
       		document.getElementById("bedNoTemp").value=0
