...
<%@page import="jkt.hms.masters.business.HesMinorRoutineWorkDetail"%>
...
<%
	...
	List<HesMinorRoutineWork> hesRoutineWorkList = new ArrayList<HesMinorRoutineWork>();
	...
	if (map.get("hesRoutineWorkList")!= null)
		hesRoutineWorkList = (List<HesMinorRoutineWork>)map.get("hesRoutineWorkList");
	
	if (map.get("orderminorList")!= null)
		orderminorList = (List<HesMinorRoutineWorkDetail>)map.get("orderminorList");
...
<h2>Update Minor Routine Works Entry</h2>
<div class="clear"></div>
...
<input type="image" src="/hms/jsp/images/go.gif" class="button"
	onClick="submitForm('minorRoutineForm','hes?method=searchMinorRoutine');" /><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
</form>
</div>
<div class="clear"></div>
...
<%
		String dt = "";
		String deliveryDate = "";
		for (HesMinorRoutineWork routineWork : hesRoutineWorkList )
		{
			dt = HMSUtil.convertDateToStringWithoutTime(routineWork.getEntryDate());
			deliveryDate = dt;
%>
...
<input type="text" id="entryNo" name="entryNo" value="<%=routineWork.getEntryNo()%>" tabindex=1 readonly="readonly" />
...
<input type="text" id="workParticularName" name="workParticularName" value="<%=routineWork.getWorkParticularName().getWorkParticularName()%>" validate="Work Particulars Name,String,yes" tabindex=1 readonly="readonly" />
<input type="hidden" id="workParticular" name="workParticular" value="<%=routineWork.getWorkParticularName().getId()%>" validate="Work Particulars Name,String,yes" tabindex=1 readonly="readonly" />

<div class="clear"></div>
<label><span>*</span>Department/Area</label>
<input type="text" id="departmentName" name="departmentName" value="<%=routineWork.getDepartment().getDepartmentName()%>" validate="Department Name,String,yes" tabindex=1 readonly="readonly" />
<input type="hidden"" id="deptName" name="deptName" value="<%=routineWork.getDepartment().getId()%>" validate="Department Name,String,yes" tabindex=1 readonly="readonly" />

<label>Remarks</label>
<textarea id="remarks" name="remarks" rows="5" validate="Remarks,String,no"><%=routineWork.getRemarks()%></textarea>
...
<%
		...
		{
			if(routineWork.getEmpWorkCompleted().getId().equals(masEmployee.getId()))
			{	
%>
...
<table border="0" cellspacing="0" cellpadding="0" id="chargeDetails">
	...
	<%
		int inc = 1;
		if(orderminorList != null && hesClosingList != null)
		{
			for(int a = 0; a < orderminorList.size(); a++)
        	{    
        		Object[] object = (Object[])orderminorList.get(a);
				Integer ITEMID = (Integer)object[0];
        		String instrumentName = (String)object[1];
		    	String instrumentCode  = (String)object[2];
		    	Long qtyData = (Long)object[3];
	...
	<tr>
		...
		<td><input id="materialCode<%=inc%>" name="materialCode" type="text" value="<%=instrumentCode%>"
			size="3" MAXLENGTH="3" readonly="readonly" /></td>
			
		<td width="20%">
			<input type="text" id="materialName<%=inc%>" name="materialName" size="25" value="<%=instrumentName%>[<%=instrumentCode%>]" MAXLENGTH="25" onblur="onBlurMaterialName(this.value,'<%=inc %>')" />
		...
		<td width="10%"><input id="qty<%=inc%>" name="qty" type="text" size="5" MAXLENGTH="5" value="<%=qtyData%>"
...
%>
			<td>
				<input id="closingStock<%=inc%>" name="closingStock" type="text" value="<%=big.intValue()%>"  size="5" readonly="readonly" />
				<input id="ItemID<%=inc%>" name="ItemID" type="text" value="<%=ITEMID%>" size="5" readonly="readonly" />
			</td>
		</tr>
...
<input type="button" name="sss" align="right" class="button" value="Submit"
 onclick="if(checkFilledRow()){submitForm('purchaseGrid','hes?method=updateMinorRoutineWorksEntry');}" />
