...
%>
<div class="titleBg">
<h2>Items Freezed Report</h2>
</div>
<form name="freezed" method="post" action="">
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.freezed.<%=FROM_DATE%>,event);" />
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('freezed','stores?method=generateItemsFreezed');"
