...
<select id="departmentId"	name="<%=DEPARTMENT_ID %>">
	...
	<% }%>
		...
		<div class="clear"></div>
		<label>BarCode Scan</label> <input type="text" name="barcodetext" value="" MAXLENGTH="14" />
	</div>
	<div class="clear"></div>
	...
	String hinNo = "";
		...
		%>
			...
			data_arr[<%= counter%>][10] = "Male Medical Ward" 
				...
				%>
				data_arr[<%= counter%>][11] = "<%=dgSampleCollectionHeader.getPharmacyLabQueueId().getTokenNo()%>"
		...
		%>
			...
			data_arr[<%= counter%>][1] = "<%= HMSUtil.convertDateToStringWithoutTime(radioQueue.getDiagnosisDate())%>"
			...
			data_arr[<%= counter%>][10] = "Male Medical Ward" 
				...
				%>
				data_arr[<%= counter%>][11] = "<%= radioQueue.getPharmacyLabQueueId().getTokenNo()%>" 
					data_arr[<%= counter%>][12] = "" 
