...
<h4>Labor Details</h4>
			<div style="float: right;margin-right: 50px;">
				<input type="button" class="button" value="Add" align="right"
					onClick="javascript:addRowLabRoom4Table();" /> <input type="button"
					name="Reset" value="Delete" class="button" align="right"
					onClick="javascript:removeRow('lbRoom4Table');" />
			</div>
			<div class="clear"></div>
			<div class="clear"></div>
			<div class="">
				<div class="tableForTab"
					...
					<div id="divTemplet1">
						...
						<tr>
							...
							<td scope="col" style="width: 100px"><input type="text"
								id="lb4date<%=incr%>" value="<%=dateC%>" name="lb4date<%=incr%>"
								size="8" tabindex="<%=inxRow%><%=inxCol + 2%>"
								class="dateTextSmall" readonly="readonly"> <img
								src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
								validate="Pick a date"
								onclick="setdate('<%=dateC%>',document.laborRoom4.lb4date<%=incr%>,event);" /></td>
							<td scope="col"><input tabindex="<%=inxRow%><%=inxCol + 3%>" 
								type="text" value="<%=time%>" name="lb4Time<%=incr%>" 
								id="lb4Time<%=incr%>" size="6" /></td>
							<td scope="col"><input name="lb4MaterPulse<%=incr%>" onkeypress="return isNumberKey(event)"
								id="lb4MaterPulse<%=incr%>" size="6"
								tabindex="<%=inxRow%><%=inxCol + 4%>" type="text" 
								class="allownumericwithoutdecimal" /></td>
							<td scope="col"><input tabindex="<%=inxRow%><%=inxCol + 5%>" onkeypress="return isNumberKey(event)"
								type="text" size="6" name="lb4RespRate<%=incr%>"
								id="lb4RespRate<%=incr%>" size="10" /></td>
							<td scope="col"><input tabindex="<%=inxRow%><%=inxCol + 6%>"
								type="text" size="6" name="lb4BP<%=incr%>" id="lb4BP<%=incr%>" onkeypress="return isNumberKey(event)"
								size="8" /></td>
							<td scope="col"><input tabindex="<%=inxRow%><%=inxCol + 7%>"
								type="text" name="lb4LungBase<%=incr%>"
								id="lb4LungBase<%=incr%>" size="20" /></td>

							<td scope="col"><select
								tabindex="<%=inxRow%><%=inxCol + 8%>"
								name="lb4KneeJerk<%=incr%>" id="lb4KneeJerk<%=incr%>">
									<option value="0">select</option>
							...
							<td scope="col"><input tabindex="<%=inxRow%><%=inxCol + 9%>"
								type="text" name="lb4FatalHeart<%=incr%>" onkeypress="return isNumberKey(event)"
								id="lb4FatalHeart<%=incr%>" size="6" /></td>

							<td scope="col"><select name="lb4NST<%=incr%>"
								id="lb4NST<%=incr%>" tabindex="<%=inxRow%><%=inxCol + 10%>">
									<option value="0">select</option>
							...
							<td scope="col"><input
								...
								name="lb4OxyInfRate<%=incr%>" id="lb4OxyInfRate<%=incr%>"
								size="6" /></td>
								
							<td scope="col"><input
								tabindex="<%=inxRow%><%=inxCol + 12%>" type="text"
								name="lb4Contraction<%=incr%>" id="lb4Contraction<%=incr%>"
								size="6" /></td>

							<td scope="col"><input
								tabindex="<%=inxRow%><%=inxCol + 13%>" type="text"
								name="lb4IO<%=incr%>" id="lb4IO<%=incr%>" size="6" /></td>
						</tr>
						<%
							}
						%>
	...
	<input type="button" class="button" value="Submit" align="left"
		onClick="if(labRoom4Valid()){submitFormForButton('laborRoom4','ipd?method=addLaborRoom4')}" />
