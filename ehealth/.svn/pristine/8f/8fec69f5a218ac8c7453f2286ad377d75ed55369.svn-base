...
<label>CHC</label>
<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('rchActivity','/hms/hms/pubHealth?method=getBasicCenterListchc&chcphc='+this.value,'testDiv');" >
<%	if(chcList.size() >0){%>
...
<label>PHC</label>
<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('rchActivity','/hms/hms/pubHealth?method=getBasicCenterListphc&chcphc='+this.value,'testDiv');" >
<%	if(phcList.size() >0){%>
				<option value="0">Select</option>
				 <%
