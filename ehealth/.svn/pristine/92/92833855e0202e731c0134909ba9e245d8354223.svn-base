...
<form name="pendingBloodSamlpeCollection" action="" method="post">
<div class="titleBg">
<h2>Pending For Sample Testing</h2>
</div>
<div class="Block">
...
<input type="text" class="date"
	...
	onClick="setdate('<%=currentDate %>',document.pendingBloodSamlpeCollection.fromDate,event)" />
...
<label> <span>*</span> To Date</label> <input type="text" class="date"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',documentpendingBloodSamlpeCollection.toDate,event)" />
<div class="clear"></div>
<div class="clear"></div>
<label> Tube Number</label> <input type="text" name="tubeNumber"
	value="" MAXLENGTH="15" /> 

<div class="clear"></div>
<input type="button" name="search" 
value="Search" class="button" onclick="submitForm('pendingBloodSamlpeCollection','/hms/hms/bloodBank?method=showPendingListBloodSampleCollection')"  />
...
<table id=" " width="100%" border="0" cellspacing="0" cellpadding="0">
 <thead>
		...
		<tr onclick="submitForm('pendingBloodSamlpeCollection','/hms/hms/bloodBank?method=showBloodSampleCollectionJsp&samplId=${samplescreening.id}')" style="cursor: pointer;">
...
<c:if test="${requestScope.map.currentPage >= 2}">
        <td><a href="/hms/hms/bloodBank?method=showPendingListBloodSampleCollection&page=${requestScope.map.currentPage - 1}">Previous</a></td>
...
<table border="0" cellpadding="2" cellspacing="1">
        <tr>
            <c:forEach begin="1" end="${requestScope.map.noOfPages}" var="i">
                <c:choose>
                    ...
                    <c:otherwise>
                      <td><a href="/hms/hms/bloodBank?method=showPendingListBloodSampleCollection&page=${i}">${i}</a></td> 
    ...
    <c:if test="${requestScope.map.currentPage lt requestScope.map.noOfPages}">
     <td><a href="/hms/hms/bloodBank?method=showPendingListBloodSampleCollection&page=${requestScope.map.currentPage + 1}">Next</a></td>
        <%--  <td onclick="submitFormForButton('donorSearch',
