...
<label class="auto">From Date </label> <input
	name="<%=FROM_DATE_FOR_VALIDATION%>" type="text" class="date" /> <img
...
<label class="auto">To Date </label> <input
	name="<%=TO_DATE_FOR_VALIDATION %>" type="text" class="date" /> <img
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
...
<input name="save" type="button" class="button" value="Search"
	onClick="checkAndSubmit()" /> <input name="save" type="button"
...
<table id="searchresulttable" width="100%" cellspacing="0"
	...
	<tbody id="tableData">
		...
		<%
	  		...
	  		if(resourceRequisition.getExperience()!= null)
	  		{
	  			expYrs = resourceRequisition.getExperience().getExperienceYears();
	  		}
		...
		<label>Name of Initiator</label> <label class="value"><%=resourceRequisition.getInitiatorName()%></label>
		...
		<label>Desirable Experience</label> <label class="value"><%=expYrs %></label>
		<label>Position Purpose </label> <label class="value"><%=positionPurpose%></label>
		<label>Required Skills</label> <label class="value"><%=resourceRequisition.getRequiredSkill()%></label>
		<label>Total No. Of Positions</label> <label class="value"><%=resourceRequisition.getTotalNoPosition()%></label>
		<label>Salary Range </label> <label class="value"><%=resourceRequisition.getSalaryLowerRange() +" to "+ resourceRequisition.getSalaryUpperRange() %></label>
		</div>
		...
		<tr class=<%= klass%> id="<%=counter%>">
			<td><input type="checkbox" name=<%=id %>
				...
				class="radioCheck" /></td>
			<td><a
				href="javascript:Request(document.getElementById('<%=counter %>'),'requestResource')"
				rel="balloon<%=counter%>"><%=resourceRequisition.getInitiatorName()%></a></td>
			<td><a
				href="javascript:Request(document.getElementById('<%=counter %>'),'requestResource')"
			...
			<td><a
				href="javascript:Request(document.getElementById('<%=counter %>'),'requestResource')"
				rel="balloon<%=counter%>"><%=resourceRequisition.getRequestStatusMaster().getStatusDesc()%></a></td>
		</tr>
...
<script>
	...
	<input type="button"  class=button value="Go" onClick ="submitForm('requestResource','recruitment?method=updateRequisitionStatus')"/>
	<div class="clear"></div>
