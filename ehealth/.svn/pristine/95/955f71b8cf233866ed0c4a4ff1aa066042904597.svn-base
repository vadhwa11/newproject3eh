...
<% if(masStoreBrandList.size()!=0){
	 StoreItemBatchStock storeItem1 = null;
	 for (Iterator iterator = masStoreBrandList.iterator(); iterator.hasNext();) {
		 storeItem1 = (StoreItemBatchStock) iterator.next();
		 itemName=storeItem1.getItem().getNomenclature();
	 }
%>
<table align="left" class="small">
	<tr valign="top">
		<td width="15%" align="left"><font face="arial" size="4">Item
				Name :</font></td>
		<td width="15%" align="left"><font face="arial" size="4"
			color="blue"><%=itemName %></font></td>
		<td width="15%" align="left"><font face="arial" size="4">Qty.
				Requested:</font></td>
				
		<td width="15%" align="left"><font face="arial" size="4"
			color="blue"><%=qtyRequested %></font></td>
		
		<td width="15%" align="left"><font face="arial" size="4">Qty.
				Pending:</font></td>
				
		<td width="15%" align="left"><font face="arial" size="4"
			color="blue"><%=qtyPending  %></font></td>
...
<form name="itemIssuePopUp" method="post">
  ...
  %>
			<tr>
				<td width="10%">
				<input type="hidden" value="<%=storeItem.getId()%>" name="storeItemId<%=inc%>" /> 
				<input type="text" readonly="readonly" value="<%=storeItem.getBatchNo() %>" /> 
				<input type="hidden" name="<%=RequestConstants.BATCH_NO %>"value="<%=storeItem.getBatchNo() %>" /> 
				<input type="hidden"name="visitId" id="visitId" value="<%=visit %>" />
				</td>
				<td width="13%">
				<input type="text" name="<%=RequestConstants.AU %>" size="5" readonly="readonly" value="<%=storeItem.getItem().getItemConversion().getPurchaseUnit().getUnitName()%>" />
				</td>
				<td width="13%">
				...
				<input type="text" name="unitRate<%=inc%>" size="5" readonly="readonly" value="<%=mrp%>" />
				</td>
				<%
	 	...
	 	%>
				...
				<!-- dilute condition added by amit das on 19-11-2016 -->
				<%if(null !=storeItem1.getItem().getMixable()){ %>
		<input
					value="<%=""+storeItem.getClosingStock().intValue()%><%=(storeItem.getItem().getMixable().equalsIgnoreCase("Y"))? "("+storeItem.getClosingStock().intValue()*storeItem.getItem().getMixtureQuantity()+" "+storeItem.getItem().getDispUnit()+")":""%>" id="<%=stockIn%>"
					readonly="readonly" /> 
	<%}else{ %>
	<input
					value="" id="<%=stockIn%>"
...
<!-- 					readonly="readonly" />  -->
				...
				<td width="13%"><input type="text" tabindex="1"
					...
					validate='Issued Quantity,float,yes' class="medcaption"
					id="<%=issuedQtyTemp %>"
	...
	<input type="hidden" name="presId" id="presId<%=nextRowVal%>"></input>
	...
	<div id="saveId" style="display: inline;">
		<input id="save" property="save" type="button" name="Submit13"
			...
			<%-- onclick="if(setTotalQty(<%=inc%>,<%=rowVal%>)){submitProtoAjax('itemIssuePopUp','/hms/hms/stores?method=addPatientDrugIssue');disableSaveButton();}" /> --%>
			onclick="if(setTotalQty(<%=inc%>,<%=rowVal%>)){submitProtoAjax('itemIssuePopUp','/hms/hms/stores?method=addPatientDrugIssue');}" />
	</div>
	<%}%>
