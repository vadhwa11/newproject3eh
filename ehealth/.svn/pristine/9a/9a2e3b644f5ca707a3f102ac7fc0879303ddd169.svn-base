...
<%
	...
	List<MasWing> wingList = (List<MasWing>)map.get("wingList");
	String userName = "";
...
<div class="clear"></div>
<input type="button" name="search" value="Search" class="button" onclick="submitForm('search','personnel?method=wingSearch','checkSearch')" tabindex=1 /> 
<%--- Report Button    
...
<% 
	...
	if(wingList.size()==0 && map.get("search") != null)
	  {
	 %><h4> <a href="personnel?method=showWingJsp">Show All Records</a></h4> <%
    }
...
<div class="clear"></div>
<div class="Block">
<label><span>*</span> Wing Code </label>
...
<label><span>*</span> Wing Name </label> 
...
<div class="clear"></div>
<div id="edited"></div>
<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('empStatus','personnel?method=addWing');" accesskey="a" tabindex=1 /> 
<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button" onClick="submitForm('empStatus','personnel?method=editWing')" accesskey="u" tabindex=1 /> 
<input type="button" name="Delete"	id="deletebutton" value="Activate" style="display: none;" class="button" onClick="submitForm('empStatus','personnel?method=deleteWing&flag='+this.value)" accesskey="d" tabindex=1 /> 
...
data_header[0][0] = "Wing Code"
data_header[0][1] = "data";
data_header[0][2] = "25%";
data_header[0][3] = "<%= EMP_STATUS_CODE%>"

data_header[1] = new Array;
data_header[1][0] = "Wing Name"
...
<%
Iterator itr=wingList.iterator();
          int  counter=0;
          while(itr.hasNext())
           {
        	  MasWing  masWing = (MasWing)itr.next(); 
%>

data_arr[<%= counter%>] = new Array();
data_arr[<%= counter%>][0] = <%= masWing.getId()%>
data_arr[<%= counter%>][1] = "<%=masWing.getWingCode()%>"
data_arr[<%= counter%>][2] = "<%= masWing.getWingName()%>"
data_arr[<%= counter%>][3] = "<%= masWing.getLastChgBy() %>"
data_arr[<%= counter%>][4] = "<%= HMSUtil.convertDateToStringWithoutTime(masWing.getLastChgDate()) %>"
data_arr[<%= counter%>][5] = "<%= masWing.getLastChgTime() %>"
