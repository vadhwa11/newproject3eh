...
<input type="hidden" name="<%=CHANGED_BY%>" value="<%=userName%>" /> <input
	...
	formFields = [
			[0, "<%= HIN_ID%>", "id"],[1,"diagDate"],[2,"diagTime"],[3,"serNo"], [4,"rank"], [5,"sPerson"], [6,"serviceType"], [7,"hin"], [8,"patName"], [9,"relation"], [10,"orderBy"] ];
	 statusTd = 10;
	</script></div>
 <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
data_header[9][2] = "15%";
data_header[9][3] = "subDept";

data_arr = new Array();
	<%
	    int  counter=0;
		if (patientDeatilList != null && patientDeatilList.size() > 0) {
			if(deptType.equalsIgnoreCase("DIAG") || deptType.equalsIgnoreCase("RADIO")){ 
		...
		%>
			...
			if(patient.getSFirstName() != null  && !(patient.getSFirstName().equals(""))){
				...
				<%
				if(dgSampleCollectionHeader.getOrderByDepartment() != null){
				%>
					data_arr[<%= counter%>][10] = "<%=dgSampleCollectionHeader.getOrderByDepartment().getDepartmentName()%> "
				<%}%>
			<%
