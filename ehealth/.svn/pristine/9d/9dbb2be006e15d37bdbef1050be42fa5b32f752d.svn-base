...
function checkk()
{
var SDate = document.c_diseases.<%= FROM_DATE%>.value;
var EDate = document.c_diseases.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
<h2>Consolidate Communicable Diseases</h2>
</div>
<form name="c_diseases" method="post" action="">
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.c_diseases.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.c_diseases.<%=TO_DATE%>,event);" />
<div class="clear"></div>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('c_diseases','pubHealth?method=generateConsolidat_communicable_diseasesReport','checkk()');"
