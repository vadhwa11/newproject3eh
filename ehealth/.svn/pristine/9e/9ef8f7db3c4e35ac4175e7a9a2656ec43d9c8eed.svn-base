...
public class PatientOrderBookingDataToLeanServerSchedular extends QuartzJobBean {

	private static final Logger logger = org.apache.log4j.Logger
			.getLogger(PatientOrderBookingDataToLeanServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		String ack = null;
		List<LeanServerSampleData> leanServerOrderDatas = null;
		Map<String, Object> dataMap = labDataService
				.getPatientOrderBookingDataForLeanServer();

		if (dataMap.get("leanServerOrderDatas") != null) {
			leanServerOrderDatas = (List<LeanServerSampleData>) dataMap
					.get("leanServerOrderDatas");
		}
		if (leanServerOrderDatas != null) {
			for (LeanServerSampleData leanServerSampleData : leanServerOrderDatas) {
	...
	private String hl7MessageSender(String msg, MasHospital hospital) {
		...
		try {
			...
			logger.debug("MalformedURLException in Patient Sample Collection Data To Lean Server Schedular");
		} catch (ConnectException e) {
			logger.debug("Connection refused in Patient Sample Collection Data To Lean Server Schedular");
		} catch (SocketTimeoutException e) {
			logger.debug("Connection timeout in Patient Sample Collection Data To Lean Server Schedular");
		} catch (IOException e) {
			logger.debug("IOException in Patient Sample Collection Data To Lean Server Schedular");
		}
