...
public class PatientAppointmentDataToLeanServerSchedular extends QuartzJobBean {

	private static final Logger logger = org.apache.log4j.Logger.getLogger(PatientAppointmentDataToLeanServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		Map<String,Object>	dataMap=registrationDataService.getPatientAppointmentDataForLeanServer();
		if(dataMap.get("leanServerPatientAppointmentData")!=null){
			leanServerPatientAppointmentDatas = (List<LeanServerPatientAppointmentData>)dataMap.get("leanServerPatientAppointmentData");
		} 
		if(leanServerPatientAppointmentDatas!=null){
			for(LeanServerPatientAppointmentData leanServerPatientAppointmentData : leanServerPatientAppointmentDatas){ 
				if(leanServerPatientAppointmentData.getHospitalId()!=null) {  
					objectMap.put("hospitalId", leanServerPatientAppointmentData.getHospitalId()); 
					...
					if(dataMap.get("hospital")!=null){ 
						...
						ack = hl7MessageSender(leanServerPatientAppointmentData.getPaitentAppointmentData(),hospital); 
						if("1".equalsIgnoreCase(ack)){ 
							registrationDataService.updateLeanServerPatientAppointmentData(leanServerPatientAppointmentData);	  
						}
	...
	private String hl7MessageSender(String msg,MasHospital hospital) {
		...
		String uri=header+"/hms/hms/appointment?method=saveClientAppointmentPatientToServer&message="
