...
<%
		...
		List<BloodRequestEntryHeader> requestheaderList=null;
		
		if(request.getAttribute("map") != null){
			map = (Map<String, Object>)request.getAttribute("map");
		}
		requestheaderList=new ArrayList<BloodRequestEntryHeader>();
...
<div class="titleBg">
<h2>Pending Sample Validation</h2>
</div>
...
<table width="100%" colspan="7" id="chargeDetails" cellpadding="0"
	...
	for(BloodRequestEntryHeader bllodRequest:requestheaderList){
		...
		if(null !=bllodRequest.getInpatient()){
			if(null != bllodRequest.getInpatient().getHin()) {
				hinNo = bllodRequest.getInpatient().getHin().getHinNo();
				patientName = bllodRequest.getInpatient().getHin().getPFirstName();
				age = bllodRequest.getInpatient().getHin().getAge();
			}
			...
			hinNo = bllodRequest.getHin().getHinNo();
			patientName = bllodRequest.getHin().getPFirstName();
			age = bllodRequest.getHin().getAge();
		}
	
		%>
	
		<tr onclick="populateBloodRequestValidation('<%=bllodRequest.getId() %>')" style="cursor: pointer;">
