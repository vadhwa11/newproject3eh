...
<%-- function checkk()
{
var SDate = document.freezed.<%= FROM_DATE%>.value;
var EDate = document.freezed.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
<h2>Report of Items certified as Sub-Standard</h2>
</div>
<form name="substandard" method="post" action="">
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	class="date" validate="From Date,dateOfAdmission,no" MAXLENGTH="30"
	readonly="readonly" />
	<img id="calendar"
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.substandard.<%=FROM_DATE%>,event);" />
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('substandard','stores?method=generateSubStanderd');"
