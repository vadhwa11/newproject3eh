...
<form name="patientSearch" action="" method="post">
	...
	<%
		...
		if(patientMap.get("pharmacyLabQueueList") != null){
			pharmacyLabQueueList= (List<PharmacyLabQueue>)patientMap.get("pharmacyLabQueueList");
		}
		...
		String FromDate="",ToDate="",patientFName="",hinNo1="",empName="";
		int tokenNo=0;
		if(map.get("fromDate")!=null){
			FromDate=HMSUtil.convertDateToStringWithoutTime((Date)map.get("fromDate"));
		}
		if(map.get("toDate")!=null){
			ToDate=HMSUtil.convertDateToStringWithoutTime((Date)map.get("toDate"));
		}
		if(map.get("patientFName")!=null){
			patientFName=(String)map.get("patientFName");
		}
		...
		if(map.get("doctorName")!=null){
			empName=(String)map.get("doctorName");
		}
		if(FromDate.equals("")){
...
%>
	...
	<div class="Block">
		...
		<label><span>*</span> From Date</label> <input type="text"
			...
			value="<%=FromDate%>" readonly="readonly" MAXLENGTH="30" /> <img
		...
		<label><span>*</span> To Date</label> <input type="text" id="ToDateId"
			name="<%=TO_DATE %>" value="<%=ToDate %>" class="date"
		...
		<label><%=prop.getProperty("com.jkt.hms.registration_no")%></label> <input
			id="hinId" type="text" name="<%=HIN_NO %>" value="<%=hinNo1 %>" MAXLENGTH="16" onkeypress="return searchKeyPress(event);"/>

		<div class="clear"></div>
		<label>Patient Name</label> <input type="text"
			name="<%=P_FIRST_NAME %>" id="<%=P_FIRST_NAME %>" 
			value="<%=patientFName %>" MAXLENGTH="15" validate="Patient Name,metachar,no" onkeypress="return searchKeyPress(event);"/>
		...
		<label><%=prop.getProperty("com.jkt.hms.token_num")%></label> <input
			type="text" name="<%=VISIT_NUMBER %>" id="<%=VISIT_NUMBER %>" value="<%=tokenNo>0?tokenNo:"" %>" MAXLENGTH="3" validate="visitNumber,int,no"/>
		<%}%>
		<label>Doctor Name</label> <input type="text" name="<%=DOCTOR_NAME %>" id="<%=DOCTOR_NAME %>" value="<%=empName %>"
			value="" MAXLENGTH="15"  validate="Doctor Name,metachar,no" onkeypress="return searchKeyPress(event);"/>
		<div class="clear"></div>
		<label>Sample</label>
		...
		<label>Modality</label><%if(subGroupId!=0){ %> <select id="subChargeCodeId"
		...
		<input type="hidden" id="patientType" name="patientType" value="<%=patientType%>" validate="patientType,metachar,no"/> 
		...
		<input type="button" value="Search" class="button"	onclick="searchPatient()" id="btnSearchSample"/>
...
<div id="searchtable" tabindex="2"></div>
	...
	<script type="text/javascript">
	  formFields = [
			[0, "orderId", "id"],[1,"tokenNo"],[2,"orderNo"],[3,"admissionDate"],[4,"ipNo"],[5,"orderDate"],[6,"orderTime"], 
			[7,"hin"],[8,"patName"], [9,"age"], [10,"sex"],[11,"wardNo"],[12,"bedNo"], [13,"pType"],[14,"doctorName"],
			[15,"department"],[16,"departmentWard"],[17,"specialty"],[18,"Priority"],[19,"visitNo"],[20,"chargeSlipNo"],[21,"subGroupId"]];
	  statusTd = 15;
	</script>
</form>
</div>
...
data_header[18][0] = "Visit No." 
data_header[18][1] = "hide";  
data_header[18][2] = "10%";
data_header[18][3] = "visitNo"  

data_header[19] = new Array;
data_header[19][0] = "ChargeSlipNo" 
data_header[19][1] = "hide";  
data_header[19][2] = "10%";
data_header[19][3] = "chargeSlipNo" 

data_header[20] = new Array;
data_header[20][0] = "subGroupId" 
data_header[20][1] = "hide";  
data_header[20][2] = "10%";
data_header[20][3] = "<%=subGroupId %>"
...
data_arr = new Array();
	...
	<%
		...
		String hinNo = "";
		String  hinId="";
		Map<String,String> multipleDepartOrderMap=new HashMap<String,String>();
		outerloop:
		for(PharmacyLabQueue ladQueue: pharmacyLabQueueList){
			if(RequestConstants.UHID_FOR_QUALITY_TESTING.equals(ladQueue.getDgOrderhdId().getHin().getHinNo())){
				continue;
			}
			String flag = "";
			if(deptType.equalsIgnoreCase("DIAG")){
				...
				set = ladQueue.getDgOrderhdId().getDgOrderdts();
			for(DgOrderdt orderDt : set){
			//if(orderDt.getMainChargecode().getDepartment().getDepartmentType()==null){
//			}
			//System.out.println("dt Id "+orderDt.getId());
			//System.out.println("Main Chare Id "+orderDt.getMainChargecode().getId());
			//System.out.println("depart Id "+orderDt.getMainChargecode().getDepartment().getId());
			//System.out.println("dt Id "+orderDt.getId());
					//System.out.println("Main Chare Id "+orderDt.getMainChargecode().getId());
					//System.out.println("Main Chare Id "+orderDt.getMainChargecode().getMainChargecodeName());
					//System.out.println("@#depart Id "+orderDt.getMainChargecode().getDepartment().getId());
				if((orderDt.getMainChargecode().getDepartment().getDepartmentType().getDepartmentTypeCode().equalsIgnoreCase("DIAG"))&&
					...
					flag = "lab" ;
				}else if((orderDt.getMainChargecode().getDepartment().getDepartmentType().getDepartmentTypeCode().equalsIgnoreCase("DIAG"))&&
						(orderDt.getOrderStatus().equalsIgnoreCase("P")) &&("IP".equalsIgnoreCase(orderDt.getOrderhd().getPatientType()))){
					flag = "lab" ;
			...
			patientName=ladQueue.getDgOrderhdId().getHin().getFullName();
			
				age=ladQueue.getDgOrderhdId().getHin().getAge();
				if(ladQueue.getDgOrderhdId().getHin().getSex()!=null)
			sex=ladQueue.getDgOrderhdId().getHin().getSex().getAdministrativeSexName();
			hinNo=ladQueue.getDgOrderhdId().getHin().getHinNo();
			
			if(!multipleDepartOrderMap.containsKey(hinNo)){
				multipleDepartOrderMap.put(hinNo,hinNo);
				
			if(flag.equals("lab") || flag.equals("") ){
			%> 
			  		data_arr[<%= counter%>] = new Array();
					data_arr[<%= counter%>][0] = <%= ladQueue.getDgOrderhdId().getHin().getId()%>
										
					<%if(ladQueue.getDgOrderhdId().getVisit()!=null && !"".equalsIgnoreCase(ladQueue.getTokenNo().toString())){%>
						data_arr[<%= counter%>][1] = "<%= ladQueue.getTokenNo()%>"
					<%}else{%>
						data_arr[<%= counter%>][1] = ""
					<%}%>	
					data_arr[<%= counter%>][2] = "<%= ladQueue.getDgOrderhdId().getOrderNo()%>"
					data_arr[<%= counter%>][3] = "01/03/2015"
					<%String ipdNo="";
					String ward="";
					String bedNo="";
					if(ladQueue.getDgOrderhdId().getInpatient()!=null){
						ipdNo=ladQueue.getDgOrderhdId().getInpatient().getAdNo();
						bedNo=ladQueue.getDgOrderhdId().getInpatient().getBed().getBedNo();
						ladQueue.getDgOrderhdId().getInpatient().getDepartment();
					}%>
					data_arr[<%= counter%>][4] = "<%=ipdNo%>"
					data_arr[<%= counter%>][5] = "<%= HMSUtil.convertDateToStringWithoutTime(ladQueue.getDgOrderhdId().getOrderDate())%>"
					data_arr[<%= counter%>][6] = "<%=ladQueue.getDgOrderhdId().getOrderTime()%>"
					...
					if(ladQueue.getDgOrderhdId().getPrescribedBy()!=null){
						
						if(ladQueue.getDgOrderhdId().getPrescribedBy().getEmployeeName()!=null){
							doctorName=doctorName.concat(ladQueue.getDgOrderhdId().getPrescribedBy().getEmployeeName()+" ");
						} 	
					} 
					if(ladQueue.getDgOrderhdId().getDepartment()!=null && !"".equals(ladQueue.getDgOrderhdId().getDepartment().getDepartmentName())){
						departmentName=ladQueue.getDgOrderhdId().getDepartment().getDepartmentName();
					}
					%>
					data_arr[<%= counter%>][11] = "<%=departmentName%>"
					data_arr[<%= counter%>][12] = "<%=bedNo%>"
					data_arr[<%= counter%>][13] = "<%=ladQueue.getDgOrderhdId().getPatientType()%>" 
					...
					<% 
					String status="";
					if("u".equalsIgnoreCase(ladQueue.getDgOrderhdId().getRoutineUrgentStatus())){
						status="Urgent";
					}else if("r".equalsIgnoreCase(ladQueue.getDgOrderhdId().getRoutineUrgentStatus())){
						status="Routine";
					} 
					%>
					data_arr[<%= counter%>][18] = "<%=status%>"
					data_arr[<%= counter%>][19] = "<%=ladQueue.getDgOrderhdId()!=null?ladQueue.getDgOrderhdId().getVisit().getId():0%>"
						
					data_arr[<%= counter%>][20] = "<%=ladQueue.getDgOrderhdId()!=null?ladQueue.getDgOrderhdId().getBillChargeSlpNo():""%>"
							
							data_arr[<%= counter%>][21]="<%=subGroupId!=0?subGroupId:0%>"		
						     <%
						     counter++;
...
<%session.setAttribute("patientDetailList",patientDetailList);%>
	...
	function searchPatient(){
		submitForm('patientSearch','/hms/hms/lab?method=showPendingSampleCollectionSearchResult&flag='+'<%=patientType%>'+'&'+csrfTokenName+'='+csrfTokenValue);
	}
	
	//callSampleCollOpList();//commented by govind 12-07-2017
	function callSampleCollOpList(){
		  var fromDate=document.getElementById("fromDateId").value;
		    var toDate=document.getElementById("ToDateId").value;
		    var hinNo=document.getElementById("hinId").value;
			var pFirstName=document.getElementById("pFirstName").value;
		    var visitNumber=document.getElementById("visitNumber").value;
		    ...
		    var invName=document.getElementById("invId").value;

		    var patientType=document.getElementById("patientType").value;
	   // alert("tokenNo "+tokenNo+" pName "+pName+" hinNo "+hinNo);
	submitProtoAjaxWithDivName('patientSearch','/hms/hms/lab?method=getSampleCollectionOpListRefresh&flag='+patientType+'&searchFlag=1'+'&fromDate='+fromDate+'&toDate='+toDate+'&hinNo='+hinNo+'&pFirstName='
 			+pFirstName+'&visitNumber='+visitNumber+'&doctorName='+doctorName+'&sampleName='+sampleName+'&subChargeCodeId='+subChargeCodeId+'&priorityId='+priorityId+'&subChargeId='+subChargeCodeId+'&invName='+invName,'test');
	
	//submitProtoAjaxWithDivName('pendingSampleCollection','/hms/hms/lab?method=getSampleCollectionOpListRefresh'+getNameAndData('pendingSampleCollection'),'test');
	
		//setTimeout(callSampleCollOpList, 30000);//commented by govind 12-07-2017
	}
	...
	function searchKeyPress(e) {
		...
		e = e || window.event;
		if (e.keyCode == 13) {
			document.getElementById('btnSearchSample').click();
			return false;
		}
		return true;
	}
