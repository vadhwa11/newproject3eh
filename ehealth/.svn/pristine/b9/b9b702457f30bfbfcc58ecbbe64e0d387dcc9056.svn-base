...
<%@page import="jkt.hrms.masters.business.HrMasItaxSurcharge"%>
<%@page import="jkt.hrms.masters.business.HrMasSurcharge"%>
<div class="titleBg">
<h2>Income Tax Surcharge/Edu. Cess</h2>
</div>
<div class="clear"></div>
<div class="Block"></div>
<div class="clear"></div>
...
<%
 		if (searchItaxSurchargeMasterList.size() > 0) {
 			...
 			{
 	%>
<h4><a href="incomeTaxMaster?method=showIncomeTaxSurchargeJsp">Show
All Records</a></h4>
<%
			}
		}
...
<%
	...
	formFields = [
			[0, "<%= COMMON_ID%>", "id"],  [1,"<%= SEARCH_NAME %>"],  [2,"surchargeBase"], [3,"subchargeCode"], [4,"lowerLimit"],[5,"upperLimit"], [6,"perOne"],[7,"perTwo"],[8,"<%= CHANGED_BY%>"],[9,"<%= CHANGED_DATE %>"],[10,"<%= CHANGED_TIME %>"],[11,"<%=STATUS%>"],[12,"<%="minTaxSal"%>"]];
	 statusTd = 11;
	</script></div>
<div class="clear"></div>
<div class="division"></div>
<div class="Block">
<form name="incomeTaxsurcharge" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="HrMasItaxSurcharge" /> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="FinancialYear" />
<input type="hidden" name="title" value="IT Exemption" /> <input
	type="hidden" name="<%=JSP_NAME %>" value="hr_incomeTaxSurcharge" /> <input
	type="hidden" name="pojoPropertyCode" value="" /> <label><span>*</span>
Financial Year</label> <select name="<%= SEARCH_NAME %>"
	...
	<% }
			...
			%>
</select> <label><span>*</span>Surcharge Code</label> <select
	name="surchargeCode" id="surchargeCode"
	validate="Surcharge Code,string,yes" tabindex=1
	...
	<%if(hrMasSurchargeList != null)
			{
				...
				{%>
	<option value="<%=hrMasSurcharge.getId()%>"><%=hrMasSurcharge.getSurchargeDescription() %></option>
	<% }
			}
			%>
</select>
...
<label><span>*</span>Lower Limit</label> <input type="text"
	name="lowerLimit" value="" validate="Lower limit,string,no"
	class="textbox_size20" MAXLENGTH="6" tabindex=1 /> <label><span>*</span>Upper
Limit </label> <input type="text" name="upperLimit" value=""
	validate="Upper limit,string,no" class="textbox_size20" MAXLENGTH="6"
	/ tabindex=1 />

<label><span>*</span>Min.
Taxable Salary </label> <input type="text" name="minTaxSal" value=""
	validate="Min. Taxable Salary ,float,yes" class="textbox_size20"
	MAXLENGTH="10" / tabindex=1 /></div>

<div class="clear"></div>

<label><span>*</span>% One</label> <input type="text" name="perOne"
	value="" validate="% One,string,no" class="textbox_size20"
	MAXLENGTH="6" tabindex=1 /> <label><span>*</span>% Two</label> <input
	type="text" name="perTwo" value="" validate="% Two,string,no"
	class="textbox_size20" MAXLENGTH="6" / tabindex=1 /> <label><span>*</span>Surcharge
Base</label> <select name="surchargeBase" validate="Surcharge Base,string,yes"
	tabindex=1>
	<option value="">Select</option>
...
<input type="button" name="add" id="addbutton" value="Add"
	class="button"
	onClick="submitForm('incomeTaxsurcharge','incomeTaxMaster?method=addIncomeTaxSurchargeJsp');"
	accesskey="a" tabindex=1 />

<input type="button" name="edit" id="editbutton" value="Update"
	style="display: none;" class="button"
	onClick="submitForm('incomeTaxsurcharge','incomeTaxMaster?method=editIncomeTaxSurchargeJsp')"
	accesskey="u" tabindex=1 />

<input type="button" name="Delete" id="deletebutton" value="Activate"
	style="display: none;" class="button"
	onClick="submitForm('incomeTaxsurcharge','incomeTaxMaster?method=deleteIncomeTaxSurchargeJsp&flag='+this.value)"
...
data_arr[<%= counter%>][4] = "<%= hrMasItaxSurcharge.getLowerLimit()%>"

data_arr[<%= counter%>][5] = "<%= hrMasItaxSurcharge.getUpperLimit()%>"

data_arr[<%= counter%>][6] = "<%= hrMasItaxSurcharge.getPercentOne()%>"

data_arr[<%= counter%>][7] = "<%= hrMasItaxSurcharge.getPercentTwo()%>"


data_arr[<%= counter%>][8] = "<%= hrMasItaxSurcharge.getLastChgBy() %>"
