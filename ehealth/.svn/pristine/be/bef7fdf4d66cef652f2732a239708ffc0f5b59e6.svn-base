...
<%@page import="jkt.hms.masters.business.OtIntraOperativeTimeOut"%>
...
<div class="clear"></div>
<div class="Block">
<label>UHID</label> <input type="text" id="uhid" name="uhid"  maxlength="15"></input>
...
<script type="text/javascript">
  ...
  <input type="button" name="Submit" id="addbutton" value="Search" class="button" accesskey="a"
	onclick="if(checkBlank()){submitForm('search1','/hms/hms/ot?method=showOtSignoutList&otProcedure=<%=otProcedure%>');}" />
 <%-- 
...
<div class="clear"></div>
<form name="otSignOut" method="post" action="">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
<script type="text/javascript" language="javascript">
	...
	%>
			...
			data_arr[<%= counter%>][3] = "<%=otBooking.getHin().getPFirstName()%>"
			data_arr[<%= counter%>][4] = ""
			data_arr[<%= counter%>][5] = ""
			data_arr[<%= counter%>][6] = ""
			data_arr[<%= counter%>][7] = ""
