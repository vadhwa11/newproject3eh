...
<%@page import="jkt.hms.masters.business.CssdTemplateInstrument"%><script type="text/javascript" src="/hms/jsp/js/stores.js"></script>
...
<%
	...
	if(map.get("opdTemplateList")!=null){
		opdTemplateList = (ArrayList<OpdTemplate>)map.get("opdTemplateList");
	}
	String userName = "";
...
<form name="autoclaveRequestForm" method="post">
...
{--%>
	<%-- value="cssdAutoclaveRequestM.getOrderNo()"
}
--%>
...
Order By</label> <select name="orderBy" id="orderBy" validate="Employee,String,yes">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<%
		if(cssdTemplateInstrumentList.size()>0){
			for(CssdTemplateInstrument cssdTemplateInstrument:cssdTemplateInstrumentList){
				
			
	%>
	<tr>
		<td>
		<input id="instrumentId" name="instrumentId" type="hidden"
			size="10" MAXLENGTH="10" value="<%=cssdTemplateInstrument.getInstrument().getId() %>"/>
		<input id="instrumentCode" name="instrumentCode" type="text"
			size="10" MAXLENGTH="10" value="<%=cssdTemplateInstrument.getInstrument().getInstrumentCode() %>" readonly="readonly" /></td>

		<td><%=cssdTemplateInstrument.getInstrument().getInstrumentName() %>
		<!--<input type="text" id="instrumentName" name="instrumentName" value="" readonly="readonly" 
			size="25"/>-->
		</td>

		
		<td><input id="qty" name="qty" type="text" value="1" size="3" validate="Qty,num,no" MAXLENGTH="3" /></td>
		<td><input id="remarks" name="remarks" type="text" size="25"
			MAXLENGTH="25" validate="Remarks,String,no"/></td>
...
<div class="clear"></div>
<div class="paddingTop15"></div>
<input name="Button" type="button" class="button" value="Submit" onclick="submitForm('autoclaveRequestForm','/hms/hms/cssd?method=saveAndRefreshAutoclaveRequestGrid');" />
<div class="clear"></div>
<div class="paddingTop15"></div>		
