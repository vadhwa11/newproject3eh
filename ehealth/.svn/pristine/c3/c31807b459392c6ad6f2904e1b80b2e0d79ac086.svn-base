...
<script type="text/javascript" language="javascript">
	<%
		...
		List<MasDistrict> districts = new ArrayList<MasDistrict>();
     	if(map.get("districts")!=null){
     		districts=(List<MasDistrict>)map.get("districts");
		}
...
<select name="disId" id="disId" onchange="submitProtoAjaxWithDivName('search','/hms/hms/pubHealth?method=getTalukListHouseSurvey','talukDiv');">
	<option value="0">Select</option>
	<%
	for(MasDistrict dis:districts){
	%>
...
<div class="clear"></div>
<input type="button" name="Search" id="addbutton"	onclick="if(checkData()){submitForm('search','pubHealth?method=showHouseSurveyInStateJsp');}"	value="Search" class="button"  />
<div class="clear"></div>
</div>
		</form>
<div class="Block">
...
<tbody id="tableData">
	...
	%> 
			<form name="phHouseSurveys<%= counter+1%>" method="post"><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
