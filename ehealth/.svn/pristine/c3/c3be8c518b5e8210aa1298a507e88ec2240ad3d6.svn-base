...
if(map.get("phComm") !=null){
	phComm=(List<PhDiseaseRegistration>)map.get("phComm");
}
...
%>
     ...
     <select name="bName<%= rowNo%>" id="bName<%= rowNo%>"  multiple="multiple" style="height: 50px;" onclick="total(this);">
      <%	if(phComm.size() >0){%>
			
				<option value="0">Select</option>
	
				 <%
				 for (PhDiseaseRegistration tal : phComm) {
				  ...
				  <%
				   ...
				   %>
		              ...
		              <%
		              int m=0;
		              for (PhDiseaseRegistration rote : phComm) {
