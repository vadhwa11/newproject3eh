...
prop.load(in);
	...
	if(map.get("visitPatientList") != null){
		visitPatientList=(List<Visit>)map.get("visitPatientList");
	}
	String userName = "";
...
<input type="hidden"	name="date" id="date" value="<%=date %>" validate="Pick a date"/>
<div class="clear"></div>

	<label>Patient First Name : </label>
	<input type="text"	name="<%= RequestConstants.P_FIRST_NAME %>" id="patientFName"	MAXLENGTH="30" validate="patientFName,String,yes"  tabindex=1  />

<label>Patient Middle Name : </label>
<input type="text"	name="<%= RequestConstants.P_MIDDLE_NAME %>" id="patientMName"	MAXLENGTH="30" validate="patientFName,String,yes"  tabindex=1 />
<div class="clear"></div>
<label>Patient Last Name : </label>
<input	type="text" name="<%= RequestConstants.P_LAST_NAME %>"	id="pateintLName" MAXLENGTH="30" validate="patientFName,String,yes"  tabindex=1 />
...
<form name="opdPhysiotherapyList" method="post">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
<div class="Block"><label class="auto">Consulting Doctor</label> <select
	name="consultingDoc" validate="consultingDoc,string,yes">
	...
	onClick="submitForm('opdPhysiotherapyList','opd?method=showPhysiotherapyPatientList&filter=true')"
	class="button" "/>
...
<script type="text/javascript" language="javascript">
	...
	for(int j=0;j<visitPatientList.size();j++)
	{
	...
	{
	%>
	data_arr[<%= i%>][4] = "<%=visit.getVisitDate()%>"
	<%
	}else{
	%>
	data_arr[<%= i%>][4] = ""
	...
	if(visit.getHin().getHinNo() !="")
	{
	%>
	data_arr[<%= i%>][7] = "<%=visit.getHin().getHinNo()%>"
	<%
	}else{
	%>
	data_arr[<%= i%>][7] = ""
	...
	data_arr[<%= i%>][8] = ""
	<%}
	if(patientHin.getAge() != null)
	{
	%>

	data_arr[<%= i%>][9] = "<%=patientHin.getAge()%>"
	<%
	}else{
	%>
	data_arr[<%= i%>][9] = ""
	<%   }if(patientHin.getSex()!=null){
	%>
	data_arr[<%= i%>][10] = "<%=patientHin.getSex().getAdministrativeSexName()%>"
	<%
	}else{
	%>
	data_arr[<%= i%>][10] = ""
	...
	data_arr[<%= i%>][11] =""
	<%}%>
	data_arr[<%=i%>][12] = '<input type="hidden" id="iCardStatus<%=i%>"  name="iCardStatus<%=i%>" value="<%=visit.getHin().getId()%>"  />'
	<%
	i++;
	}
