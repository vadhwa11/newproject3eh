...
function buildCal(m, y){
 ...
 for(i=1;i<=42;i++){
  ...
  if(x!='&nbsp;'){
   ...
   if(document.updateRegistration){
	  
	   	if(document.getElementById('qDobId') == dateobj1){
			   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.calculateAgeInAjax();window.close();'
	   		}else{
	   	...
	   	if(document.getElementById('dobId') == dateobj1){
			   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.calculateAgeInAjax();window.close();'
	   		}else{
	   		    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
	   		}
	   		...
	   		if(document.getElementById('reviewDate') == dateobj1){
		   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.checkReviewDate();window.close();'
	   		}else{
	   		    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
	   		}	   
	   	} 
		   	...
		   	if(document.admissionByHin.dependentCardIssueDate == dateobj1){
   			 ...
   			 }else if(document.addresume){
   			   	if(document.addresume.dobForVerification == dateobj1){
   			   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.validateAge();window.close();'
		   		}else{
		   		    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		   		}
		   	  }else if(document.dischargeByHin){
    		//	if(document.newTravelRequest.tDate == dateobj1 || document.newTravelRequest.fDate == dateobj1 ||){
     				if(dateobj1.name =="followUp")
      					tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.checkFollowUpDate();window.close();'
     				else
      					tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
    				} else if(document.grnGrid){
    		//	if(document.newTravelRequest.tDate == dateobj1 || document.newTravelRequest.fDate == dateobj1 ||){
     				if(dateobj1.name =="invoiceDate")
      					tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.checkInvoiceDate();window.close();'
     				else
      					tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
    				} 
    			//	}  
...
function openWindowForCalender(text,posy,posx){
 ...
 var styleStr ="chrome,modal,personalbar=no, top="+ posy + ", left= " + posx + ", hotkeys=yes,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbar=no,resizable=no,copyhistory=yes,width=260,height=320";
