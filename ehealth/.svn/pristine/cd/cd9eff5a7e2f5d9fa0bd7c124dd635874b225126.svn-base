...
<%
	...
	HesEquipmentBreakdownEntry hesBreakdown = new HesEquipmentBreakdownEntry();
	...
	{
		ratio_str = (Integer)map.get("radio_str");
	}
...
<input type="text" name="<%=ON_DATE%>"
	...
	<img	id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16"	border="0" validate="Pick a date"	onclick="setdate('<%=currentDate%>',document.equipmentBreakdownEntryForm.<%= ON_DATE%>,event);" />
...
<input type="text" id="breakId" rows="8" name="breakId" value="<%=hesBreakdown.getNatureOfBreakdown() %>" class="textbox_size20" > </input>
