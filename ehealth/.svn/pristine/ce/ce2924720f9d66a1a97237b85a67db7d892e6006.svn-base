...
<input type="button" name="search" value="Search" class="button" onclick="submitForm('search','inPatientMaster?method=searchCareType','checkSearch')" tabindex=1 /> 
...
<input type="hidden" name="<%=JASPER_FILE_NAME%>" value="Mas_care_type" validate="Administrative Sex Code,string,no"></form>
</div>
</div>
<div class="clear"></div>
</div>
...
<% 
  if(searchCareTypeList.size()>0)
   ...
   {
 %> <h4><a href="inPatientMaster?method=showCareTypeJsp">Show All
Records</a> </h4><%
   }
   }
...
<input id="codeId" type="text" name="<%= CODE%>" value="" validate="CareType Code,string,yes" class="textbox_size20"
	MAXLENGTH="8" / tabindex=1>
<label><span>*</span> CareType Name</label>
<input type="text" name="<%= SEARCH_NAME %>" value="" validate="CareType Name,string,yes" class="textbox_size20" MAXLENGTH="30" / tabindex=1 onkeypress="return submitenter(this,event,'inPatientMaster?method=addCareType')">
<script>
...
<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button" onClick="submitForm('careType','inPatientMaster?method=editCareType')" accesskey="u" tabindex=1 /> 
...
data_arr[<%= counter%>][2] = "<%= masCareType.getCareTypeName()%>"
	data_arr[<%= counter%>][3] = "<%= masCareType.getLastChgBy()!=null?(masCareType.getLastChgBy().getId()!=null?masCareType.getLastChgBy().getId():"0" ):"0"%>"
