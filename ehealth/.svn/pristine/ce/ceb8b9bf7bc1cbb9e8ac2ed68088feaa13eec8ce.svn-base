...
<div class="clear"></div>
	...
	<div class="Block">
		...
		<%
			...
			String fName = ""; 
			if (dgSampleCollectionHeader.getOrder().getPrescribedBy()
			...
			String mobileNumber = "-";
			if (dgSampleCollectionHeader.getOrder().getHin().getMobileNumber() != null
			...
			String unit = "-";
			if (dgSampleCollectionHeader.getOrder().getHin().getUnit() != null
					&& !"".equalsIgnoreCase(dgSampleCollectionHeader.getOrder()
							.getHin().getUnit().getUnitName().trim())) {
				unit = dgSampleCollectionHeader.getOrder().getHin().getUnit()
						.getUnitName();
			}
		%>
	...
	<div class="Block">
		...
		<%
			...
			Integer empId = masEmpaneled.getId();
			String empFName = "";
 ...
 %>
				...
				<td><input type="checkbox" id="rejected<%=inc%>"
					name="check1<%=inc%>" value="n" onclick="rejected(this,<%=inc%>);" /></td>
				<td>
					<%
 ...
 %>
	 ...
	 %>
				...
				<%
					inc++;
								...
								.equalsIgnoreCase("P")) {
							System.out.println("Detail Id >>"+dgSampleCollectionDetails.getId());
							%>
	...
	<input type="button" class="button" value="Submit"
		onclick="submitForm('sampleAcceptance','lab?method=submitSampleAcceptanceForEmpanelled','validateSampleValidation');"
