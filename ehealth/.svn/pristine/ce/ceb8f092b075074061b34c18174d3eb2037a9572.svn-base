...
public class PatientVisitUpdateToCentralServerSchedular extends QuartzJobBean {

	private static final Logger logger = org.apache.log4j.Logger
			.getLogger(PatientVisitUpdateToCentralServerSchedular.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		Map<String, Object> dataMap = registrationDataService
				.getPatientVisitUpdateForCentralServer();
	...
	private String hl7MessageSender(String msg, MasHospital hospital) {
		...
		HohClientSimple client = new HohClientSimple(hospital.getServerIp(),
				Integer.parseInt(hospital.getServerPort()),
		...
		try {
			...
			logger.debug("EncodingNotSupportedException in Patient Visit Update To Central Server Schedular");
		} catch (HL7Exception e) {
			logger.debug("HL7Exception in Patient Visit Update To Central Server Schedular");
		} catch (ConnectException e) {
			logger.debug("Connection refused in Patient Visit Update To Central Server Schedular");
		} catch (SocketTimeoutException e) {
			logger.debug("Connection timeout in Patient Visit Update To Central Server Schedular");
		} catch (IOException e) {
			logger.debug("IOException in Patient Visit Update To Central Server Schedular");
		} catch (DecodeException e) {
			logger.debug("DecodeException in Patient Visit Update To Central Server Schedular");
		} catch (EncodeException e) {
			logger.debug("EncodeException in Patient Visit Update To Central Server Schedular");
		}
