...
<!--  <select><option>Select</option></select> -->
	...
	<label>Item Name</label>
	 ...
	 <div class="clear"></div>
<input type="button" name="Search" id="addbutton" onclick="if(checkData()){submitForm('search','maintenance?method=showPendingListOfInspctionJsp');}"	value="Search" class="button" />
<input type="button" name="Search" id="addbutton" onclick="if(checkData()){submitForm('search','maintenance?method=showAllPendingListOfInspctionJsp');}"	value="Show All" class="button" />
</div>
...
<table>
	...
	<% 
		...
		for(MmServiceRequest object:mmServiceRequest){
			...
			if(object.getApprovedBy()!=null){
				approvedBy=object.getApprovedBy().getEmployee().getEmployeeName();
	...
	%>
			...
			<tr onclick="submitForm('servicRequest<%= counter+1%>', 'maintenance?method=showAssignResourceJsp')" style="cursor: pointer;"><td><%= counter+1%></td>
