...
<div class="titleBg">
<h2>Item Master Local(Non-Medical)</h2>
</div>
<div class="clear"></div>
<div class="Block">
<div id="searcharea">
...
<input type="button" name="search" value="Search" class="button" onclick="submitForm('search','pharmacy?method=searchItemLocalNonMed','checkSearch')"	tabindex=1 />
...
<input type="text"	name="<%= SEARCH_NAME%>" value="" readonly="readonly" validate="Nomenclature,string,yes"	MAXLENGTH="250" style="width: 550px;" tabindex=1 />

<label><span>*</span> Common Name</label>
<input type="text"	id="commonName" name="commonName" value="" disabled="disabled" validate="Common Name,string,yes"	maxlength="50"  tabindex=1 />
...
<input	type="button" name="edit" id="editbutton" value="Update"	style="display: none" class="button" onclick="submitForm('item','pharmacy?method=editItemLocalNonMed&flag='+this.value)"	accesskey="u" tabindex=1 />
<input type="button" name="Delete"	id="deletebutton" style="display: none" value="Activate" class="button"	onclick="submitForm('item','pharmacy?method=deleteItemLocalNonMed&flag='+this.value)" accesskey="d"	tabindex=1 />
<input type="reset" name="Reset" id="reset"	value="Reset" class="buttonHighlight" onclick="submitFormForButton('item','pharmacy?method=showItemLocalNonMedJsp)" accesskey="d"	tabindex=1 accesskey="r" />
...
data_header[9][2] = 0;
data_header[9][3] = "<%=MIN_STOCK %>";

data_header[10] = new Array;
data_header[10][0] = ""
data_header[10][1] = "hide";
data_header[10][2] = 0;
data_header[10][3] = "<%=MAX_STOCK %>";

data_header[11] = new Array;
data_header[11][0] = ""
data_header[11][1] = "hide";
data_header[11][2] = 0;
data_header[11][3] = "<%=LEAD_TIME %>";
...
data_header[12][2] = 0;
data_header[12][3] = "<%=SPECIFICATION %>";

data_header[13] = new Array;
data_header[13][0] = ""
data_header[13][1] = "hide";
data_header[13][2] = 0;
data_header[13][3] = "<%=SLOW_MOVING_DAYS %>";

data_header[14] = new Array;
data_header[14][0] = ""
data_header[14][1] = "hide";
data_header[14][2] = 0;
data_header[14][3] = "<%=FAST_MOVING_DAYS %>";

data_header[15] = new Array;
data_header[15][0] = ""
data_header[15][1] = "hide";
data_header[15][2] = 0;
data_header[15][3] = "<%=NON_MOVING_DAYS %>";

data_header[16] = new Array;
data_header[16][0] = ""
data_header[16][1] = "hide";
data_header[16][2] = 0;
data_header[16][3] = "<%=EXPIRY %>";
...
data_header[24][0] = "Rol"
data_header[24][1] = "hide";
data_header[24][2] = 0;
data_header[24][3] = "<%=ROL %>";
...
data_header[25][2] = 0;
data_header[25][3] = "kmscl_item_code";

data_header[26] = new Array;
data_header[26][0] = ""
data_header[26][1] = "hide";
data_header[26][2] = 0;
data_header[26][3] = "kmsclCategory";
...
data_arr[<%= counter%>][10] = "<%= masStoreItem.getMinStock() %>"
<%}else{%>
data_arr[<%= counter%>][10] = ""
<%}%>
...
data_arr[<%= counter%>][11] = "<%= masStoreItem.getMaxStock() %>"
<%}else{%>
data_arr[<%= counter%>][11] = ""
<%}%>
...
data_arr[<%= counter%>][12] = "<%= masStoreItem.getLeadTime() %>"
<%}else{%>
data_arr[<%= counter%>][12] = ""
<%}%>
...
data_arr[<%= counter%>][13] = "<%= masStoreItem.getSpecification() %>"
<%}else{%>
data_arr[<%= counter%>][13] = ""
<%}%>

<% if(masStoreItem.getSlowMovingDays() != null){%>
data_arr[<%= counter%>][14] = "<%= masStoreItem.getSlowMovingDays() %>"
<%}else{%>
data_arr[<%= counter%>][14] = ""
<%}%>

<% if(masStoreItem.getFastMovingDays() != null){%>
data_arr[<%= counter%>][15] = "<%= masStoreItem.getFastMovingDays() %>"
<%}else{%>
data_arr[<%= counter%>][15] = ""
<%}%>

<% if(masStoreItem.getNonMovingDays() != null){%>
data_arr[<%= counter%>][16] = "<%= masStoreItem.getNonMovingDays() %>"
<%}else{%>
data_arr[<%= counter%>][16] = ""
<%}%>
...
data_arr[<%= counter%>][17] = "<%= masStoreItem.getExpiry() %>"
<%}else{%>
data_arr[<%= counter%>][17] = ""
<%}%>
...
data_arr[<%= counter%>][18] = "<%= masStoreItem.getVed() %>"
<%}else{%>
data_arr[<%= counter%>][18] = ""
<%}%>
...
data_arr[<%= counter%>][19] = "<%= masStoreItem.getAbc() %>"
<%}else{%>
data_arr[<%= counter%>][19] = ""
<%}%>
...
data_arr[<%= counter%>][25] = "<%= masStoreItem.getRol() %>"
<%}else{%>
data_arr[<%= counter%>][25] = ""
<%}%>
	<% if(masStoreItem.getKmsclItemCode() != null){%>
	data_arr[<%= counter%>][26] = "<%= masStoreItem.getKmsclItemCode() %>"
	<%}else{%>
	data_arr[<%= counter%>][26] = ""
	<%}%>
	
	<% if(masStoreItem.getKmsclCategory() != null){%>
	data_arr[<%= counter%>][27] = "<%= masStoreItem.getKmsclCategory() %>"
	<%}else{%>
	data_arr[<%= counter%>][27] = ""
	<%}%>
<%
