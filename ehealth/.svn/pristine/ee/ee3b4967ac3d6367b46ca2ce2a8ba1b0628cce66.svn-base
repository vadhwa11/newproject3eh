...
<%
		...
		{
%>
			<option value=<%=hesEquipmentBreakdownName.getEquipment().getId()%>><%=hesEquipmentBreakdownName.getEquipment().getEquipmentName()%></option>
<%
		}
%>
</select>
...
<input type="button" name="sss" align="right" class="button" value="Submit" onclick="submitForm('equipmentBreakdownEntryForm','hes?method=submitEquipmentBreakdownEntryJsp');" />
