...
fName=document.getElementById("fileNameId").value;
...
submitForm('sampleCollection','investigation?method=getFileName&browse=browse&sampleCollectionDetailId='+sampleCollectionDetailId+'&hinNo='+hinNo+'&inpatientId='+inpatientId+'&parameterName=fromResultEntry&formName=sampleCollection&test2='+updatedtemplate+'&'+csrfTokenName+'='+csrfTokenValue); 
}
return true;
}
...
<form name="sampleCollection" method="post"  action="" enctype="multipart/form-data">
	<div class="titleBg"> 
		<h2>Result Entry</h2>
	</div>
	<%
...
%>
		...
		if(dgDetails.getSampleCollectionHeader().getOrder().getDepartment()!=null){
			departmentName = dgDetails.getSampleCollectionHeader().getOrder().getDepartment().getDepartmentName();
		}
...
%>
	...
	%>
		...
		<input type="hidden" name="<%=SAMPLE_COLLECTION_DATE%>"
			...
			<!-- <label><span>*</span>Entered By</label> -->
		<%Users user = (Users)session.getAttribute("users");
...
%>
	...
	<div class="cmntable">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<%
				String remark="-";
				if(histoObj.getRemarks()!=null){
					remark=histoObj.getRemarks();
				}%>
				<td><label><%=remark%></td>
	...
	<div class="clear"></div>
	<input type="hidden" id="hinNo" value="<%=hinNo%>">
	<label class="auto">Result</label> <input type="file" name="<%=UPLOAD_FILENAME%>" id="fileNameId" onchange="see();" class="browse" size="50" />
 ...
 %>
	 ...
	 %>
		<textarea id="abc" name="<%=TEMP_RESULT %>"
			class="tableTextareaEditor"><%=new String(Base64.decodeBase64(dgTemp.getResult()))%> </textarea>
 ...
 if(templateSet.size()==0 && !browse.equals("")) {
	 %>
		<textarea value="" id="abc" name="<%=TEMP_RESULT %>"
			class="tableTextareaEditor"> <%=appendedHtml %> </textarea>
		<% }%>
	</div>
	<div class="clear"></div>
...
--%>
	<label><span></span>Film Size Used</label> <input type="text"
		...
		onclick="if(checkFilmUsed()){formSubmission();}" align="right" /> <input
...
function rejectSample(val){
	...
	submitForm('sampleCollection','/hms/hms/investigation?method=rejectSampleHisto&sampleDetailsId='+val+'&sampleHeaderId=<%=sampleCollectionDetailId%>');
...
var itemId=0;
if(undefined !=document.getElementById("filmSizeUsedResponse"))
 itemId=document.getElementById("filmSizeUsedResponse").value;

var filmUsedId=0;
if(undefined !=document.getElementById("filmUsed"))
 filmUsedId=document.getElementById("filmUsed").value;
