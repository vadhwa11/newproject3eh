...
function checkk()
{
var SDate = document.VPDiseases.<%= FROM_DATE%>.value;
var EDate = document.VPDiseases.<%= TO_DATE %>.value;
...
<%
	...
	String currentDateYYYYMMDD = (String)utilMap.get("currentDateYYYYMMDD");
%>
<div class="titleBg">
<h2>Vaccine Preventable Diseases Report</h2>
</div>
<form name="VPDiseases" method="post" action="">
<div class="clear"></div>
<div class="Block">
...
<label>CHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="chc" onclick="submitProtoAjaxWithDivName('VPDiseases','/hms/hms/pubHealth?method=getVPDiseasesChcPhcList&district='+this.value,'tDiv');">
<label>PHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="phc" onclick="submitProtoAjaxWithDivName('VPDiseases','/hms/hms/pubHealth?method=getVPDiseasesChcPhcList&district='+this.value,'tDiv');">
</div>
...
<div class="clear"></div>
 <div id="tDiv">
	<%if(map.get("instituteTypeShortName") != null && map.get("instituteTypeShortName").toString().equalsIgnoreCase("CHC")) {%>
		<label>CHC </label>
    	<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('VPDiseases','/hms/hms/pubHealth?method=getBsFwsList&chcphc='+this.value,'testDiv');">
			<option value="0">Select</option>
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.VPDiseases.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.VPDiseases.<%=TO_DATE%>,event);" />
	<input 	type="hidden" name="<%=CURRENT_DATE%>" value="<%=currentDateYYYYMMDD%>" />
<div class="clear"></div>
<div class="clear"></div>
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('VPDiseases','pubHealth?method=generateVPDiseasesReport','checkk()');"
