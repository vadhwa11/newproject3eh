...
<tr>
 ...
 %> <input name="<%=SUBTEST_ID%>" type="text" size="2" value="" readonly />
	...
	<td>
		...
		size="5" value="<%=dgDetail.getSample().getId()%>" readonly /><label><%=dgDetail.getSample().getSampleDescription()%></label>
 ...
 %>
	...
	<td>
		...
		<input name="<%=RESULT%>"
		id="<%=RESULT%><%=resultEntryIndexForMultiple%>" tabindex="1" onblur=" changeResultValueTextColor(this.value,'<%=minValue%>','<%=maxValue%>','<%=resultEntryIndex + 1%>')"
 ...
 %> 
 <input name="collectionId<%=resultEntryIndexForMultiple%>" value="<%=sampleId %>" type="hidden">
 <input name="<%=RESULT%>" onblur=" changeResultValueTextColor(this.value,'<%=minValue%>','<%=maxValue%>','<%=resultEntryIndex + 1%>')"
 ...
 %><label>-</label> <input name="<%=UNIT_OF_MEASUREMENT_ID%>" type="hidden" size="2"
		value="" readonly /> <%
 	}
 %>
	</td>
 ...
 %> <input name="normalId" type="hidden" id="normalId" size="5"
		...
		value="<%=dgDetail.getNormal().getNormalValue()%>"
		readonly /> <%
