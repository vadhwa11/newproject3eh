...
<input type="hidden" name="<%=JASPER_FILE_NAME%>" value="Opd_instruction_template">
</form>
</div>
</div>
</div>
		...
		if(searchOpdInstructionTreatmentList.size()>0)
		 {
			...
			{
	%><h4> <a href="opdMaster?method=showOpdInstructionTreatmentJsp">Show All Records</a></h4> <%
			}
		 }
...
<label class="auto"><span>*</span> Instruction Treatment Code </label> 
<input id="codeId" type="text" name="<%= CODE%>" value="" validate="Instruction TreatmentCode,string,yes" class="textbox_date" MAXLENGTH="8" / tabindex=1>
<label class="auto"><span>*</span> Instruction Treatment Name</label> 
<input type="text" name="<%= SEARCH_NAME %>" value="" validate="Instruction TreatmentName,string,yes" class="textbox_date" MAXLENGTH="30" / tabindex=1 onkeypress="return submitenter(this,event,'opdMaster?method=addOpdInstructionTreatment')">
<script>				document.opdInstructionTreatment.<%=CODE%>.focus();
			</script>
<div class="clear"></div>
<div id="edited"></div>
...
<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button" onClick="submitForm('opdInstructionTreatment','opdMaster?method=editOpdInstructionTreatment')" accesskey="u" tabindex=1 /> 
<input type="button" name="Delete"	id="deletebutton" value="Activate" style="display: none;"	class="button" onClick="submitForm('opdInstructionTreatment','opdMaster?method=deleteOpdInstructionTreatment&flag='+this.value)" accesskey="d" tabindex=1 /> 
...
data_arr[<%= counter%>][2] = "<%= opdInstructionTreatment.getOpdInstructionTreatmentName()%>"
	 data_arr[<%= counter%>][3] = "<%= opdInstructionTreatment.getLastChgBy()!=null?(opdInstructionTreatment.getLastChgBy().getId()!=null?opdInstructionTreatment.getLastChgBy().getId():"0" ):"0"%>"
