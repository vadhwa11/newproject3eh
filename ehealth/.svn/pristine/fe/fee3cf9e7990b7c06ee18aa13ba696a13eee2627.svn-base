			...
			if(searchCountryList.size()>0)
		 	{
				...
				{
%>
				 	<a href="hes?method=showEmergencyEquipmentBreakdownCallEntry"> Click For Go To Search Page</a> <%
				}
		 	}
...
<input type="text" id="actionTaken" rows="8" name="actionTaken"  validate="Action Taken,string,no"> </input>
<label><span>*</span>Nature of Repairs Carried Out:</label>
<input type="text" id="repair" rows="8" name="repair"  validate="Nature of Repairs,string,no" > </input>
<div class="clear"></div>
<label><span>*</span>Time of Complete Breakdown:</label>
<input type="text" name="<%=DELIVERY_TIME%>" value="<%=currentTime %>"  validate="Call Time,deliveryTime,no"
	MAXLENGTH="15" /> 
<label><span>*</span>Serviced By:</label>
<select name="servicedBy" id="servicedBy" validate="Serviced By,String,no">
...
<input type="button" name="edit" id="editbutton" value="Add" class="button" style="display: none;" onclick="submitForm('equipmentBreakdown','hes?method=addEquipmentBreakdownEntry')" accesskey="a" tabindex=1 />
