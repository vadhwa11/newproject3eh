...
function checkk()
{
var SDate = document.childHealth.<%= FROM_DATE%>.value;
var EDate = document.childHealth.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
<h2>Child Health Immunization Registration Report</h2>
</div>
<form name="childHealth" method="post" action="">
...
<label>CHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="chc" onclick="submitProtoAjaxWithDivName('childHealth','/hms/hms/pubHealth?method=getComDiseaseChcPhcList&district='+this.value,'tDiv');">
<label>PHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="phc" onclick="submitProtoAjaxWithDivName('childHealth','/hms/hms/pubHealth?method=getComDiseaseChcPhcList&district='+this.value,'tDiv');">
</div>
...
<div class="clear"></div>
 <div id="tDiv">
	<%if(map.get("instituteTypeShortName") != null && map.get("instituteTypeShortName").toString().equalsIgnoreCase("CHC")) {%>
		<label>CHC </label>
    	<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('childHealth','/hms/hms/pubHealth?method=getBsFwsList&chcphc='+this.value,'testDiv');">
			<option value="0">Select</option>
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.childHealth.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.childHealth.<%=TO_DATE%>,event);" />
<div class="clear"></div>
<div class="clear"></div>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('childHealth','pubHealth?method=generateChildHealthImmunRegisterReport','checkk()');"
