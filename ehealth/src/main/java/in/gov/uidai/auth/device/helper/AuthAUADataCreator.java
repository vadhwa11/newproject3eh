...
Pid p=(in.gov.uidai.authentication.uid_auth_request_data._1.Pid) pid;
SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
String timeStampStr = dateFormat.format(new Date()).replace(" ", "T");
//String xmlPid=createPidXML((in.gov.uidai.authentication.uid_auth_request_data._1.Pid) pid);
//byte[] encXMLPIDData = this.encrypter.encryptUsingSessionKey(sessionKey, pidXmlBytes);
byte[] encXMLPIDData = this.encrypter.encryptUsingSessionKey(sessionKey, pidXmlBytes,timeStampStr);
String key = "Bar12345Bar12345"; // 128 bit key
String initVector = "RandomInitVector"; // 16 bytes IV
//byte[] encXMLPIDData = this.encrypter.encrypt(key,initVector, encXMLPIDData);
			byte[] hmac = this.hashGenerator.generateSha256Hash(pidXmlBytes);
			byte[] encryptedHmacBytes = this.encrypter.encryptUsingSessionKeyNoAdd(sessionKey, hmac,timeStampStr);
	...
	private String createPidXML(Pid pid) {
		...
		String pidStr=pidXML.toString();
		if(pidStr.contains("xmlns:ns2=\"http://www.uidai.gov.in/authentication/uid-auth-request-data/1.0\"")) {			
			pidStr = pidStr.replace("xmlns:ns2=\"http://www.uidai.gov.in/authentication/uid-auth-request-data/1.0\"",  "");			
		}
		if(pidStr.contains("ns2:")) {			
			pidStr = pidStr.replace("ns2:",  "");			
		}
