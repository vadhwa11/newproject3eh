...
public class OrganismComparator implements Serializable,
	...
	public int compare(DgResultEntryDetailSen m1, DgResultEntryDetailSen m2) {
		...
		// Integer organism1 = m1.getOrganism().getId();
		// Integer organism2 = m2.getOrganism().getId();

		if (organismGroup1 == organismGroup2) {
			Integer organism1 = m1.getOrganism().getId();
			Integer organism2 = m2.getOrganism().getId();
			if (organism1 == organism2) {
				Integer sensitivity1 = m1.getSensitivity().getId();
				Integer sensitivity2 = m2.getSensitivity().getId();
				return sensitivity1.compareTo(sensitivity2);
			} else {
	...
	public static TreeSet<DgResultEntryDetailSen> getApplicationTreeSet() {
		return new TreeSet<DgResultEntryDetailSen>(new OrganismComparator());
	}
}