...
public interface LeaveDetailsHandlerService {
	...
	List getWaitingLeavesList(MasEmployee user,int hospitalId);
	...
	void approveLeaves(MasEmployee employee, String remarks, Box box);
	...
	void disapproveLeaves(MasEmployee user,
			String disapproveMessage,Box box);

	List getEmpNamesList(int userId);

	void sendSuggestion(MasEmployee employee, String suggestionMessage,
			Box box);

	int getRH(List<HrLeaveDetails> leaveList, String first_half_start,
			String first_half_end) throws IOException, ParseException;
