...
public abstract class AModemDriver
{
			...
			while (true)
			{
				...
				while (true)
				{
					...
					if (System.getProperty("smslib.dumpqueues") != null) 
						//getGateway().getService().getLogger().logDebug("OUT READER QUEUE : " + (int) c + " / " + (char) c, null, getGateway().getGatewayId());
	...
	protected boolean waitForNetworkRegistration() throws GatewayException, TimeoutException, IOException, InterruptedException
	{
		...
		while (true)
		{
			...
			switch (answer)
			{
				...
				case 1:
					//getGateway().getService().getLogger().logDebug("GSM: Registered to home network.", null, getGateway().getGatewayId());
					return true;
				case 2:
	...
	private class CharQueue
	{
		...
		public synchronized void put(byte c)
		{
			...
			if (System.getProperty("smslib.dumpqueues") != null) 
				//getGateway().getService().getLogger().logDebug("IN READER QUEUE : " + (int) c + " / " + (char) c, null, getGateway().getGatewayId());
			notifyAll();
		}
		...
		public synchronized byte get() throws TimeoutException, InterruptedException
		{
			...
			while (true)
			{
				...
				catch (InterruptedException e)
				{
					...
					{}//getGateway().getService().getLogger().logWarn("Ignoring InterruptedException in Queue.get().", null, getGateway().getGatewayId());
					else
					{
		...
		public synchronized byte peek() throws InterruptedException
		{
			...
			while (true)
			{
				...
				{
					if (getGateway().getStatus() == GatewayStatuses.STARTED) 
					{}//getGateway().getService().getLogger().logWarn("Ignoring InterruptedException in Queue.peek().", e, getGateway().getGatewayId());
					else
					{
