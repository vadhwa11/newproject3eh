...
%>
<div class="clear"></div>
<label>IPD No.</label> <label class="value"><%=inpatient.getAdNo().toString()%></label>
<%}else{ %> <label class="value">-</label> <%}%> <label>Reg No.</label> <label
	class="value"><%=inpatient.getHin().getHinNo() %></label>
	...
	<%
		...
		patientFirstName=inpatient.getHin().getPFirstName() ;
		if(inpatient.getHin().getPMiddleName()!=null){
		patientMiddleName= inpatient.getHin().getPMiddleName();}
		if(inpatient.getHin().getPLastName()!=null){
		patientLastName= inpatient.getHin().getPLastName();}
...
<input id="clinicalNote" type="text" name="<%= CLINICAL_NOTE%>" value="" validate="Clinical Notes,string,yes"
	MAXLENGTH="100" tabindex=1 style="width: 600px;" />

<div class="clear"></div>
<label><span>*</span> Advised By</label> <select id="placedBy"
	name="<%=EMPLOYEE_ID %>" validate="Advised By,string,yes" tabindex=1>
...
<!-- <label>Common</label> -->
<input type="hidden" id="rareCommon1"	name="<%=RARE_COMMON  %>" class="radioCheck" value="c"	checked="checked" />
<!--<label class="auto">Rare</label> -->
<input	type="hidden" id="rareCommon2" name="<%=RARE_COMMON  %>"	class="radioCheck" value="r" />
<label>Routine</label>
<input type="radio" id="routine1" name="<%=ROUTINE%>" class="radioCheck" value="r" checked="checked" />
<label class="auto">Urgent</label>
	 <input type="radio" id="routine2" name="<%=ROUTINE%>"	class="radioCheck" value="u" onclick="showUrgentDetails(this.value);" />
	<input type="text" name="<%=URGENT_DETAILS%>" id="urgentDetails" style="display: none;" onchange="showUrgentRemarks();" />
<div class="clear"></div>
</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	...
	<tr>
		...
		<td>
			<input type="text" id="amount<%=inc%>" value="" name="<%=AMOUNT%><%=inc%>" 
			onblur="totalCost();" class="readonly" readonly="readonly" />
		</td>
		<td>
			<input type="text" value="" id="netamount<%=inc%>" value="" name="<%=NET_AMOUNT %><%=inc%>" 
...
<div class="clear"></div>
<div class="paddFltRight110"><label class="medium"><a>Total Cost</a></label>
<input type="text" id="totalCostId" name="<%=TOTAL_AMOUNT%>" value="0" class="auto"
 class="readOnly" readOnly />
<input type="hidden" id="outstandingId" name="<%=OUTSTANDING%>" value="" class="readOnlySmall" readonly="readonly" />
<input type="hidden" id="totalNetId" name="<%=TOTAL_NET_AMOUNT%>" value="" class="readOnlySmall" readOnly />
</div>
<div class="clear"></div>
...
<input type="button" class="button" value="Submit"
	...
	align="right" />
<input type="reset" class="buttonHighlight"	name="Reset" id="reset" value="Reset"
	onclick="resetClicked('orderBooking');" accesskey="r" />
...
if(document.getElementById('qty'))
	...
	function totalCost()
	{
		...
		document.getElementById('outstandingId').value = amt;
		document.getElementById('totalNetId').value = amt;
	}
...
function addRow(){
	...
	e6.type = 'text';
	e6.name='<%=NET_AMOUNT %>'+ (iteration);
	e6.readOnly = true;
	e6.id='netamount'+(iteration);
...
function showUrgentRemarks(){
	if(document.getElementById('routine2').value='u'){
		if(document.getElementById('urgentDetails').value ==""){
			alert("Please Enter Reason of urgency.");
			return false;
		}
	}
		return true;
