...
<title>Re Tender - SO For L1</title>

<script>

function onChangeProposal()
{
	if (ReTenderLPOForm.<%=PROPOSAL_ID%>.value!="" && ReTenderLPOForm.<%=TENDER_NO%>.value!="" && ReTenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value!="")
	{
...
ReTenderLPOForm.method="post";
submitForm('ReTenderLPOForm','tender?method=getReTenderGroupListForLPO&tenderId='+ReTenderLPOForm.<%=TENDER_NO%>.value);
}
...
<h2 align="left" class="style1">Re Tender - SO For L1</h2>
</div>
...
<font class="boxtitle">Re Tender - SO For L1</font></div>
