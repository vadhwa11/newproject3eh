...
<script>
	...
	if (document.TenderComPriorPNCForm.<%=TENDER_NO%>.value=="" || document.TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID %>.value=="")
	{
	alert('Pl. Check Your Inputs (Tender No & Group)');
	return;
	}
	
	document.TenderComPriorPNCForm.method="post";
	submitForm('TenderComPriorPNCForm','tender?method=printReportChartPriorPNCJsp');
	}

	function onChangeTender()
	{
	//document.TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.options.length=1;
	document.TenderComPriorPNCForm.method="post";
	submitForm('TenderComPriorPNCForm','tender?method=showChartPriorPNCJsp&groupId='+document.TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
	}
</script>
<div class="titleBg">
<h2>Tender Comparative Prior PNC Data</h2>
</div>
<form name="TenderComPriorPNCForm">
...
<div class="Clear"></div>
<h4>Tender Comp. Prior PNC</h4>
<div class="Block">
<label>Group</label> 
<select	name="<%=TENDER_SUPPLIER_GROUP_ID %>" onChange="onChangeTender();">
...
<div class="clear"></div>
<input type="button" name="softcopy" onClick="printReport()" value="Submit" class="button">
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
