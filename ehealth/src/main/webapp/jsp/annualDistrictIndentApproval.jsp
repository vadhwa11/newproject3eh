...
%>
<form name="annualDistrictIndentApproval" method="post">
<div class="titleBg">
<h2>District Indent Approval</h2>
</div>
...
<%--<label>Demand No.</label> <input type="text"
	...
	<%if(indentDate != null){ %>
	 <input type="text" name="<%=DEMAND_DATE %>" value="<%=HMSUtil.convertDateToStringWithoutTime(indentDate) %>" readonly="readonly"   />
	 <%}else{ %>
...
<select name="districtId" id="districtId"	>
	...
	<%if(districtList.size()>0){
		for(MasDistrict masDistrict:districtList){
			if(masDistrict.getId().equals(districtId)){
		%>
...
<table border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getPrevYearIndQty()!= null?storeInternalIndentT.getPrevYearIndQty().intValue():"" %>" name="year1IndentQty" id="year1IndentQtyId<%=i %>" readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getPrevToPrevYearIndQty()!= null?storeInternalIndentT.getPrevToPrevYearIndQty().intValue():"" %>" name="year2IndentQty" id="year2IndentQtyId<%=i %>"  readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getPrevYearConsmQty()!= null?storeInternalIndentT.getPrevYearConsmQty().intValue():"" %>" name="previousYearConsumption"  id="previousYearConsumptionId<%=i %>"  readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getStockInHand()!= null?storeInternalIndentT.getStockInHand().intValue():"" %>" name="stock" validate="Stock In Hand,num,no" id="<%=stock+i %>"  readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getLeadTime()!= null?storeInternalIndentT.getLeadTime().intValue():"" %>" name="leadTime" validate="Lead Time,num,no" id="leadTimeId<%=i %>"   /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getConsInLeadTime()!= null?storeInternalIndentT.getConsInLeadTime().intValue():"" %>" name="consumptionInLeadTime" validate="Consumption In Lead Time,num,no" id="consumptionInLeadTimeId<%=i%>" readonly="readonly"   /></td>
						...
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getIncrementalQuantity()!= null?storeInternalIndentT.getIncrementalQuantity().intValue():"" %>" name="incrementalQty" id="incrementalQtyId<%=i %>" readonly="readonly"/></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getPendingIndentQty()!= null?storeInternalIndentT.getPendingIndentQty().intValue():"" %>" name="pendingIndentQty" id="pendingIndentQtyId<%=i %>" validate="Pending Indent Qty,num,no"  readonly="readonly"/></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getRequiredQty()!= null?storeInternalIndentT.getRequiredQty().intValue():"" %>" name="requiredQty" id="requiredQtyId<%=i %>" validate="Requested Qty,num,no" readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getAdditionalQty()!= null?storeInternalIndentT.getAdditionalQty().intValue():"" %>" name="additionalQty" validate="Stock In Hand,num,no" id="additionalQtyId<%=i %>"  readonly="readonly" /></td>
						<td><input type="text" size="8" value="<%=storeInternalIndentT.getQtyRequest()!= null?storeInternalIndentT.getQtyRequest().intValue():"" %>" name="qtyRequest" id="<%=qtyRequest+i %>" validate="Demanded Qty,num,no"  readonly="readonly"/></td>
						<td><input type="text" size="10" value="<%=storeInternalIndentT.getRemarks()!= null?storeInternalIndentT.getRemarks():"" %>" name="remarks" id="remarks<%=i %>" validate="Remarks,string,no" readonly="readonly"/>
...
<textarea name="dmoRemarks"   validate="Remarks,string,no" cols="0" rows="0" class="large" maxlength="198" tabindex="1" ></textarea>
...
<div class="clear"></div>
<input type="button" name="Next/Update" type="submit" value="Approve"	onClick="submitForm('annualDistrictIndentApproval','stores?method=submitAnnualDistrictApproval','validateRows');" class="button">
<input type="button" name="Next/Update" type="submit" value="SendBack"	onClick="submitForm('annualDistrictIndentApproval','stores?method=submitAnnualDepartmentApproval');" class="button">
