...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"><script
	type="text/javascript" language="javascript">
 	...
 	appDate=HMSUtil.convertStringTypeDateToDateType(currentDate);
...
<div class="titleBg">
<h2> Patient Appointments(VISIT)</h2>
</div>
<div class="clear"></div>
...
<table border="0" cellspacing="0" cellpadding="0">
 ...
 %>
		...
		<input type="text" id="appointmentDate<%=counter%>"	name="apptDate"	value="<%=HMSUtil.changeDateToddMMyyyy(appPatientAppointments.getAppointmentDate())%>"	class="readOnly" readonly="readonly" />
		...
		<td><input type="text" id="appointmentDate<%=counter%>"
			...
			onclick="submitForm('patientSearch','/hms/hms/registration?method=showVisitDetailsJsp&<%=HIN_NO%>=<%=appPatientAppointments.getHin().getHinNo()%>');" /></td>
	</tr>
	<%
		} }
	%>
