...
<%
	...
	String sameHosp="";
	if(map.get("sameHosp") !=null)
	{
		sameHosp =(String)map.get("sameHosp");
	}
	%>
...
<h4><%=message %></h4>
<div class="clear"></div>
<input type="button" value="Back" class="button" onClick="submitForm('messageForm','/hms/hms/registration?method=showOnlineAppointmentJsp')" />
<input type="hidden" id="hinId" name="hinId"	value="<%=map.get("phinId") %>" />
...
<% if(uhidStatus1 && flag.equals("appointment") && recordExists==false ){%> 
<input type="button" name="Print"
	value="Generate Slip" target="_blank" class="buttonBig"
	onclick="submitFormForDirectPrint('messageForm','/hms/hms/appointment?method=printAppointmentSlip&sameHosp=<%=sameHosp%>>')"/><!--// added by govind on 11-01-2017 -->
