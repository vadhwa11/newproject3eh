...
<form name="opPackageBooking" method="post" action=""
	onload="form.reset();"><script type="text/javascript">
...
<label>Patient Name</label> <%
 ...
 %> <label class="value"><%=patient.getPFirstName() %> <%=patient.getPMiddleName()!=null ? patient.getPMiddleName():""%> <%=patient.getPLastName()!=null ?patient.getPLastName():""%>
</label> <%
 	}
 %> 
 <div class="clear"></div>
...
<label><span>*</span> Consultant Name</label> <select
 ...
 %> <input type="hidden" name="<%=HIN_ID %>"
	...
	value="<%=patient.getPFirstName() %> <%=patient.getPMiddleName()!=null ? patient.getPMiddleName():""%> <%=patient.getPLastName()!=null ?patient.getPLastName():""%>" />
<input type="hidden" name="<%=AGE%>" value="<%=patient.getAge() %>" />
<input type="hidden" name="<%=SEX_ID%>"
	value="<%=patient.getSex().getId() %>" /> <input type="hidden"
	name="<%=INPATIENT_ID %>" value="<%=inpatient.getId() %>" /> 
	
	<%-- <input type="hidden"
	name="chargeSlipNo" value="<%=inpatient.g %>" /> --%>
	<%} 
	%> <input type="hidden" id="cnsltDocTextId"
	name="<%=CONSULTING_DOCTOR%>" value="" /> <label>Package</label> <select
	name="<%=PACKAGE_ID %>"
...
<div class="clear"></div>
<div class="Block"><label class="auto">Package Amount</label> <input
	type="text" id="pkgAmtId" name="<%=DISCOUNTED_VALUE_PACKAGE %>"
	class="small" value="" readOnly />
...
%> <label class="auto">Advance Adjustment</label> <input type="checkbox"
 ...
 %> <label class="auto">Outstanding</label> <input type="text"
	...
	onblur="calculateNetAmountAfterAdvOS('<%=adv %>');" maxlength="9" /> <label
	class="auto">Round Off</label> <input type="text" id="roundId"
...
function addRow(){
	...
	e2.readOnly = true;
	e2.onblur= function(){
					if(validateQtyForBilling(this.value,this.id)){calculateNetAmount(iteration);calculateTotalAmt();};
					};
...
function displayPkgAmt(val)
{
	...
	for(i=0;i<amtArr.length;i++){
		if(val == amtArr[i][0]){
			...
			document.getElementById('amtRcvd1').value = Math.round(amtArr[i][1]).toFixed(2);
