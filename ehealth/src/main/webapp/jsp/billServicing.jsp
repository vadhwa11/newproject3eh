...
<%--  <label>Family Income Status</label> 
 <input type="text"  readonly="readonly" value="<%=patient.getBplStatus()!=null:patient.getBplStatus():"" %>" /> --%>
 <%-- <%
 String familyId="";
 ...
 /* for(Patient pa:patientList){
	 emp=pa.getEmp().getEmployeeName();
 } */
 %>
<%--  <label>Consultant Name</label><input	type="text" readonly="readonly" value="<%=orderHdList.get(0).getVisit().getDoctor().getFirstName()%>"/> --%>
<%
	if (!orderNo.equals("")) {
%>
...
<!-- added scheme authorizer by Amit Das on 15-02-2016 -->
<!-- Commented By Ujjwal with discussion with girjesh Sir -->
...
<input type="text"  readonly="readonly" value="<%=adv%>" /> 
 ...
 <%--<input type="hidden"  readonly="readonly" value="<%=patient.getPatientType().getPatientTypeName() %>" />
  <input type="hidden" value="<%=patient.getPatientType().getId()%>" name="patientTypeId2" id="patientTypeId2" /> --%>
 <%-- <% if(patient.getPatientType().getId()== 3){ %>
...
<label>Project</label> <%} %><input	type="text" readonly="readonly" value="<%=patient.getCompany().getCompanyName() %>"/>

<input type="hidden" id="companyId" name="companyId" value="<%=patient.getCompany().getId() %>" />

<input type="hidden" id="OpStatus" name="OpStatus" value="<%=patient.getCompany().getOpOnAcc()%>" />
<%} else if(patient.getPatientType().getId() == 3){ %>
...
<input type="hidden" id="patientTypeCode" name="patientTypeCode"
								value="<%=patient.getPatientType().getPatientTypeCode()%>" />  --%>
...
<!-- 		<th scope="col" style="display:none;">Std. Deduction</th>
 -->		<th scope="col">Amount</th>
	...
	<!-- 	<th scope="col"style="">Prapotioan</th> -->
...
<%
    ...
    Date currentDate = new Date();
		...
		int orderHdId = 0;
		System.out.println("orderdtList.size()---"+orderdtList.size());
					...
					orderHdId = orderDt.getOrderhd().getId();
					 Set<MasChargeCodeRates> chargeSet = new HashSet<MasChargeCodeRates>();
						if(orderDt.getChargeCode().getMasChargeCodeRates() != null){
							...
							System.out.println(orderDt.getChargeCode().getChargeCodeName());
...
%>
	...
	%>
		<%
			if(orderDt.getChargeCode().getDiscountable().equalsIgnoreCase("y") ){
		%>
		<script type="text/javascript">
	...
	%>
	<input type="hidden" name="tempBillHdId" value="<%=tempOpBillHeader.getId() %>" />
	...
	<tr>
		...
		<td><input type="hidden" name="tempBillDetailsId<%=inc %>" value="<%=tempBlDetails.getId() %>" />
		<input type="radio"	value="<%=inc%>" name="selectedChrage" class="radioCheck" /></td>

		<td><input type="text" name="chargeCode<%=inc%>" id="chargeCode<%=inc%>"
			value="<%=tempBlDetails.getChargeCode().getChargeCodeName() %>[<%=tempBlDetails.getChargeCode().getChargeCodeCode() %>]"
			onblur="if(validateChargeCodeForBillingAutoComplete(this.value, '<%=inc %>','op')){submitProtoAjaxWithDivNameForBilling('billServicing','/hms/hms/opBilling?method=fillChargeCode&charge1='+encodeURIComponent(this.value)+'&rowVal=<%=inc %>&hin=<%=hin%>&type=op&schemeList='+document.getElementById('schemeList').value,'rateVal');openPopupForItem(this.value,<%=inc %>);}"
			readonly="readonly" tabindex="1" />
		...
		</script> <input type="hidden" id="chargeId<%=inc %>" name="<%=CHARGE_CODE_ID%><%=inc %>"
			value="<%=tempBlDetails.getChargeCode().getId()%>" /></td>

		<td><input type="text" size="5" id="qty<%=inc%>" name="<%=QUANTITY %><%=inc%>"
			value="<%=tempBlDetails.getQuantity() %>" validate="Qty,int,no"
			onblur="if(validateQtyForBilling(this.value,this.id,<%=inc %>)){calculateNetAmount(<%=inc %>);calculateTotalAmt();};"
		...
		<td><input type="text" size="12" value="<%=tempBlDetails.getChargeCode().getStdDeductionGen()%>"
			id="standardDeductionId<%=inc%>" name="<%=STANDARD_DEDUCTION%><%=inc%>"
			validate="Standard Deduction,float,no" readonly="readonly" /></td>

		<%}else if(registrationType.equals("S")){ %>
		<td><input type="text" size="12" value="<%=tempBlDetails.getChargeCode().getStdDeductionSpl()%>"
		...
		%> <input type="hidden" value="<%=prptDiscAmt %>" id="prprtnlDis<%=inc%>" 
			name="<%=PROPORTIONAL_DISCOUNT%><%=inc%>" validate="Proporational Discount,string,no" 
			readonly="readonly"	size="10" /></td>
		<td><input type="text" value="<%=tempBlDetails.getNetAmt() %>"
			id="netamount<%=inc%>" name="<%=NET_AMOUNT %><%=inc%>"
			validate="Net Amount,float,no" readonly="readonly" size="10" /></td>
		<td><input type="checkbox" value="" id="lalPathId<%=inc%>"	name="lalPath<%=inc%>"
			readonly="readonly" size="10" onclick="assignYesNoLalPath(<%=inc%>);" />
			<input type="hidden" value="n" id="lalPathText<%=inc%>"	name="lalPathText<%=inc%>"
			readonly="readonly" size="10"  />
		 <input type="hidden" value="" name="companyId2<%=inc%>" id="companyId2<%=inc%>" />
		  <input type="hidden" value="" name="billTypeId<%=inc%>" id="billTypeId<%=inc%>" />
		...
		<td><input type="button" name="add" value="" class="buttonAdd"
			onclick="addRow();" onblur="addRow();" tabindex="1" />
		...
		<input type="hidden" name="<%=FA_ACCOUNT_ID%><%=inc %>"
			value="<%=tempBlDetails.getChargeCode().getAccount().getId()%>" /></td>
		<%
int subLdId =0 ;
...
%>
		<input type="hidden" name="<%=FA_SUB_LED_ID%><%=inc %>"	value="<%=subLdId%>" />
	...
	<tr>
		...
		<td><input type="text" name="chargeCode<%=inc%>" id="chargeCode<%=inc%>"
			onblur="if(validateChargeCodeForBillingAutoComplete(this.value, '<%=inc %>','op')){submitProtoAjaxWithDivNameForBilling('billServicing','/hms/hms/opBilling?method=fillChargeCode&charge<%=inc%>='+encodeURIComponent(this.value)+'&rowVal=<%=inc%>&hin=<%=hin%>&type=op','rateVal');}"
			tabindex="1" />
		<div id="ac2update<%=inc%>" style="display: none;" class="autocomplete"></div>
		<script type="text/javascript" language="javascript" charset="utf-8">
	   function eventCallback(element, entry){
		   //alert(document.getElementById('companyId'))
		   var comanyId='';
		   if(document.getElementById('companyId') !=null)
			 {
			...
			return entry + "&mainChargeCodeId=" + document.getElementById('mainChargeCodeId').value+"&subChargeCodeId="+document.getElementById('subChargeCodeId').value+"&companyId="+comanyId+"&blStatus="+document.getElementById('checkId').value;
		}
	    new Ajax.Autocompleter('chargeCode<%=inc%>','ac2update<%=inc%>','opBilling?method=getChargeCodeForAutoComplete',{parameters:'requiredField=chargeCode<%=inc%>&'+csrfTokenName+'='+csrfTokenValue, callback: eventCallback});

			</script></td>
		<td><input type="text" size="5" id="qty<%=inc%>" name="<%=QUANTITY %><%=inc%>" value="" validate="Qty,int,no"
			onblur="if(validateQtyForBilling(this.value,this.id,<%=inc %>)){calculateNetAmount(<%=inc %>);calculateTotalAmt();}"
			maxlength="5" readonly="readonly" /></td>
		<td id="rateVal"><input type="text" size="12" value=""	id="resrate<%=inc%>" name="<%=RATE%><%=inc%>"
		...
		<%}else{ %>
		<td style="display:none;"><input type="text" value="" id="standardDeductionId<%=inc%>" name="<%=STANDARD_DEDUCTION%><%=inc%>" 
			validate="Standard Deduction,float,no" readonly="readonly"  size="12" /></td>
		<%} %>
		
		<td><input type="text" value="" id="amount<%=inc%>" size="13"
			name="<%=AMOUNT%><%=inc%>" validate="Amount,float,no" readonly="readonly" /></td>
		...
		<td><input type="text" value="" id="disamount<%=inc%>" name="<%=DISCOUNT %><%=inc%>" 
			validate="Discount Amount,string,no" maxlength="7" disabled="disabled" size="10" 
			onchange="if(checkDiscountAmt(<%=inc %>)){validateDiscAmt(this.value,<%=inc %>);disableDiscountPercent(<%=inc%>);calculateNetAmount(<%=inc %>);calculateTotalAmt()};"/></td>
		...
		<td><input type="text" value="" id="netamount<%=inc%>" name="<%=NET_AMOUNT %><%=inc%>" 
			validate="Net Amount,string,no"	readonly="readonly" size="10" /></td>
			
		<td style="display:none;"><input type="checkbox" value="" id="lalPathId<%=inc%>"	name="lalPath<%=inc%>"
			readonly="readonly" size="10" onclick="assignYesNoLalPath(<%=inc%>);" disabled="disabled"/>
			<input type="hidden" value="n" id="lalPathText<%=inc%>"	name="lalPathText<%=inc%>"
			readonly="readonly" size="10"  />
		 <input type="hidden" value="" name="companyId2<%=inc%>" id="companyId2<%=inc%>" />
		  <input type="hidden" value="" name="billTypeId<%=inc%>" id="billTypeId<%=inc%>" />
...
%>
<input type="text" id="compDiscountId" name="compDiscount" value="<%=compDisc %>" class="readOnly" readonly="readonly"  /> 
...
%> <input type="hidden" id="charityId" name="charity"
	class="readOnly" readonly="readonly" value="<%=charity %>" /> <%
...
<%-- <input type="text" id="discountOnBillId"
	name="<%=DISCOUNT_ON_BILL %>" class="auto3" value="0.00"
	onblur="calculateDiscAmtForBill(this.value,'op');"
	validate="Discount On Bill,float,no" maxlength="5" /> <label
	class="smallAuto">(%)</label>
	<label>Discount On Total Bill</label> <input
	type="text" id="discountAmtBillId" name="discountAmtBillId" value="0.00"
	class="" onblur="calculateDiscPercentForOpService(this.value,'op');"
	validate="Bill Charity Amount,float,no" maxlength="5" />
	<label
	class="smallAuto">(Rs)</label> --%>
...
<label>Round Off</label> <%
	...
	value="<%= roundOff%>" class="readOnly" readonly="readonly" />
	 <!-- <label>Net Amount</label>  --><%
	if(tempOpBillHeader.getNetAmt() != null){
		totalNetAmt = tempOpBillHeader.getNetAmt();
	}
	%>
<%-- <input type="hidden" id="totalNetId" name="<%=TOTAL_AMOUNT%>" value="<%=totalNetAmt %>" class="readOnly" readonly />
<input type="hidden" id="totalNetId1" name="<%=TOTAL_AMOUNT%>" value="<%=totalNetAmt %>" class="readOnly" readonly /> --%>
...
<%-- <label>Authorizer</label>
<td><select name="authorizerId" id="authorizerId" >
	<option value="0">Select</option>
	<%	for (MasAuthorizer authorizer : authorizerList) {	%>
	<option value="<%=authorizer.getId() %>"><%=authorizer.getAuthorizerName()%></option>
	<%}%>
...
<label><input type="checkbox" id="advAdjCheckId" name="" class="radiobutMargin" value="" onclick="displayOPAdvance(this);" /> 
...
<label>Advance Adjustment</label> <input type="checkbox"
	...
	%> 
	<input type="text" id="advAdjId" name="<%=ADVANCE_ADJUSTMENT %>" value="<%=advAdjAmt %>" class="small" 
...
<label class="heightAuto"><input type="checkbox" id="charitYId" value="" onclick="transferCharity(this);" class="radiobutMargin" /> Refund Transfer To Charity</label>
