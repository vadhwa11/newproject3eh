...
<%
	...
	List<MasDeliveryType> deliveryType = new ArrayList<MasDeliveryType>();
	if(map.get("deliveryTypeList") != null) {
		deliveryType=(List<MasDeliveryType>)map.get("deliveryTypeList");
	}
...
%>
<div class="titleBg">
<h2>Birth Registration Report</h2>
</div>
...
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.birth.<%=TO_DATE%>,event);" />
<div class="clear"></div>
<label>Gender </label>
...
<select name="deliveryType" id="deliveryType" class="">
	...
	<% if(deliveryType.size() > 0) { %>
		...
		<%
		for(MasDeliveryType delivery : deliveryType) {
		%>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('birth','pubHealth?method=generateBirthRegisterReport','checkk()');"
