...
if(map.get("bloodGroupList") != null)
{
	bloodResultEntryHeaderId=(Integer)map.get("bloodResultEntryHeaderId");
}
...
int bldSamplescreenHId=0;
  if(null !=screeningList && screeningList.size()>0){
	  for(BloodResultEntryHeader bsh:screeningList){
		 ...
		 //Quantity=bsh.getScreeningTest().getSampleCollection().getQuantityCollected().toPlainString();
		  if(null !=bsh.getScreeningTest().getSampleCollection().getComponentQuantity())
				 Quantity=bsh.getScreeningTest().getSampleCollection().getComponentQuantity();
		  }
	  }
  }
...
<input id="serialNoId" type=hidden name="<%=SERIAL_NO%>"
	value="<%=serialSeqNo %>" title="Serial Number" /> 
	<label class="value"> <%=serialSeqNo %> </label>  --%>
	
	<input type="hidden" name="bloodResultEntryHeaderId" value="<%=bloodResultEntryHeaderId %>"/>
	 <label>Bag Number</label> 
	 <input	type="text"  name="BagNumber" value="<%=BagNum %>" readonly="readonly" tabindex=1 maxlength="20" 
	 ...
	 <input	type="text"  name="TubeNumber" value="<%=TubNum %>" tabindex=1
	  maxlength="20"  readonly="readonly" onblur="ajaxFunctionForTestPatient(testEntry);" /> 
...
<input type="hidden" size="2" value="" name="noOfRecords" id="noOfRecords" /> <input type="hidden" name="pageNo" id="pageNo" value="" />
...
<label>Rh Type<span>*</span></label> 
   
	<select id="pType" name="cgRh"
	validate="Rh Type,string,yes">
	<option value="0">Select</option>
	<option value="+">+ve</option>
	<option value="-">-ve</option>
	<option value="weak +ve">Weak +ve</option>
</select> 
 
 
<label>Remarks</label> 
 <textarea name="cgRemark"rows="4" cols="50" class="textareaMediua"></textarea> 
  </div>
 </div>  
  </div>
...
<input type="button" class="button" value="Submit"
	onclick="submitForm('testEntry','bloodBank?method=submitBloodTestEntryCg');"
	align="right" /> 
