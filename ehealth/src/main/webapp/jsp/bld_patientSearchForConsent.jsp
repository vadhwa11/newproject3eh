...
<input type="text" name="<%=HIN_NO %>" id="hinNo" value="" maxlength="20" />
...
<script type="text/javascript">
 ...
 <input type="button" name="Submit" id="addbutton" value="Search" class="button" accesskey="a"
	onclick="if(checkBlank()){submitForm('search','/hms/hms/bloodBank?method=showBloodConsentDetails');}"/>
...
<div class="clear"></div>
<form name="bloodConsent" method="post" action="">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
<div id="searchtable" tabindex=2></div>
 ...
 <script type="text/javascript">
	formFields = [
			[0, "<%= HIN_ID%>", "id"], [1,"hin"], [2,"patName"], [3,"gender"], [4,"inpatientId"]];
	</script>
</div></form>
<script type="text/javascript" language="javascript">
	...
	if (patientList != null && patientList.size() > 0) { %>
		...
		<%if(!(checkList.contains(bldEntry.getHin().getHinNo()))){%>
			...
			<%
			counter++;
			checkList.add(bldEntry.getHin().getHinNo());
		}else{
		}
	    }
	}
