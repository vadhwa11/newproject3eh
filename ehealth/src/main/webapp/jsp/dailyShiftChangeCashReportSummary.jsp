...
<div class="titleBg">
<h2>Daily Shift Change Cash Report(Summary)</h2></div>
<form name="report" method="post" action="">

<div class="clear"></div>
	<div class="Block">
	<label><span>*</span>User Name</label>
	<select id="user" name="userName" validate="User Name,string,yes">
	...
	<input type="text" name="<%= FROM_DATE%>" value="<%=date%>" validate="From Date,string,yes" class="date" MAXLENGTH="30"  readonly="readonly" id="fromDate"/>
	...
	<input type="text" name="<%= TO_DATE%>" value="<%=date%>" validate="To Date,string,yes" class="date" MAXLENGTH="30"  readonly="readonly" id="toDate"/>
		<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onclick="javascript:setdate('<%=FROM_DATE%>',document.report.<%=TO_DATE%>,true);" />
	...
	<div class="clear"></div>
		...
		<div class="clear"></div>
<%--- Report Button   --%>
<input type="button" name="Report" value="Generate Report" class="buttonBig" onclick="if(check()){submitForm('report','billing?method=printDailyShiftChangeCashReportSummary')};" accesskey="g" tabindex=1/>
