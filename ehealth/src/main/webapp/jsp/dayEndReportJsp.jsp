...
<label><span>*</span> From Date</label> <input
	...
	validate="From Date,string,yes" class="date" MAXLENGTH="30"
	readonly="readonly" id="fromDate"/> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onclick="javascript:setdate('<%=FROM_DATE%>',document.report.<%=FROM_DATE%>,true);" />

<label><span>*</span> To Date</label> <input type="text"
	name="<%= TO_DATE%>" value="<%= date%>" validate="To Date,string,yes"
	class="date" MAXLENGTH="30" readonly="readonly" id="toDate" /> <img
...
<label><span>*</span> From Time</label> <input type="text"
	name="<%= FROM_TIME%>" value="" class="date"
	validate="From Time,string,yes"
	onkeyup="mask(this.value,this,'2,5',':');" MAXLENGTH="8" id="fromTime"/> <label
	class="smallAuto">(HH:MM:SS)</label> <label><span>*</span> To
Time</label> <input type="text" name="<%= TO_TIME%>" value="" class="date"
	validate="To Time,string,yes"
	onkeyup="mask(this.value,this,'2,5',':');" MAXLENGTH="8" id="toTime" /> <label
...
<%--- Report Button   --%> <input type="button" name="Report"
	...
	onClick="if(check()){submitForm('report','billing?method=printDayEndReport')};"
