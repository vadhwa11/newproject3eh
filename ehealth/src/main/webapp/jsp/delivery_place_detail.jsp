...
function checkk()
{
var SDate = document.delivery2.<%= FROM_DATE%>.value;
var EDate = document.delivery2.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
<h2>Delivery Place  Details Report</h2>
</div>
<form name="delivery2" method="post" action="">
<div class="clear"></div>
<div class="Block">
...
<label>CHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="chc" onclick="submitProtoAjaxWithDivName('delivery2','/hms/hms/pubHealth?method=getDelivery2ChcPhcList&district='+this.value,'tDiv');">
<label>PHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="phc" onclick="submitProtoAjaxWithDivName('delivery2','/hms/hms/pubHealth?method=getDelivery2ChcPhcList&district='+this.value,'tDiv');">
</div>
...
<div class="clear"></div>
 <div id="tDiv">
	<%if(map.get("instituteTypeShortName") != null && map.get("instituteTypeShortName").toString().equalsIgnoreCase("CHC")) {%>
		<label>CHC </label>
    	<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('delivery2','/hms/hms/pubHealth?method=getBsFwsList&chcphc='+this.value,'testDiv');">
			<option value="0">Select</option>
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.delivery2.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.delivery2.<%=TO_DATE%>,event);" />
<div class="clear"></div>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('delivery2','pubHealth?method=generateDeliveryPlaceDetailReport','checkk()');"
