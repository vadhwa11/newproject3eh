...
function goPage(pidx) {
	...
	submitForm('indentForm','laundry?method=showAddDepartmentIndentLaundry&internalIndentId='+internalIndentId);
}
...
function getOtherItemsForDepartmentIndent(val){
	 ...
	 for(var i=parseInt(counterValue)+1;i<(parseInt(counterValue)+14);i++){
	 ...
	 return false;
	  ...
	  var toWard=document.indentForm.<%= TO_WARD%>.value;
	    if(pvmsNo!=""){
	ajaxFunctionForGetOtherItemsForDepartmentIndent('indentForm','laundry?method=getOtherItemsForIndentLaundry&requiredField=' + encodeURIComponent(pvmsNo)+'&<%=FROM_WARD%>='+fromWard+'&<%=TO_WARD%>='+toWard+''   , val);
...
function jsNext(){
		...
		submitForm('indentForm','laundry?method=doAddInternalIndentItemsLaundry&internalIndentId='+internalIndentId + '&pageNo='+pageNo+'&<%=FROM_WARD%>=' + deptId + '&<%=TO_WARD%>=' + toDept + '&<%=DEMAND_NO%>=' + demandNo+ "&<%=DEMAND_DATE%>="+demandDate);
...
if(document.indentForm.pageNo.value!=1){
		...
		submitForm('indentForm','laundry?method=showAddDepartmentIndentLaundry&internalIndentId='+internalIndentId + '&pageNo='+pageNo+'&<%=FROM_WARD%>=' + deptId + '&<%=TO_WARD%>=' + toDept + '&<%=DEMAND_NO%>=' + demandNo+ "&<%=DEMAND_DATE%>="+demandDate);
}
else{
...
function jsAdd()
{
		...
		var demandNo = document.indentForm.<%= DEMAND_NO%>.value;
		var demandDate = document.indentForm.<%= DEMAND_DATE%>.value;
		submitForm('indentForm','laundry?method=doAddInternalIndentItemsLaundry&internalIndentId='+internalIndentId + '&<%=FROM_WARD%>=' + deptId + '&<%=TO_WARD%>=' + toDept + '&<%=DEMAND_NO%>=' + demandNo+ "&<%=DEMAND_DATE%>="+demandDate);
...
var status1="YES"
window.opener.location.href = "laundry?method=getLaundryIndentData&<%=FROM_WARD%>="+indentForm.<%=FROM_WARD%>.value+"&<%=DEMAND_NO%>="+demandNo+"&status1="+status1;
...
<div class="titleBg">
<h2>Department(Laundry) Indent Addition</h2>
</div>
...
new Ajax.Autocompleter('<%="nomenclature"+""+counter %>','ac2update','laundry?method=getItemListForIndentLaundry',{minChars:3,parameters:'requiredField=<%=nomenclature%>&internalIndentId='+document.getElementById('internalIndentId').value+'&storeDepartmentId='+document.getElementById('storeDepartmentId').value+'&demandNo='+document.indentForm.<%= DEMAND_NO%>.value+'&from_ward='+document.indentForm.<%= FROM_WARD%>.value+''});
	...
	%>
		<td><input type="text" tabindex="1" disabled value="<%=counter%>"
			name="srno" class="readOnly" readonly="readonly" size="2" /></td>
		<td><input type="text" value="" tabindex="1" name="nomenclature"
