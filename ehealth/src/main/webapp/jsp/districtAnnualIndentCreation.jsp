...
%>
<form name="districtWiseIndent" method="post">
<div class="titleBg">
<h2>Annual Indent Creation At District Level</h2>
</div>
...
<label>Demand No.</label>
 <input type="text" name="<%=DEMAND_NO %>" value="<%=demandNo %>" readonly="readonly" MAXLENGTH="8" class="readOnly"   /> 
	...
	<label>Indent Date</label>
	 ...
	 <img id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16"	border="0" validate="Pick a date" class="calender" tabindex="1"	onClick="setdate('<%=date%>',document.districtWiseIndent.<%= DEMAND_DATE%>,event)" />
...
<select name="districtId" id="districtId" validate="District,string,yes" >
	<option value="0">Select</option>
	<%if(districtList.size()>0){
		for(MasDistrict masDistrict:districtList){
			if(masDistrict.getDistrictName()!=null){
		%>
...
<table border="0" cellpadding="0" cellspacing="0" id="indentGrid">
					...
					%>
			<tr>
				<td width="%5"><input type="checkbox" class="radioCheck" size="5" value="" name="srno" id="srNoId<%=i %>" readonly="readonly" /></td>
						
				<td onclick="javascript:openPopupWindow('<%=storeInternalIndentT[1]!= null?storeInternalIndentT[1]:"" %>','<%=storeInternalIndentT[2]!= null?storeInternalIndentT[2]:"" %>','<%=storeInternalIndentT[3]!= null?storeInternalIndentT[3]:"" %>','<%=storeInternalIndentT[4]!= null?storeInternalIndentT[4]:"" %>','<%=i %>');"><input type="text" size="5" value="<%=storeInternalIndentT[2]!= null?storeInternalIndentT[2]:"" %>" name="pvms" id="<%=pvms+i %>"  readonly="readonly" />
			<%-- 	<input type="hidden" size="5" value="<%=storeInternalIndentT[10]!= null?storeInternalIndentT[10]:"" %>" name="storeInternalTId" id="storeInternalTId<%=i %>"  readonly="readonly" /> --%></td>
				<td><input type="text" value="<%=storeInternalIndentT[3]!= null?storeInternalIndentT[3]:"" %>" tabindex="1" name="nomenclature" size="20" id="<%=nomenclature+i %>" onblur="getOtherItemsForDepartmentIndent(<%=i %>);"  />
					<div id="ac2update" class="autocomplete" style="display: none;"></div>
					<script type="text/javascript" language="javascript" charset="utf-10">
						new Ajax.Autocompleter('<%="nomenclature"+""+i %>','ac2update','stores?method=getItemListForIndent',{minChars:3,parameters:'requiredField=<%=nomenclature%>&toWard='+document.districtWiseIndent.<%=TO_WARD%>.value+''});
						</script></td>
				<td><input type="text" size="4" value="<%=storeInternalIndentT[4]!= null?storeInternalIndentT[4]:"" %>" name="au" id="<%="au"+""+i %>" readonly="readonly" /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[5]!= null?((BigDecimal)storeInternalIndentT[5]).intValue():"" %>" name="year1IndentQty" id="year1IndentQtyId<%=i %>" readonly="readonly" /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[6]!= null?((BigDecimal)storeInternalIndentT[6]).intValue():"" %>" name="year2IndentQty" id="year2IndentQtyId<%=i %>"  readonly="readonly" /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[7]!= null?((BigDecimal)storeInternalIndentT[7]).intValue():"" %>" name="previousYearConsumption"  id="previousYearConsumptionId<%=i %>"  readonly="readonly" /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[8]!= null?((Integer)storeInternalIndentT[8]):"" %>" name="stock" validate="Stock In Hand,num,no" id="<%=stock+i %>"  readonly="readonly" /></td>
				...
				<td><input type="text" size="8" value="" name="incrementalPercentage" validate="IncrementalPercentage,float,no" id="incrementalPercentageId<%=i %>" onblur="calculateIndentQty(<%=i %>);"  /></td>
				<td><input type="text" size="8" value="" name="incrementalQty" id="incrementalQtyId<%=i %>"  validate="IncrementalQty,float,no" /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[9]!= null?((BigDecimal)storeInternalIndentT[9]).intValue():"" %>" name="pendingIndentQty" id="pendingIndentQtyId<%=i %>" onblur="calculateIndentQty(<%=i %>);" validate="Pending Indent Qty,float,no" /></td>
				<td><input type="text" size="8" value="" name="requiredQty" id="requiredQtyId<%=i %>" validate="Required Qty,float,yes" /></td>
				<td><input type="text" size="8" value="" name="additionalQty" onblur="calculateIndentQty(<%=i %>);" validate="Additional Qty,float,no" id="additionalQtyId<%=i %>"   /></td>
				<td><input type="text" size="8" value="<%=storeInternalIndentT[0]!= null?((Integer)storeInternalIndentT[0]):"" %>" name="qtyRequest" id="<%=qtyRequest+i %>" validate="Demanded Qty,float,no" />
				...
				<td><input type="text" size="10" value="" name="remarks" id="remarks<%=i %>"   />
...
<div class="clear"></div>
<%-- 
<label>DMO Remarks</label>
<textarea name="dmoRemarks"  validate="Present Complaint,string,no" cols="0" rows="0" class="large" maxlength="300" tabindex="1" ></textarea>
--%>
...
<div class="clear"></div>
<input type="button" name="Next/Update" type="submit" value="Submit"	onClick="submitForm('districtWiseIndent','stores?method=submitDistrictWiseIndent','validateQty');" class="button" />
...
{
 var url="/hms/hms/stores?method=showInstituteIndentPopupJsp&itemId="+itemId+"&itemCode="+itemCode+"&itemName="+itemName+"&au="+au+"&rowVal="+rowVal;
...
function getOtherItemsForDepartmentIndent(val){
	...
	districtWiseIndent.method="post";
	 ...
	 return false;
	  ...
	  var toWard=document.districtWiseIndent.<%= TO_WARD%>.value;
	    if(pvmsNo!=""){
	ajaxFunctionForGetOtherItemsForDepartmentIndent('districtWiseIndent','stores?method=getOtherItemsForIndent&requiredField=' + encodeURIComponent(pvmsNo)+'&<%=TO_WARD%>='+toWard+''   , val);
