...
if(request.getAttribute("map") != null){
	...
	/*	if(mapEmployee.get("tradeList") != null){
		...
		}//else if(session.getAttribute("masHospitalList") != null){
		//	masHospitalList = (List<MasHospital>)session.getAttribute("masHospitalList");
	//	}
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
			    		      <label class="medium">Emp Code</label>     
                 ...
                 <input type="button" name="search" value="Search" class="buttonBig" onclick="submitForm('search','personnel?method=searchEmployeeApp','checkSearch')" tabindex=1  />
...
<div class="clear"></div>
 ...
 <div id="searchresults" >
	...
	%> 
  <br/>
    <a href="personnel?method=showEmployeeApprovedJsp">Show All Records</a>
	<%
			}
		 }else{%> 
			 <a href="personnel?method=showEmployeeApprovedJsp">Show All Records</a>
	...
	,[60,"<%=QUALIFICATION + "1"%>"],[61,"<%=QUALIFICATION + "2"%>"],[62,"<%=QUALIFICATION + "3"%>"]
	,[63,"<%=COURSES_NAME + "1"%>"],[64,"<%=COURSES_NAME + "2"%>"],[65,"<%=COURSES_NAME+ "3"%>"]
	,[66,"<%=SPL_QUALIFICATION + "1"%>"],[67,"<%=SPL_QUALIFICATION+ "2"%>"],[68,"<%=SPL_QUALIFICATION + "3"%>"]
	,[69,"<%=INSTITUTION + "1"%>"],[70,"<%=INSTITUTION+ "2"%>"],[71,"<%=INSTITUTION+ "3"%>"]
	,[72,"<%=START_DATE + "1"%>"],[73,"<%=START_DATE + "2"%>"],[74,"<%=START_DATE+ "3"%>"]
	,[75,"<%=END_DATE + "1"%>"],[76,"<%=END_DATE + "2"%>"],[77,"<%=END_DATE + "3"%>"]
	,[78,"<%=COURSES_DURATION + "1"%>"],[79,"<%=COURSES_DURATION+ "2"%>"],[80,"<%=COURSES_DURATION+ "3"%>"]
	,[81,"<%=QUALIFIED_DATE + "1"%>"],[82,"<%=QUALIFIED_DATE+ "2"%>"],[83,"<%=QUALIFIED_DATE+ "3"%>"]
	,[84,"<%=AWARDS+ "1"%>"],[85,"<%=AWARDS + "2"%>"],[86,"<%=AWARDS + "3"%>"]
	,[87,"<%=EDUCATION_SKILLS+ "1"%>"],[88,"<%=EDUCATION_SKILLS+ "2"%>"],[89,"<%=EDUCATION_SKILLS+ "3"%>"]
	...
	,[164,"<%=EMPLOYER_ADDRESS+ "5"%>"],[165,"<%=EXPERIENCE_SKILLS+"5"%>"],[166,"<%=MIDDLE_NAME%>"],[167,"PEN"]
  ...
  <input type="hidden" id="rowid" value = "2" />  
	  ...
	  <!--  <div class="division"></div> -->
		...
		<div class="clear"></div>
	  		...
	  		<label > PEN </label>
	  		<input type="text" id="PEN" name="PEN" value="" MAXLENGTH="12" class="medium3" validate="PEN,String,yes" />
			...
			<input type="text" id="employee_Code" name="employee_Code" value="" MAXLENGTH="6" class="medium3" validate="Employee Code,String,yes"  
...
<%--
			...
			<select name="<%=EMPLOYEE_GRADE_ID %>">
		 	...
		 	<option value="<%=(Integer)grade[0] %>"><%=(String)grade[1]%></option>
		 	<%} %>
			</select>
			<div class="clear"></div>
			<label>Employee Category </label>
			 	<select name="<%=CATEGORY%>" id="empCategory" validate="Employee Catogory,int,no">
				 	<option value="0">Select</option>
			 ...
			 --%>
			<!--<label>Emp Photo</label>
			<input type="text" name="<%= EMPLOYEE_PHOTO%>" value="" validate="Employee Photo,string,no" class="medium3" MAXLENGTH="250"/ >
			
			<input type="text" name="<%=EMPLOYEE_PHOTO%>" value="" >-->
...
<%-- 			<input type="text" id="dateOfBirth" name="<%=DATE_OF_BIRTH%>" value=""  class="calDate"   onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');"/> --%>
			...
			<%-- <input type="text" name="<%= PASSPORTISSUEDATE%>" id="passportIssueDate"  onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" value="" validate="Passport Issue Date,date,no"  class="calDate" /> --%>
			<input type="text" name="<%= PASSPORTISSUEDATE%>" id="passportIssueDate"  validate="Passport Issue Date,date,no" class="date" MAXLENGTH="30" onkeyup="mask(this.value,this,'2,5','/');" onblur="validateExpDate(this,'td');"/> 
			...
			<label style="margin-left:36px;">Passport Expiry Date</label>
			<%-- <input type="text" id="passportExpiryDate"   onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" name="<%= PASSPORTEXPIRYDATE%>" value="" validate="Passport Expiry Date,date,no"  class="calDate" onchange="chkPassportDate(this.value)"/> --%>
			<input type="text" name="<%= PASSPORTEXPIRYDATE%>" id="passportExpiryDate"  validate="Passport Expiry Date,date,no" class="date" MAXLENGTH="30" onkeyup="mask(this.value,this,'2,5','/');" onblur="validateExpDate(this,'td');"/> 
			...
			<input type="text" name="<%= PASSPORTISSUEPLACE%>" value="" validate="Passport Issue Place,string,no"  MAXLENGTH="20"/>
			...
			<input type="text" name="<%= DRIVING%>" value="" validate="Driving Licence,string,no"  MAXLENGTH="20"/> --%>
   	  	 	...
   	  	 	<div class="clear"></div>
   	  	 	<!-- 
   	  	 	<label>Type Of Vehical</label>
	
			<select name="vehi" id ="vehi"  validate="Type Of Vehical,string,no" class="mediumm">
			<option value="0">Select</option>
			
			<option value="Two vehiler">Two vehiler</option>
				<option value="Four vehiler">Four vehiler</option>
			
			</select>
			
				<label>Registration No</label>
			<input type="text" name="vehiRegis" value="" validate="Registration No,string,no"  MAXLENGTH="20"/>
			
				<label>Make</label>
			<input type="text" name="make" id="make"  value="" validate="Make,string,no"  MAXLENGTH="20"/>
   	  	 	
   	  	 	<div class="clear"></div>
   	  	 	<label>Model</label>
			<input type="text" name="model" id="model"  value="" validate="Model,string,no"  MAXLENGTH="20"/> -->
		...
		<div class="clear"></div>
	  	 <div class="Block" id="title4"> 
			...
			<input type="text" name="<%= BANK_ACCOUNT_CODE%>" value="" validate="Bank Account Code,string,no" class="medium3" MAXLENGTH="10"/>
			...
			<input type="text" name="<%= ACCOUNT_HEAD%>" value="" validate="Account Head,name,no" class="medium3" MAXLENGTH="10"/ >
   	  	 	 <%--
		...
		<div class="clear"></div>
	  	 <div class="Block" id="title5"> 
			...
			<input type="text" id='officePhone' style="width:155px;" name="<%= TELL_NO_OFFICE%>" value="" validate="Office Phone,alphanumeric,no"  MAXLENGTH=12 />
			
			<label>Resi. Phone</label>
			<input type="text" name="<%= TELL_NO_RESIDENCE %>" style="width:155px;" value="" validate="Residence Phone,alphanumeric,no" class="medium3" MAXLENGTH="12" />
		...
		<%-- 	<label>Emp Url</label>
			...
			<label style="margin-left:-20px">Any Certification</label>
...
countries.init()
		...
		<div class="clear"></div>
			...
			<input type="text" id="expserviceStartDate1"  name="<%=SERVICE_START_DATE+"1"%>"   class="calDate" onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Service Start Date 1 ,date,no"   MAXLENGTH="30" />
			<%-- <img src="/erp/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.employee.<%=SERVICE_START_DATE+"1"%>,event)" validate="Pick a date" class="calender" /> --%>
	 		
	 		<label>Service End Date</label>
			
			 <input type="text" id="SERVICE_END_DATE1"  name="<%=SERVICE_END_DATE+"1"%>" class="calDate" onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Service End Date,date,no"   MAXLENGTH="30" />
			<%-- <img src="/erp/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.employee.<%=SERVICE_END_DATE+"1"%>,event)" validate="Pick a date" class="calender" /> --%>
			...
			<label style="width:176px;">Awards</label>
		...
		<div id="country7" class="tabcontent">
			...
			<label>Service Start date</label> 
			<input type="text" id="expserviceStartDate2"  name="<%=SERVICE_START_DATE+"2"%>"   class="calDate" onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Service Start Date,date,no"  />
			
			
			<%-- <img src="/erp/jsp/images/cal.gif" width="16" onclick="javascript:setdate('',document.employee.<%=SERVICE_START_DATE+"2"%>,event)" height="16" border="0" validate="Pick a date"  class="calender"/> --%>
	 		
	 		<label>Service End Date</label>
		
			
			 <input type="text" id="SERVICE_END_DATE2"  name="<%=SERVICE_END_DATE+"2"%>" class="calDate"  onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Service End Date 2,date,no"   MAXLENGTH="30" />
			<%-- <img src="/erp/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.employee.<%=SERVICE_END_DATE+"2"%>,event)" validate="Pick a date" class="calender" /> --%>
...
countries1.init()
		...
		<div class="clear"></div>
			...
			<input type="text" name="<%=INSURANCE_START_DATE%>" value=""  onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Insurance Start Date,string,no" class="calDate">
			...
			<input type="text" name="<%=INSURANCE_END_DATE%>" value=""  onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Insurance Start Date,string,no" class="calDate">
	 		<%-- <img src="/erp/jsp/images/cal.gif" width="16" onClick="javascript:setdate('',document.employee.<%=INSURANCE_END_DATE%>,event)" height="16" border="0" validate="Pick a date" class="calender" /> --%>			
			...
			<div id="edited"></div>
			<input type="hidden" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('employee','personnel?method=addEmployeeCliniRx');" accesskey="a" />
			<input type="hidden" name="edit" id="editbutton" value="Update"  style="display:none;" class="button" onClick="submitForm('employee','personnel?method=addEmployeeCliniRx')" accesskey="u"  />
			<input type="hidden" name="Delete" id="deletebutton" value="Activate"  class="button" onClick="submitForm('employee','personnel?method=deleteEmployee&flag='+this.value)" accesskey="d" />		
			<input type="hidden"  name ="Reset" id="reset" value ="Reset" class="button" onclick="location.reload();" accesskey="r" />
			<input type="button" name="save" value="Confirmed" class="button" onClick="submitFormForButton('employee','/hms/hms/personnel?method=submitApproveEmployee');" />
...
data_arr[<%= counter%>][1] = "<input type='checkbox'  value='<%=masEmp.getId()%>' name='employeeId' class='none' />"
<%}else {%>
data_arr[<%= counter%>][1] = ""
<%} 
if(masEmp.getAppointmentDate() != null){%>
data_arr[<%= counter%>][2] = "<%=masEmp.getEmployeeCode()%>"
<%}else {%>
data_arr[<%= counter%>][2] = ""
...
<%}if(masEmp.getStatus() != null){
	String status = ""; 
	if(masEmp.getStatus().equalsIgnoreCase("y") || masEmp.getStatus().equalsIgnoreCase("e"))
		status= "Active";
	else
		status= "Inactive";
%>
    ...
    xmlHttp.onreadystatechange=function()
    {
      if(xmlHttp.readyState==4){
      	...
      	for (loop = 0; loop < items.childNodes.length; loop++)
      	{
	       ...
	       // alert("---status>>>"+status.childNodes[0].nodeValue)
	        if(status.childNodes[0].nodeValue == "yes"){
	        	alert("Employee code allready Exist.");
	        	 document.getElementById('employee_Code').value="";
