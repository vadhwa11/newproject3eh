...
<div class="titleBg">
<h2>Suspension Order Entry</h2>
</div>
<div class="Block">
<div id="searcharea">
<div id="searchbar">
<!-- <form name="search" method="post"   action=""> -->
<form name="empSuspen" method="post" enctype="multipart/form-data">
...
<!-- <option value="1">Order By GOK </option> -->
	<%for(MasEmployee masEmp:suspendByList){ %>
	<option value="<%=masEmp.getId()%>"><%=masEmp.getEmployeeName()+" ("+(masEmp.getRank()!=null?masEmp.getRank().getRankName():"")+")"%></option>
	<%} %>
  </select>
</div>
<div class="clear"></div>
...
<input type="hidden" id="fromDate" value="<%=dateCal+"/"+month+"/"+year%>" name="fromDate" class="date" onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Actual Reliving date,date,no"   MAXLENGTH="30" />
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.empSuspen.fromDate,event)" validate="Pick a date" class="calender" />

<label > To Date </label> 

 <input type="hidden" id="toDate" value="<%=dateCal+"/"+month+"/"+year%>" name="toDate" class="date" onkeyup="mask(this.value,this,'2,5','/');"	 maxlength="10" onblur="validateExpDate(this,'td');" validate="Actual Reliving date,date,no"   MAXLENGTH="30" />
<!-- <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.empSuspen.toDate,event)" validate="Pick a date" class="calender" /> -->
--%>

<label>Suspension Period</label>
<input type="text" id="suspPeriod" name="suspPeriod"  maxlength="3"    />
			<select id="suspPeriodUnit" name="suspPeriodUnit" 	 validate="Suspension Period,string,no" tabindex=1>
...
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" onClick="javascript:setdate('',document.empSuspen.orderDate,event)" validate="Pick a date" class="calender" />
...
<input type="hidden" id="mode" name="mode" value="Suspension" MAXLENGTH="12" class="medium3" validate="Remarks,String,no" />
...
<!-- <input name="button"  type="button"	value="Submit" class="button"	 onClick="submitForm('empSuspen','/hms/hrms/training?method=saveEmpSuspention','chkDate')"  /> -->
 <input name="button"  type="button"	value="Submit" class="button"	 onClick="submitForm('empSuspen','/hms/hrms/training?method=saveEmpSuspention&'+csrfTokenName+'='+csrfTokenValue)"  />
	...
	//alert(""+pen)
	 ...
	 function getSuspendByList(){
			...
			if(PEN != ""){
				...
				if(emp_id !=""){
			submitProtoAjaxWithDivName('empSuspen','/hms/hrms/training?method=getSuspendByList&empId='+emp_id+'&'+csrfTokenName+'='+csrfTokenValue,'testDiv');
				}
			}else{
		...
		function chkDate()
		{
			...
			obj1 = document.<%="empSuspen"%>.fromDate.value;
			obj2 = document.<%="empSuspen"%>.toDate.value;
