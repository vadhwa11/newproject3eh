...
<%--<label>Voucher No<span>*</span></label> --%>
 <input id="voucherNoId" type="hidden" readonly="readonly" size="5" name="<%=VOUCHER_NO%>" value="<%= voucherNo %>"  validate="Voucher No,metachar,no" MAXLENGTH="8"  tabindex=1 />
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	id="voucherDetails" class="cmntable">
	...
	<td width="4"><select id="drcr<%=i %>" name="drCr<%=i  %>" class="smallest" onchange="changeField(this.value,<%=i %>);" validate="DrCr,metachar,yes" tabindex=1 >
...
//alert("group-=="+document.getElementById('itemGroupId').value);
	...
	<td><select id="costCenterId<%=i %>"  name="<%=COST_CENTER_ID %><%=i%>"  class="medium" tabindex=1 validate="Cost Center,string,no" >
	<option value="0">Select</option>
...
%>
<script>
wardArray1[<%=l%>]= new Array();
wardArray1[<%=l%>][0] = "<%=ward.getId()%>";
wardArray1[<%=l%>][1] = "<%=ward.getWardName()%>";
...
function addRow(){
	...
	//e8.type='text'
	e8.name='wardName'+ (iteration);
	e8.id='wardId'+(iteration);
	e8.options[0] = new Option('Select', '0');
	for(var n21 = 0;n21<wardArray1.length;n21++){
		e8.options[n21+1] = new Option(wardArray1[n21][1],wardArray1[n21][0]);
	}
	e8.className='medium';
	...
	e31.name='<%=COST_CENTER_ID%>'+ (iteration);
	e31.id = 'costCenterId'+(iteration);
	e31.options[0] = new Option('Select', '0');
	for(var k = 0;k<costCenterArray.length;k++){
		e31.options[k+1] = new Option(costCenterArray[k][1],costCenterArray[k][0]);
	}
	e31.tabIndex='1';
	e31.className='medium';
	cell4.appendChild(e31);
