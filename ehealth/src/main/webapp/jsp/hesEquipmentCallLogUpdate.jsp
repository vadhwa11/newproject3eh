...
<%
	...
	if(map.get("searchList")!=null){
		searchList=(List<HesEquipmentCallLogEntry>)map.get("searchList");
	}
	...
	if (map.get("employeeListTo")!= null){
		employeeList = (List<MasEmployee>)map.get("employeeListTo");
	}	
...
<%
		for (HesEquipmentCallLogEntry hesEquipmentCallLogEntry : searchList )
		{
%>
			<option value=<%=hesEquipmentCallLogEntry.getEquipmentMaster().getId()%>><%=hesEquipmentCallLogEntry.getEquipmentMaster().getEquipmentName()%></option>
<%
		}
%>
</select>
...
<%
		for (HesEquipmentCallLogEntry hesEquipmenCallLogObj : searchList )
		{
%>
...
<label>Remarks</label>
<%
  for(HesEquipmentCallLogEntry callLogObj : searchList ){%>
<input type="hidden" id="radio_str" name="radio_str" value="<%=callLogObj.getId() %>" / >     
     
 <textarea id="remarks" rows="8" name="remarks"  ><%=callLogObj.getRemarks() %></textarea>  
  <%}
...
<input type="button" name="sss" align="right" class="button" value="Update"
 onclick="submitForm('equipmentCallLogEntryForm','hes?method=modifyCallLogEntry');" />
