...
<%@page import="jkt.hms.masters.business.HesEquipmentMaintenance"%><script type="text/javascript" src="/hms/jsp/js/pops_global.js"></script>
...
<form name="equipmentMaintenanceSearchForm" method="post"">
...
<div class="clear"></div>
  ...
  <label>Date </label> <input type="text" name="<%=ON_DATE%>"
	...
	onclick="setdate('<%=currentDate%>',document.equipmentMaintenanceSearchForm.<%= ON_DATE%>,event);" />
	
<label>Equipment Name :</label>
<select name="searchEquipment">
	<option value="" >-Select Equipment Name-</option>
<%
		for (HesEquipmentMaintenance hesEquipmentMaintenanceObj : equipmentList )
		{
%>
			<option value=<%=hesEquipmentMaintenanceObj.getEquipmentMaster().getId()%>><%=hesEquipmentMaintenanceObj.getEquipmentMaster().getEquipmentName()%></option>
<%
		}
%>
</select>
...
<%
		for (HesEquipmentMaintenance hesEquipmentMaintObj : equipmentList )
		{
%>
			<option value=<%=hesEquipmentMaintObj.getId()%>><%=hesEquipmentMaintObj.getEntryNo()%></option>
<%
		}
%>
</select>
...
<input type="image" src="/hms/jsp/images/go.gif" class="button"
	onClick="submitForm('equipmentMaintenanceSearchForm','hes?method=searchEquipmentMaintenanceForm');" />
...
<jsp:include page="searchResultBlock.jsp" /> <%
 ...
 %>
<div class="clear"></div>
<jsp:include page="hes_Equipment_Maintenance.jsp" /> 
<%}
 %> <script type="text/javascript">
<!--
...
//-->
	...
	onClick="submitForm('equipmentMaintenanceSearchForm','hes?method=showEquipmentMaintenanceUpdateJsp');" />
