...
Box box = HMSUtil.getBox(request);
	...
	if(map.get("searchList")!=null){
		searchList=(List<HesEquipmentMaintenance>)map.get("searchList");
	}
	...
	if (map.get("hesEqpMaster")!= null){
		hesEqpMaster = (List<HesEquipmentMaintenance>)map.get("hesEqpMaster");
	}
...
<%
		for (HesEquipmentMaintenance hesEquipmentMaintenance : searchList  )
		{
%>
...
<%
			for(MasEmployee masEmployee : employeeListTo){
			%>
...
<input type="text" name="<%=DELIVERY_TIME%>" value="<%=currentTime %>"  validate="Schedule Time,deliveryTime,yes"
	...
	<%
	   for(HesEquipmentMaintenance maintenanceObj : searchList){%>
		 <input type = "hidden" id="radio_str" name="radio_str" value="<%=maintenanceObj.getId()%>" /> 
	   <%}
	%>
...
<input type="button" name="sss" align="right" class="button" value="Update"
 onclick="submitForm('equipmentPreventMaintenanceEntryForm','hes?method=modifyMaintenanceEntry');" />
