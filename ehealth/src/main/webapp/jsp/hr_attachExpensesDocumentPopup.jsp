...
function jsImport()
{
	...
	submitForm('attachFile','etravel?method=addAttachExpensesFile&filename='+filename+csrfTokenName+'='+csrfTokenValue);
...
<%
	int i = 0;
	if(temExpensesAttachList.size()>0){
		for(TempTrvsub tempTrvsub1:temExpensesAttachList){
%>
<tr>
<td><input name="<%=TICKET_ATTACH_ID%><%=i%>"  class="radioCheck"  type="checkbox"  value="<%=tempTrvsub1.getId()%>"  /></td>
<td><%=tempTrvsub1.getTdsaDfilename()%></td>
<td><%=tempTrvsub1.getTdsaCmts()%></td>
</tr>
<%i++;
	}}
%>
...
<input name="Delete" type="button"  class="button" value="Delete" onClick="submitForm('attachFile','etravel?method=deleteAttachExpenseFile&'+csrfTokenName+'='+csrfTokenValue);" />
