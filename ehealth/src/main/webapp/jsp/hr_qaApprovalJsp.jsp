...
function openPopupWindow()
			{ var id = 0; 
				 for(var i = 0; i < document.getElementsByName('fesbilitystudyId').length; i++){
		  		...
		  		var url="/hms/hrms/projectTracking?method=viewQaApprovalDocument&fesbilitystudyId="+id+"";
...
<div class="titleBg"> <h2>QA Approval</h2></div>
...
<div id="searchtable" tabindex=2 ></div>
    <script type="text/javascript">
	formFields = [
			[0, "<%= COMMON_ID%>", "id"], [1,"radioId"], [2,"<%= PROJECT_ID %>"],[3,"<%= SITE_ID %>"],[4,"<%= PLANNED_DATE %>"],[5,"<%= ACTUAL_DATE %>"],[6,"<%= PI_HEADER_ID %>"],[7,"<%= CONTACT_PHONE %>"],[8,"<%=QA_APPROVAL_STATUS%>"],[9,"<%=QA_APPROVER_EMP_ID%>"],[10,"<%=QA_APPROVAL_DATE%>"],[11,"<%=QA_APPROVAL_COMMENTS%>"], [12,"<%= CHANGED_BY %>"],[13,"<%= CHANGED_DATE %>"],[14,"<%=CHANGED_TIME%>"],[15,"<%=STATUS%>"]];
	 statusTd = 15;
	</script>
...
<script>
	...
	<div class="clear"></div>
	<div class="division"></div>
	<form name="qaApprovalStatus" method="post" action="" >
	<div class="Block">
     ...
     <select name="<%=QA_APPROVAL_STATUS %>" value="" validate="Status,string,yes" tabindex=1 >
    ...
    <input type="text" name="<%=QA_APPROVAL_DATE%>" value="" readonly="readonly" validate="Date,string,yes" class="textbox_size20" MAXLENGTH="15"/ tabindex=1 />
    <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" class="calender" onclick="setdate('',document.qaApprovalStatus.<%=QA_APPROVAL_DATE%>,event)" />
     
     <div class="clear"></div>
     <label>Comments</label>
     <input type="text" name="<%=QA_APPROVAL_COMMENTS%>" value="" validate="Comments,string,no" class="large" MAXLENGTH="350" / tabindex=1 />
    <div class="clear"></div>
    </div>
     <div class="clear"></div>
       
            <div id="edited"></div>
            <input type="button" name="add" id="addbutton" value="Add" style="display: none;" class="button" onClick="submitForm('qaApprovalStatus','projectTracking?method=addQAApprovalStatus')" accesskey="a" tabindex=1/>
		
			<input type="button" name="edit" id="editbutton" value="Update" style="display: none;" class="button" onClick="submitForm('qaApprovalStatus','projectTracking?method=updateQAApprovalStatus')" accesskey="u" tabindex=1 />
            
            <input type="button" name="Delete" id="deletebutton" value="Activate" style="display: none;" class="button" onClick="submitForm('qaApprovalStatus','projectTracking?method=deleteQAApprovalSatus&flag='+this.value)" accesskey="d" tabindex=1/>
...
data_header[7][2] = "25%";
data_header[7][3] = "<%=QA_APPROVAL_STATUS %>";

data_header[8] = new Array;
data_header[8][0] = "Approver By"
data_header[8][1] = "data";
data_header[8][2] = "25%";
data_header[8][3] = "<%=QA_APPROVER_EMP_ID %>";

data_header[9] = new Array;
data_header[9][0] = "Approval Date"
data_header[9][1] = "data";
data_header[9][2] = "25%";
data_header[9][3] = "<%=QA_APPROVAL_DATE %>";

data_header[10] = new Array;
data_header[10][0] = ""
data_header[10][1] = "hide";
data_header[10][2] = 0;
data_header[10][3] = "<%=QA_APPROVAL_COMMENTS %>";
...
data_arr[<%= counter%>][8] = "<%if(prjFesStudyHeader.getQaApprovalStatus() != null){%><%= prjFesStudyHeader.getQaApprovalStatus()%><%}%>"
data_arr[<%= counter%>][9] = "<%if(prjFesStudyHeader.getQaApprover() != null){%><%= prjFesStudyHeader.getQaApprover().getFirstName()%> <%=prjFesStudyHeader.getQaApprover().getLastName()%><%}%>"
data_arr[<%= counter%>][10] = "<%if(prjFesStudyHeader.getQaApprovalDate() != null){%><%= HMSUtil.convertDateToStringWithoutTime(prjFesStudyHeader.getQaApprovalDate())%><%}%>"
data_arr[<%= counter%>][11] = "<%if(prjFesStudyHeader.getQaApproverComments() != null){%><%= prjFesStudyHeader.getQaApproverComments()%><%}%>"
...
function backProjectSetting()
 ...
 function nextProjectSetting()
 {
   var id = <%=projectId%>
   obj = eval('document.'+formName)
     obj.action = "projectTracking?method=showProjectApproval&projectId="+id;
     obj.submit();
     return true;
