...
<select name="<%= RequestConstants.TYPE_OF_INDENT%>" id="typeOfIndent">
	...
	value="Bangalore" class="textbox_size20" MAXLENGTH="30" id="nrs" /> <br />
<label class="bodytextB"><font id="error"></font>Authority:</label> <textarea
	name="<%=RequestConstants.AUTHORITY%>" MAXLENGTH="256" cols="27"
	rows="2" id="Authority" class="txtarea">37/36A && 37/37 </textarea> <label
	class="bodytextB"><font id="error"></font>Postal Address:</label> <textarea
	name="<%=RequestConstants.ADDRESS%>" MAXLENGTH="200" rows="2" cols="30"
	id="address" class="txtarea"></textarea> <%}else{ %> <label
	class="bodytextB"><font id="error"></font> Indent No: </label> <input
...
<label class="bodytextB"><font id="error"></font>NRS:</label> <input
	...
	name="<%= RequestConstants.NRS %>" value="<%=nrs%>" class="readOnly"
	MAXLENGTH="30" /> <br />
...
<div style="float: left; padding-left: 15px;"><input type="button"
	...
	onclick="if( checkSave()&&test()&&checkForNext()){submitForm('indentGrid','nonExp?method=addNextOrSubmitIndentToDepot&buttonName=next');}"
	align="right" /> <input type="button" name="sss" align="right"
	class="button" value="Submit"
	onclick="if( checkSave()&&test()&&checkForSubmit()){submitForm('indentGrid','nonExp?method=addNextOrSubmitIndentToDepot&buttonName=submit');}" />
</div>
