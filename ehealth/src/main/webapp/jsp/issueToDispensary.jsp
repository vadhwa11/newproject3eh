...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"><input
	...
	name="<%=RequestConstants.ISSUE_ID%>" id="issueId" value="<%=issueId%>" validate="IssueId,int,no"/>
<div class="clear"></div>
<h4>Issue Details</h4>
<div class="clear"></div>
...
<div id="testDiv"><select  name="<%= REQUEST_NO%>" tabindex="1"
	validate="Demand No,string,yes" id="requestNo"
    ...
    //	int tempVar=((pageNo-1)*20)+1;
 	...
 	inc=((pageNo-1)*indentTList.size())+1;
 	 ...
 	 int incTemp2=inc+indentTList.size();
		...
		<tr>
		 <input type="hidden" name="tt8" id='issueQtyAndBatch<%=inc %>' value="blank" readonly="readonly" validate=""/>
		 <input type="hidden" name="tt6" id="quantityToIssueOverAll<%=inc%>" value="" readonly="readonly"/>
			...
			<td width="10%"><input type="text" tabindex="1" size="35"
...
%>
			...
			%><input type="text" tabindex="1" size="8"
				value="<%=issyeRequestDiff%>" readonly="readonly" validate="Issue qty,float,no"
				name="<%=RequestConstants.QTY_IN_REQUEST%>" id="<%=qtyRequested%>" />
			<%}else if(qtyIssueRequest.equals(qtyIssueDB)){%>
			<input type="text" tabindex="1" size="8" validate="Issue qty,float,no"
	      ...
	      <% //} --%>
			...
			<input type="text" size="8" readonly="readonly" validate="Issue qty,float,no"
...
<input type="button" class="button" tabindex="1" onclick="if(validateIssue()){submitForm('issueDispensaryForm','stores?method=submitIssueForIndent');}" name="Submit11" value="Save" id="Save" />
