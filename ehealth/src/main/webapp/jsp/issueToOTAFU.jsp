...
<form name="issueToOTAFU" method="post">
...
<label class="bodytextB"><font id="error"></font>Request By:</label> <select
	...
	id="<%=RequestConstants.NO_OF_ROWS%>" value="0" /></div>
<br />


<div style="float: left; padding-left: 15px;"><input type="button"
	class="button" value="Next"
	onclick="if(checkForNext()&&test()){submitForm('issueToOTAFU','stores?method=addNextOrSubmitIssueToOTAFU&buttonName=next');}"
	align="right" /> <input type="button" name="sss" align="right"
	class="button" value="Submit"
	onclick="if(checkForSubmit()&&test()){{submitForm('issueToOTAFU','stores?method=addNextOrSubmitIssueToOtherUnits&buttonName=submit');}}" />
</div>
		...
		<tr>
		  ...
		  new Ajax.Autocompleter('<%=nameItem%>','ac2update','stores?method=getItemListForIssueToDispensary',{parameters:'requiredField=<%=nameItem%>&issueId='+document.getElementById('issueId').value});
			...
			<td width="10%"><input type="text" value="" id="<%=lotNo%>"
				class="medcaption"
				onblur="fillGridIssueToOTAFU('issueToOTAFU','stores?method=fillIssueToOTAFUBasedOnLotNo&lotNo='+this.value , <%=inc %>);" /></td>
			<td width="10%"><input type="text" value="0"
				validate="Qty Requested Year,floatWithoutSpaces,no" MAXLENGTH="7"
				class="medcaption" /></td>
			<td width="10%"><input type="text" value="" class="medcaption"
				name="<%=RequestConstants.QTY_ISSUED%>" readonly="readonly"
				tabindex="2" id="<%=qtyIssued%>" /></td>
			<td width="3%"><input type="button" class="morebutton"
				tabindex="1" onclick="openPopupForBrandForOTAFU(<%=inc%>);"
