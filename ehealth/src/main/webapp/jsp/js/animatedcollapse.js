...
toggle:function(divid,title){ //public method
	this.showhide(divid, "toggle",title)
},
...
showhide:function(divid, action,title){
	...
	if (this.divholders[divid] && $divref.length==1){ //if DIV exists
		...
		if ($divref.attr('groupname') && targetgroup.count>1 && (action=="show" || action=="toggle" && $divref.css('display')=='none')){ //If current DIV belongs to a group
			if (targetgroup.lastactivedivid && targetgroup.lastactivedivid!=divid) //if last active DIV is set
				...
				if(targetgroup.lastactivedivid == divid)
				{
				...
				{
					lastactiveheadid = 't' + targetgroup.lastactivedivid.substring(5,6); 
					document.getElementById(lastactiveheadid).setAttribute('class','');
				}
				document.getElementById(title).setAttribute('class','minus');
