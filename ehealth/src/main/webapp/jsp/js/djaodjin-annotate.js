...
(function($) {
  ...
  Annotate.prototype = {
   ...
   //   var rndm = Math.floor(Math.random() * 10);
      var rndm = self.options.id;
      self.baseLayerId = 'baseLayer_' + rndm;
      console.log("baseLayerId", self.baseLayerId);
      self.drawingLayerId = 'drawingLayer_' + rndm;
      // self.toolOptionId = 'tool_option_' + self.$el.attr('id');
      self.$el.append($('<canvas id="' + self.baseLayerId +
      ...
      self.exportedImages = document.getElementsByClassName('baseLayer')
    ...
    setBackgroundImage: function(image) {
      ...
      console.log("self.img.src", self.img.src);
    ...
    //     console.log("self.baseCanvas", self.baseCanvas);
    ...
    exportImage: function(options, callback) {
      ...
      options = $.extend({}, exportDefaults, options);
      var arr = [];
      for (var i = 0; i < self.exportedImages.length; i++) {
        var image = self.exportedImages[i].toDataURL(options.type,
          options.quality);
        arr.push(image);
      }
      if (callback) {
