...
<div class="Block">
	 ...
	 <img width="16" height="16" border="0" onclick="javascript:setdate('',document.mFeedBack.toDate,event)" validate="Pick a date" src="/hms/jsp/images/cal.gif">
	  ...
	  <div class="clear"></div>
	  <label>Item Name</label> <input type="text"	name="ItemName" value="" MAXLENGTH="30" id="ItemName" validate="Item Name,alphanumeric,no"/>
<div class="clear"></div>
<div class="paddingTop5"></div>
<input type="button" class="button" value="Search" onclick="if(checkData()){submitForm('mFeedBack', 'maintenance?method=showCondemnationJsp')}" />
<input type="button" class="button" value="Show All" onclick="if(checkData()){submitForm('mFeedBack', 'maintenance?method=showCondemnationJsp&flag=all')}" />
</div>
...
<table>
	<tr><th>S.No.</th><th>Request Date</th><th>Request No</th><th>Item Name</th></tr>
	<% 
		 int  counter=0;
		if (true) { %>
	...
	%>
	<form name="mCondemnation<%=counter %>" method="post">
	<input type="hidden" name="requestId" value="<%= serviceDetail.getId()%>" />
	<tr onclick="submitForm('mCondemnation<%=counter %>', 'maintenance?method=condemnDetails')">
			<td><%= counter+1%></td>
