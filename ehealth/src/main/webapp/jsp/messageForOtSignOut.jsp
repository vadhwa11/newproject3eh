...
if(map.get("otSignOutId") != null){
	otSignOutId = (Integer)map.get("otSignOutId");
}
System.out.println("otSignOutId------------------>>>>"+otSignOutId);
...
String ipNo="";
String wardName="";
if(map.get("ipNo")!=null){
	ipNo=(String)map.get("ipNo");
}
if(map.get("wardName")!=null){
...
%> <input type="button" name="yes" value="Yes" class="button"
	onclick="submitForm('messageAdt','/hms/hms/ot?method=printotSignOut&otSignOutId=<%=otSignOutId %>&ipNo=<%=ipNo%>&wardName=<%=wardName%>','checkTargetForYes');" />
