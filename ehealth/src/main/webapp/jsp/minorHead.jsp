...
<div class="clear" ></div>
<div class="titleBg">
<h2>Budget Minor Head Master </h2>
</div>
<div class="clear"></div>
<div class="Block">
...
<input type="hidden" name="<%=JASPER_FILE_NAME%>" value="bud_minor_head">
...
<%
		if(searchMinorHeadList != null)
		{
			if(searchMinorHeadList.size()>0)
		 	{
				...
				{
%>
				 	<a href="budget?method=showMinorHead">Show All Records</a> <%
				}
		 	}

	 		if(searchMinorHeadList.size()==0 && map.get("SEARCH_NAME") != null)
	  		{
%>
			 	<a href="budget?method=showMinorHead">Show All Records</a>
<%
...
formFields = [
			[0, "<%= COMMON_ID%>", "id"], [1,"<%= CODE%>"], [2,"<%=SEARCH_NAME%>"], [3,"<%=SUB_MAJOR_HEAD_ID%>"],[4,"<%=SEQUENCE_NO%>"], [5,"<%=STATUS%>"] ];
	 statusTd = 5;
	</script></div>
...
<div class="clear"></div>
<div class="Block">
<label><span>*</span> Minor Head Code</label>
...
<div class="clear"></div>
<label><span>*</span> Sub-Major Head Name</label>
<select	name="<%= SUB_MAJOR_HEAD_ID %>" validate="SubMajorHead,string,yes" tabindex=1 onkeypress="return submitenter(this,event,'generalMaster?method=addCountry')">
<option value="0">Select</option>
<%
for (BudSubMajorHead submajorHead: submajorHeadList){
%>
<option value="<%=submajorHead.getId() %>">
<%=submajorHead.getSubMajorHeadName()%></option>
<%}%>
</select>

<!--<label>Sequence No</label>
<input type="text" id="fname11"  name="" value=""  class="textbox_size20" onblur="getSequenceNo(this.value);"  MAXLENGTH="8" onblur="getSequenceNo(this.value);"  tabindex=1>
<div class="clear"></div>-->
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
<div id="edited"></div>
<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('cylindertypeMaster','budget?method=addMinorHead');" accesskey="a" tabindex=1 />
<input type="button" name="edit" id="editbutton" value="Update" class="button" style="display: none;" onClick="submitForm('cylindertypeMaster','budget?method=editMinorHead')" accesskey="u" tabindex=1 />
<input type="button" name="Delete" id="deletebutton" value="Activate" class="button" style="display: none" onClick="submitForm('cylindertypeMaster','budget?method=deleteMinorHead&flag='+this.value)" accesskey="d" tabindex=1 />
...
data_header[0][0] = "Minor Head Code"
data_header[0][1] = "data";
data_header[0][2] = "25%";
data_header[0][3] = "<%= CODE%>"

data_header[1] = new Array;
data_header[1][0] = "Minor Head Name"
...
data_header[2][2] = "40%";
data_header[2][3] = "<%=SUB_MAJOR_HEAD_ID%>";
...
Iterator itr=searchMinorHeadList.iterator();
int  counter=0;
while(itr.hasNext())
{
	BudMinorHead subminorHead = (BudMinorHead)itr.next();
	
%>
		data_arr[<%= counter%>] = new Array();
		data_arr[<%= counter%>][0] = <%= subminorHead.getId()%>
		data_arr[<%= counter%>][1] = "<%= subminorHead.getMinorHeadCode()%>"
		data_arr[<%= counter%>][2] = "<%= subminorHead.getMinorHeadName()%>"
<%
				Iterator itrGridMajorHeadList=gridsubMajorHeadList.iterator();
	 			while(itrGridMajorHeadList.hasNext())
        		{
	 				try
	 				{
	 					BudSubMajorHead  submajorheadGrid = (BudSubMajorHead)itrGridMajorHeadList.next();
		 				if(subminorHead.getSubMajorHeadId().getId().equals(submajorheadGrid.getId()) && submajorheadGrid.getStatus().equals("y"))
		 				{
%>
							data_arr[<%= counter%>][3] = "<%=submajorheadGrid.getSubMajorHeadName().trim()%>"
<%
						}
		 				else if(subminorHead.getId().equals(submajorheadGrid.getId()) && submajorheadGrid.getStatus().equals("n"))
		 				{
%>
data_arr[<%= counter%>][3] = "<font id='error'>*</font>Parent InActivated--<%=submajorheadGrid.getSubMajorHeadName().trim()%>"
...
{
	<%for(BudMinorHead budget:searchMinorHeadList){%>
