...
function checkk()
{
var SDate = document.nationalVector.<%= FROM_DATE%>.value;
var EDate = document.nationalVector.<%= TO_DATE %>.value;
...
<%
	...
	String flag="";
	if(map.get("reportFlag") !=null){
		flag=(String)map.get("reportFlag");
	}
	
	System.out.println("flag jsp "+flag);
%>
<div class="titleBg">
<h2>National Vector Borne Disease Registration Report</h2>
</div>
<form name="nationalVector" method="post" action="">
...
<label>CHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="chc" onclick="submitProtoAjaxWithDivName('nationalVector','/hms/hms/pubHealth?method=getComDiseaseChcPhcList&district='+this.value,'tDiv');">
<label>PHC</label><input type="radio" name="center" class="inputRadiobutton" id="center" value="phc" onclick="submitProtoAjaxWithDivName('nationalVector','/hms/hms/pubHealth?method=getComDiseaseChcPhcList&district='+this.value,'tDiv');">
</div>
...
<div class="clear"></div>
 <div id="tDiv">
	<%if(map.get("instituteTypeShortName") != null && map.get("instituteTypeShortName").toString().equalsIgnoreCase("CHC")) {%>
		<label>CHC </label>
    	<select name="chcphc" id="chcphc" onblur="submitProtoAjaxWithDivName('nationalVector','/hms/hms/pubHealth?method=getBsFwsList&chcphc='+this.value,'testDiv');">
			<option value="0">Select</option>
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.nationalVector.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.nationalVector.<%=TO_DATE%>,event);" />
<div class="clear"></div>
<div class="clear"></div>

<div class="clear"></div>
<%if(flag.equalsIgnoreCase("NVBD")){ %>
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('nationalVector','pubHealth?method=generateNationalVectorBorneDiseaseReport','checkk()');"
	accesskey="a" tabindex=1 />
<%}%>
<%if(flag.equalsIgnoreCase("RNTC")){ %>
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('nationalVector','pubHealth?method=generateRevisedNationalTuberculosis','checkk()');"
	accesskey="a" tabindex=1 />
<%}%>
<%if(flag.equalsIgnoreCase("NLEP")){ %>
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('nationalVector','pubHealth?method=generateNationalLeprosyEradicationProgram','checkk()');"
	accesskey="a" tabindex=1 />
<%}%>
<%if(flag.equalsIgnoreCase("MIGR")){ %>
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('nationalVector','pubHealth?method=generateMigrantSurvery','checkk()');"
	accesskey="a" tabindex=1 />
<%}%>
	<script type="text/javascript">
