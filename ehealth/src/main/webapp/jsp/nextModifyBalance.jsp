...
<label>From Date :</label> <input type="text" name="<%= FROM_DATE %>"
	MAXLENGTH="30" tabindex=1 class="date" /> <img id="calendar"
...
<input type="button" name="sss" align="right" class="button"
	value="Submit"
	onclick="if(checkSave()&& checkForSubmit()&&validateGridData()){submitForm('indentGrid','stores?method=updateNextOrSubmitBalance&buttonName=submit');}" />
<div id="pagination"><a
	onclick="if(checkSave()&& checkForNext()&&validateGridData()){submitForm('indentGrid','stores?method=updateNextOrSubmitBalance&buttonName=next');}"
...
<input type="hidden" name="modifyBalance" id="modifyBalance"
	value="modifyBalance" /> <input type="hidden" size="2" value="0"
...
<input type="hidden" name="" value="222" id="currentRow" />
<div class="cmntable">
	...
	<tbody>
		...
		<tr>
			...
			<td width="5%"><input type="text" size="2"
				value="<%=storeBalanceT.getSrNo()%>" name="<%=SR_NO%>"
		  ...
		  new Ajax.Autocompleter('<%=nameItem%>','ac2update','stores?method=getItemListForLoanoutByAutocompleteBalance',{parameters:'requiredField=<%=nameItem%>&balanceId='+document.getElementById('balanceId').value});
			...
			<!--
			<td width="10%">
			<%if(storeBalanceT.getItem().getManufacturer() !=null){ %> <select
				name="<%=RequestConstants.MANUFACTURER_ID %>" tabindex=1
				id=<%=manuId%> tabindex="1" readonly="readonly">
				<option
					value="<%=storeBalanceT.getItem().getManufacturer().getId()  %>"><%=storeBalanceT.getItem().getManufacturer().getManufacturerName() %>
				</option>
			</select> <input type="hidden"
				value="<%=storeBalanceT.getItem().getManufacturer().getId() %>"
				id="<%=manuIdTemp %>"> <%}else{ %> <select
				name="<%=RequestConstants.MANUFACTURER_ID %>" tabindex=1
				id=<%=manuId%> tabindex="1" readonly="readonly">
				<option value="">--</option>
 ...
 %>
		...
		<tr>
			...
			<td width="10%"><input type="text" size="8"
				...
				type="hidden" size="2" value="0" name="<%=ITEM_ID%>"
				id="<%=idItem%>" /></td>
