...
<%
	...
	String openMsg="";
	int open=0;
	if(request.getParameter("openMsg")!=null && open==0){
		openMsg = (String) request.getParameter("openMsg");
		open++;
	}
	String userName = "";
	...
	if (map.get("message") != null) {
		message1 = (String) map.get("message");
	}
...
<input type="text" name="patientName" id="patientName" validate="patientName,string,no" maxlength="40" value="<%=pName%>" onkeypress="return searchKeyPress(event);"/>
<label class="medium">UHID</label>
<input type="text" name="uhid" id="uhid" validate="uhid,string,no" value="<%=hinNo%>" maxlength="40" onkeypress="return searchKeyPress(event);"/>
...
<div class="clear"></div>
<form name="opd_OP_Clinic_Waiting_PatientList" method="post" action=""  onload="javascript:submitProtoAjaxWithDivName('opd_OP_Clinic_Waiting_PatientList','/hms/hms/investigation?method=showLabResultDetailAlert','alertdiv');">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
<input type="hidden" name="hospitalId" id="hospitalId" value="<%=hospitalId %>" />
<input type="hidden" name="deptId" id="deptId" value="<%=deptId %>" />
<input type="hidden" id="doctorId" name="doctorId" value="<%=userId %>">
...
<label class="auto" style="font-size:15px;margin-right:0;">Common Pool Patients (Unassigned)</label>
...
<div class="cmnTable" id="searchtable" tabindex=2> </div>
</div>
<div id="edited"></div>
...
<script type="text/javascript" language="javascript">
	...
	data_header[10][0] = "Action"
	data_header[10][1] = "hide";
	data_header[10][2] = "6%";
	
	/* data_header[11] = new Array;
	data_header[11][0] = "Skip Reason"
	...
	{	
		...
		var releaseButton = "<input type=\"button\" name=\"btn\" id=\"btn\" value=\"Release\" onclick=\"doPatientRelease(\'"+visitid+"\',\'opd\');\"  />";
		...
		<%-- data_arr[<%= i%>][12] ="<select name='skip<%=i%>' id='skip<%=i%>' ><option value='0'>Select</option><option value='E'>Emergency</option></select>"; --%>
	...
	formName = "opd_OP_Clinic_Waiting_PatientList"
...
var bReleaseClick='N';
	function openWindows(){
	    obj1 = eval('document.opd_OP_Clinic_Waiting_PatientList');
	    var url = "/hms/hms/opd?method=getOPClinicalReadViewList&"+csrfTokenName+"="+csrfTokenValue;
	    listSizeAlert();
	    obj1.action = url;
		obj1.submit();
 ...
 */	}
	...
	function validate(){
		...
		if(!chkInputLength(patientName))
			...
			submitForm('opWaitingList','/hms/hms/opd?method=showOPClinicwWaitingPatientListJsp&'+csrfTokenName+'='+csrfTokenValue);
		}
	}
						...
						if(msg.indexOf("success~~~true") != -1)
							{
									...
									bReleaseClick ='N';
									window.location ="opd?method=getOPClinicalWaitingList&selectedAppId=A1641";
	...
	function callWaitingList(){
	   ...
	   if(parkFlag=='p'){
		   submitProtoAjaxWithDivName('opWaitingList','/hms/hms/opd?method=getOPWaitingListRefresh&flag=p','test');
	   }else{
		   submitProtoAjaxWithDivName('opWaitingList','/hms/hms/opd?method=getOPWaitingListRefresh&searchFlag='+searchFlag+'&tokenNo='+tokenNo+'&patientName='+pName+'&uhid='+hinNo,'test');  
	   }
	...
	function showParkPatient(status) {
		...
		document.opd_OP_Clinic_Waiting_PatientList.action = "/hms/hms/opd?method=getOPClinicalWaitingList&flag="
			+ status;
	document.opd_OP_Clinic_Waiting_PatientList.submit();
	}
	...
	/* function bookCounterForDoctor(){
		...
		if(counterId!='0')
			submitProtoAjax('opd_OP_Clinic_Waiting_PatientList', action);
	} */
	...
	function transferToCommonPool(employeeId,page){
		var opCounter=document.getElementById("counterId").value;
		 ...
		 var url="/hms/hms/opd?method=showPlannedLeaveCommonPoolJsp&fromScreen=opd_OP_Clinic_Waiting_PatientList&"+csrfTokenName+"="+csrfTokenValue;
		 newwindow=window.open(url,"Common Pool","left=150,top=100,height=130,width=1050,status=1,scrollbars=1,resizable=0");
		 
}
	
	function showDoctorSpecificPatientList(){
		var url="/hms/hms/opd?method=getDoctorSpecificPatientList&"+csrfTokenName+"="+csrfTokenValue;
		newwindow=window.open(url,"My Patients","left=150,top=100,height=550,width=1050,status=1,scrollbars=1,resizable=0");
	}
	function stopRegistration()
	{
		if(confirm("Are You sure, You want to stop your registration for rest of the day?"))
			submitForm('opd_OP_Clinic_Waiting_PatientList_new','/hms/hms/opd?method=setVisitUptoTimeOfDoctor');
		else
			return false;
...
#searchresults1 .cmnTable table th{background:#bdd6ee;}
</style>
