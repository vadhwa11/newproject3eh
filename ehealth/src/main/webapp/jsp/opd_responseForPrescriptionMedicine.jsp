...
/* System.out.println("medicineDepartment"+medicineDepartment); */
...
<!-- <table border="0" align="center" cellpadding="0" cellspacing="0" id="grid"  >
 <tr>
	...
	%>
	 <tr>
		<td>
			<input type="checkbox" class="radioCheck" id="itemRadioCheck<%=incr%>" name="itemRadio<%=incr%>"  onchange="checkPrescriptionCheck(<%=incr %>)"  tabindex="31"/>
		...
		<input  type="text" class="nomenclaturepTab textYellow largTextBoxOpd" tabindex="32"	id="nomenclature<%=incr%>" size="35" name="nomenclaturepTab<%=incr%>" value="<%=str%>"  title="<%=str%>"
			onblur="populatePVMS(this.value,'<%=incr%>');populatePVMSTab(this.value,'<%=incr%>');checkItem('<%=incr%>');copyToPrescriptionTAb('<%=incr%>','opconsult'); ValidateCantra();displayAu(this.value,'<%=incr%>');" style=width:160px;"/>
			<script type="text/javascript">
				validatePrescriptionAutocompleteForTemplate('<%=str%>','<%=incr%>',this);
			</script>
		</td>
			<input type="hidden" name="pvmsNopTab<%=incr %>" tabindex="33" id="pvmsNo<%=incr %>" value="<%=pvmsNo %>"	size="10" readonly="readonly" />
		...
		<td><input class="textYellow opdTextBoxTSmall" type="text" name="dosagepTab<%=incr%>"  id="dosage<%=incr%>" size="10"  maxlength="45" tabindex="34" value="<%=temp.getDosage()%>" onblur="fillValue(this.value,<%=incr%>);checkFrequencyForTaperedDrugs(<%=incr%>)" validate="dosage,float,no"/>
		<div id="testDiv<%=incr%>">
		</div>
		</td><td>
		...
		<select style="width:69px;background: #FFFF99" name="frequencypTab<%=incr%>" id="frequency<%=incr%>"   tabindex="36" onchange="getFrequencyValue(this.value,<%=incr%>);fillValue(this.value,<%=incr%>);displaySOSQty(this.value,<%=incr%>);displaFrequencyType(this,<%=incr%>);checkFrequencyForTaperedDrugs(<%=incr%>);" >
			<option value="0">Select</option>
			 <%
			  int freqCnt = 0;
		...
		<td>
			...
			<input type="text" name="noOfDayspTab<%=incr%>"	id="noOfDays<%=incr%>"  <%=temp.getDuration()!=null?"readonly":"" %> class="textYellow opdTextBoxTSmall" size="3" maxlength="3" validate="No Of Days,num,no" tabindex="37" value="<%=temp.getNoofdays()%>" onblur="fillValueDays(<%=incr%>);fillValue(this.value,<%=incr%>);"/>
		...
		<td><select style="width:90px;background: #FFFF99" name="instrunctionpTab<%=incr%>" tabindex="38" id="instrunction<%=incr%>" >
				 <option value="0">Select</option>
				<%
			...
			<input type="hidden" name="totalpTab<%=incr%>" id="total<%=incr%>" value="<%=totals %>" size="5" class="textYellow opdTextBoxTSmall" validate="Total,num,no" readonly="readonly" tabindex="40"/>
