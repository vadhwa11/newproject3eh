...
serverdate = '<%=date+"/"+month1+"/"+year1%>'
    ...
    var start= new Date(document.getElementById("fromDate").value.split("/")[2], document.getElementById("fromDate").value.split("/")[1], document.getElementById("fromDate").value.split("/")[0]);
     ...
     if(start<=end){
    	 window.location.href='procurement?method=showPendingCommercialApprovalJsp&fromDate='+document.getElementById("fromDate").value+'&toDate='+document.getElementById("toDate").value;
         return true;
...
<div class="titleBg">
<h2>Pending List For Commercial Approval</h2>
</div>
<!-- thread search menu -->
<div class="Block">
...
<div class="clear"></div>
<input name="button"  type="button"	value="Search" class="button"	onclick="submitForm('searchPanel','procurement?method=showPendingCommercialApprovalJsp')" />
...
<table width="100%" colspan="7" id="itemDetails" border="0" cellpadding="0" cellspacing="0">
	...
	{ %><thead>
        ...
        <tr onclick="submitForm('popendingList<%=sn%>', 'procurement?method=showCommercialApprovalJsp')" style="cursor: pointer;"><td><%=sn %>
	...
	<%for(MmInspectionReport spm:inspectionReports) {%>
		 ...
		 <form name="popendingList1<%=sn %>" method="post">
		  <input type="hidden" name="quotationRequisitionNo" value="<%=spm.getRequest().getId() %>@R" />
		    ...
		    <tr onclick="submitForm('popendingList1<%=sn%>', 'procurement?method=showCommercialApprovalJsp')" style="cursor: pointer;">
			<td><%= sn%></td>
