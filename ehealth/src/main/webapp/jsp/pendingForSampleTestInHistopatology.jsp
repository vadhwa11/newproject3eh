...
<form name="patientSearch" action="" method="post">
	...
	<%   
		...
		if(session.getAttribute(RequestConstants.APP_ID_HISTO) != null){
			appId=(String) session.getAttribute(RequestConstants.APP_ID_HISTO);
		}
	%>
	<script type="text/javascript">
	<%
	...
	<div class="titleBg">
		<h2>Pending for Sample Grossing</h2>
	</div>
	<%}else if("A1657".equalsIgnoreCase(appId)){ %>
	<div class="titleBg">
		<h2>Pending for Sample Blocking</h2>
	</div>
	<%}else if("A1662".equalsIgnoreCase(appId)){ %>
	<div class="titleBg">
		<h2>Pending for Sample Sliding</h2>
	</div>
	<%}else if("A1663".equalsIgnoreCase(appId)){ %>
	<div class="titleBg">
		<h2>Pending for Sample Staining</h2>
	</div>
	<%} %>
	<div class="clear"></div>
	...
	<input type="button" name="save" id="addbutton"
		onclick="submitForm('patientSearch','/hms/hms/lab?method=getsampleforhistopatologyteststate&appId=<%=appId%>');"
...
<div id="searchresults" tabindex="2">
	...
	<form name="pendingSampleForHisto" method="post" action="">
 <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
	</form>
	...
	<script type="text/javascript">
	formFields = [
			[0, "<%= HIN_ID%>", "id"],[1,"diagDate"],[2,"diagTime"], [3,"hin"],[4,"ipNo"], [5,"patName"],[6,"orderBy"],[7,"age"],[8,"doctorName"],[9,"ward"],[10,"priority"]];
	 statusTd = 5;
	</script>
</div>
 <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
data_arr = new Array();
	...
	String hinNo = "";
		...
		%>
			...
			data_arr[<%= counter%>][7] = "<%= dgSampleCollectionHeader.getHin().getAge()%>"
				<%String doctorName="";
			if(dgSampleCollectionHeader.getOrder().getPrescribedBy().getFirstName()!=null){
				doctorName=doctorName.concat(dgSampleCollectionHeader.getOrder().getPrescribedBy().getFirstName()+" ");
			}
			...
			%>
			data_arr[<%= counter%>][8] = "<%=doctorName%>"
			data_arr[<%= counter%>][9] = "Male Medical Ward" 
				<% 
				String status="";
    ...
    formName = "pendingSampleForHisto"
