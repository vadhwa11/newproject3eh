...
serverdate = '<%=date+"/"+month1+"/"+year1%>'
    ...
    var start= new Date(document.getElementById("fromDate").value.split("/")[2], document.getElementById("fromDate").value.split("/")[1], document.getElementById("fromDate").value.split("/")[0]);
     ...
     if(start<=end){
    	 window.location.href='procurement?method=showPendingListForCreatingPOWOJsp&fromDate='+document.getElementById("fromDate").value+'&toDate='+document.getElementById("toDate").value;
         return true;
     }else{alert("Date is Incorrect.");return false;}
}

function openPopupWindow(obj){
		 var url="/hms/hms/procurement?method=directPoCreation";
		 newwindow=window.location.replace(url);
		}
</script>
<%
	Map map= new HashMap();
...
<div class="titleBg">
<h2>Pending List For Creating Purchase Order or WorkOrder</h2>
</div>
<div class="clear"></div>
...
<input name="button"  type="button"	value="Search" class="inputButtonAutu"	onclick="submitForm('searchPanel','procurement?method=showPendingListForCreatingPOWOJsp')" />
<input name="button"  type="button"	value="Create PO Direct Purchase" class="inputButtonAutu" onclick="openPopupWindow(this)" />
...
<table width="100%" colspan="7" id="itemDetails" border="0" cellpadding="0" cellspacing="0">
	<%int sn=1;
	if(vendor.size()>0 || inspectionReports.size()>0)
	{ %><thead>
		<tr>
           ...
           for(PrqQuotationVendorDetails vendorDetails:vendor){ %>
        ...
        <tr onclick="submitForm('popendingList<%=sn%>', 'procurement?method=showPOCreationJsp')" style="cursor: pointer;"><td><%=sn %>
        </td><td><%=HMSUtil.changeDateToddMMyyyy(vendorDetails.getHeader().getQuotationDate())%></td>
        <td><%= vendorDetails.getHeader().getQuotationNo()%></td>
        <td><%=vendorDetails.getVendor().getSupplierName()%></td> 
        </tr>
...
<!-- 	</thead> -->
	...
	<%for(MmInspectionReport spm:inspectionReports) {%>
		 ...
		 <form name="popendingList1<%=sn %>" method="post">
		  <input type="hidden" name="requitionId" value="<%=spm.getId() %>" />
		     <input type="hidden" name="flag" value="R"/>
		    <tr onclick="submitForm('popendingList1<%=sn%>', 'procurement?method=showPOCreationJsp')" style="cursor: pointer;" >
			<td><%= sn%></td>
			<td><%= HMSUtil.changeDateToddMMyyyy(spm.getRequest().getRequestDate())%></td>
			<td><%= spm.getRequest().getServiceRequestNo()%></td>
			<td><%= spm.getVendor().getSupplierName()%></td>
		</tr>
 <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
