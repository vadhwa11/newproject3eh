...
%>
	...
	<div class="Block">
		...
		<label><span>*</span> From Date</label> <input type="text"
			...
			value="<%=!fromDate.equals("")?fromDate:currentDate%>" readonly="readonly"
		...
		<label style=""><span>*</span>To Date</label> <input type="text" id="ToDateId"
			name="<%=!toDate.equals("")?toDate:currentDate%>" class="date"
		...
		<%} %>
		<label><%=prop.getProperty("com.jkt.hms.registration_no") %></label> <input
			type="text" name="<%=HIN_NO %>" value="<%=!hinNo.equals("")?hinNo:"" %>" MAXLENGTH="50" onkeypress="return searchKeyPress(event);"/>
		<div class="clear"></div>
		<label>Patient Type</label> <select name="<%=PATIENT_TYPE%>"
...
<select id="departmentId" name="<%=DEPARTMENT_ID %>">
	...
	<%}%>
		...
		<div class="clear"></div>
		<label>Patient Name</label> <input type="text"
			name="<%=P_FIRST_NAME %>" value="<%=!pFirstName.equals("")?pFirstName:"" %>" MAXLENGTH="30" onkeypress="return searchKeyPress(event);"/>
...
<select id="subChargeCodeId"
			name="<%=SUB_CHARGECODE_ID %>" onchange="bookLabForDoctor('resultEntry');">
			<option value="0">Select</option>
			<% for(MasSubChargecode subChargecode : subChargeCodeList){ %>
...
<% } %>		
	...
	<div class="clear"></div>
	<input type="button" value="Search" class="button" onclick="getResultEntryPendingList();" accesskey="a" id="submitButtonId" />
		<!-- onclick="submitForm('patientSearch','/hms/hms/investigation?method=searchPatientForLab&search=ResultEntry','checkFromNTodata','chkPatientTypeForBillno');" -->		
...
function getResultEntryPendingList(){
	var url = '/hms/hms/investigation?method=getPendingResultEntryList';
	var jsonData = '';
