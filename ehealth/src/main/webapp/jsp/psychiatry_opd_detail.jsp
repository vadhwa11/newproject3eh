...
<%@page import="jkt.hms.masters.business.MlcRefered"%>
...
<script type="text/javascript">
	...
	serverdate = '<%=getDate+"/"+month+"/"+year%>';
			...
			<%-- <%
				Map<String, Object> mapOfMLCRef = new HashMap<String, Object>();
				if(mapOfMLCRef.get("listOfMlcRefered")!=null){
					mapOfMLCRef.get("listOfMlcRefered");
				}
			%> --%>
...
<%
	...
	if(map.get("listOfMlcRefered")!=null){
		listOfMlcRefered = (List<MlcRefered>)map.get("listOfMlcRefered");
	}
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
	...
	<div class="titleBg">
		<h2>PATIENT DETAILS 09122017</h2>
	</div>
	<div class="clear"></div>
...
<div class="clear"></div>
<textarea class="historyAutoComplete" name="presentComplain" id="presentComplain"  cols="0" rows="0" maxlength="300"><%=presentComplaintHistory%></textarea>
</div>
...
<div class="clear"></div>
<textarea  name="pastIllness" class="historyAutoComplete"  id="pastIllness" cols="0" rows="0" maxlength="300"  ><%=opdPatientHistoriesStr %></textarea>
</div>
						...
						<div id="divTemplet" style="width:314px; height:78px; margin-left:5px; overflow:scroll; overflow-x:hidden;">
							<table style="width:100%; border-top: solid 1px #C0C0C0" id="alergyGrid">
								<tr>
									...
									<!-- <th style="background: rgb(189, 214, 238);">Severity</th>
									<th style="background: rgb(189, 214, 238);">Since:Month</th>
									<th style="background: rgb(189, 214, 238);">Since:Year</th>
									<th style="background: rgb(189, 214, 238);">Status</th> -->
								</tr>
								<% incr=0 ;
	 len=1;
		...
		<%-- <script type="text/javascript" language="javascript" charset="utf-8">
					...
					MasSeverityCode  masSeverityCode = new MasSeverityCode();
	     			...
	     			saverityCodeArray[<%=i%>][1] = "<%=masSeverityCode.getSeverityName()%>";
									...
									<td><input class="small textYellow" name="allergyRemarks" maxlength="30"/></td>
								</tr>
								<%} %>
							</table>
		...
		<%-- 		 <div class="Block">
					...
					<div class="clear"></div>
						<div id="divTemplet">
							<table style="width: 100%; border-top: solid 1px #C0C0C0" id="alergyGrid">
								<tr>
									<th style="background: rgb(189, 214, 238);">&nbsp;</th>
									<th style="background: rgb(189, 214, 238);">Type</th>
									<th style="background: rgb(189, 214, 238);">Allergen</th>
									<th style="background: rgb(189, 214, 238);">Severity</th>
									<th style="background: rgb(189, 214, 238);">Since:Month</th>
									<th style="background: rgb(189, 214, 238);">Since:Year</th>
									<th style="background: rgb(189, 214, 238);">Status</th>
...
<label>Presenting Complaints</label>
<div class="clear"></div>
<textarea class="large historyAutoComplete" name="presentComplain" id="presentComplain" style="width:530px;height:47px; margin-left:4px;" cols="0" rows="0" maxlength="300"><%=presentComplaintHistory%></textarea>
<div class="clear"></div>
<div class="clear"></div><div class="clear"></div>
<div class="clear"></div>
<label>History of Presenting Illness</label>
<div class="clear"></div>
<textarea  name="pastIllness" class="large historyAutoComplete"  id="pastIllness" cols="0" rows="0" maxlength="300" style="width:530px;height:47px; margin-left:4px;" ><%=opdPatientHistoriesStr %></textarea>
</div> --%>
<!-- 
		...
		<table style="width:1157px; border-top:1px solid #C0C0C0; float:left;">
		  <tr>
		    ...
		    <input type="text"  id="temperature" name="temperature" value="<%=temperature>0f?temperature:"" %>" maxlength="3"  class="textYellow allownumericwithdecimal textSmall" style="width:50px;" class="textYellow allownumericwithoutdecimal textSmall" />
		    <label class="smallAuto">&nbsp;&deg;F&nbsp; </label></td>
		    <td>
		<label class="smallAuto">BP:</label><input name="systolic" id="systolic"  placeholder="Systolic" value="<%=parts.length>0?parts[0]:noSysDias%>" validate="systolic,int,no" maxlength="3" class="textSmall allownumericwithoutdecimal" type="text" maxlength="5"> <label class="smallAuto">/</label> <input name="diastolic" id="diastolic" placeholder="Diastolic" value="<%=parts.length>0?parts[1]:noSysDias%>" validate="diastolic,int,no" maxlength="3" class="textSmall allownumericwithoutdecimal" type="text"> <label class="smallAuto">mm Hg</label>
		</td>
		    <td>
		    <label class="smallAuto">HR:</label>
		<input type="text"  id="pulse" name="pulse" value="<%=pulse>0?pulse:"" %>"  class="textYellow allownumericwithdecimal textSmall" maxlength="3" style="width:50px;" class="textYellow allownumericwithoutdecimal textSmall"/>
		<label class="smallAuto">/ min</label>
		</td> 
		
<!-- 		</table>
		
		<table style="width:407px; border-top:1px solid #C0C0C0; float:left;"> -->
	...
	<td><label class="smallAuto">Wt:</label> <input name="weight"
			...
			<td><label class="smallAuto">Ht:</label><input name="height"
				...
				class="allownumericwithoutdecimal textSmall" value="<%=height>0?height:"" %>"
				validate="height,int,no" onblur="calculateBMI();"  /> <label
	...
	<td>	
		...
		<input class="textYellow allownumericwithdecimal textSmall" name="respiratoryRate" id="respiratoryRate" type="text" maxlength="3" validate="Respiratory Rate,float,no" value="<%=respirtoryRate>0?respirtoryRate:"" %>">
		<label class="smallAuto">/ min</label>
		</td>
		<td>
		<label class="smallAuto">SPO2</label>
		<input class="textYellow allownumericwithdecimal textSmall" name="spo2" id="spo2" type="text" maxlength="3" validate="spo2,float,no" value="<%=spo2>0?spo2:"" %>" onkeypress="return isNumberOnly(event)">
		...
		<div class="clear"></div> 
			...
			<div class="clear"></div> 
			    ...
			    <div class="tabContWidth3">
					...
					<textarea name="mental_status" id="mental_status"
						cols="0" rows="0" maxlength="256" tabindex="26"
					 class=""></textarea>
			    </div>
			    <div class="tabContWidth3">
			...
			<%-- <div class="tabContWidth3">
				...
				<textarea id="localExamination" name="localExamination" cols="0" rows="0" maxlength="300" tabindex="26" class="historyAutoComplete"><%=opdPatientDetails!=null && opdPatientDetails.getLocalExamination()!=null && opdPatientDetails.getVisit().getId().equals(visitId)?opdPatientDetails.getLocalExamination().trim():"" %></textarea>
			...
			<div class="tabContWidth_diagnosis">
			...
			<!-- <textarea name="" id="" cols="0" rows="0" maxlength="300" id="snomed" name="snomed"	onblur="getICDListBasedOnSnomedId();"
						...
						<textarea   class="textYellow historyAutoComplete" id="snomed" <%=pDiagnosis!=null?"style='width:276px;'":""%> name="snomed"></textarea>
						 <%if(pDiagnosis!=null){%>
			...
			<textarea name="finalDiagnosis" id="finalDiagnosis" <%=fDiagnosis!=null?"style='width:276px;'":""%>maxlength="300" class="textYellow historyAutoComplete ui-autocomplete-input"> </textarea>
				 <%if(fDiagnosis!=null){%>
			...
			<div class="tabContWidth_diagnosis">
			<label>Deferred Diagnosis</label>
			<textarea name="deferredDiagnosis" id="deferredDiagnosis" <%=dDiagnosis!=null?"style='width:276px;'":""%>maxlength="300" class="textYellow historyAutoComplete ui-autocomplete-input"> </textarea>
				 <%if(fDiagnosis!=null){%>
						<textarea  readonly="readonly" style="width:276px;"><%=dDiagnosis%></textarea>
						<%}%>
			</div>
			
			<div class="tabContWidth_diagnosis">
			<label>Differential Diagnosis</label>
			<textarea name="differentialDiagnosis" id="differentialDiagnosis" <%=diffDiagnosis!=null?"style='width:276px;'":""%>maxlength="300" class="textYellow historyAutoComplete ui-autocomplete-input"> </textarea>
				 <%if(fDiagnosis!=null){%>
...
<input
	...
	<div style="width: 832px; float: left;" id="labInvDiv">
		...
		<%-- 		<td><input type="text" class="textYellow largTextBoxOpd popper"
					data-popbox="pop1" style="width: 672px;" value=""
					id="chargeCodeName<%=inc %>" size="65"
				  ...
				  new Ajax.Autocompleter('chargeCodeName<%=inc %>','ac2update<%=inc %>','opd?method=getInvestigationListForAutoComplete',{minChars:3,
					  callback: function(element, entry) {
				            return entry + '&labradiologyCheck=' + document.getElementById('investigationCategory')!=null?document.getElementById('investigationCategory').value:"";
				        },
					  parameters:'requiredField=chargeCodeName<%=inc %>&fromOpd=fromOpd'});
				</script></td> --%>
...
<input type="button" class="buttonDel" value=""
							...
							onclick="addRowMedicineForLP();" value="" /></div> -->
 ...
 <div class="">
	...
	for(;incrs< lens;incrs++,inxRows++){
		...
		if(patientOtherPrescriptionDetails.size()>0 && masStoreOutItemsList.size()>0){
			...
			/* nomeclatures=pds.getItem().getNomenclature(); */
...
<div style="width:-26px;background-color:floralwhite;float: right;">
  ...
  <div class="paddingTop25">
	...
	<div class="Block">
	<!-- @ START code added by rajdeo -->
	<div class="titleBg">
		<h2>Non-pharmacological advice</h2>
	</div>
	<div class="paddingTop5"></div>
	<textarea class="large" validate="N/P,string,no" id="nonPharmacological" name="nonPharmacological"
			cols="0" rows="0" maxlength="300" tabindex="26"
			style="width: 1159px; height: 46px;overflow-x:hidden;resize:vertical;"><%=nonPharmacological!=null?nonPharmacological:""%>
	</textarea>
	<div class="clear"></div>
	<div class="paddingTop5"></div>
...
<div class="clear"></div>
	...
	<textarea class="large" name="opdRemarks" id="opdRemarks"
		...
		maxlength="300" ><%=opdPatientDetails!=null && opdPatientDetails.getOpdRemarks()!=null && opdPatientDetails.getVisit().getId().equals(visitId)?opdPatientDetails.getOpdRemarks():"" %></textarea>
	
	<!-- code added by rajdeo mlc/MLC -->
	<div class="paddingTop5"></div>		
						<label class="hide">MLC
						  <%if(opdPatientDetails!=null && opdPatientDetails.getMlcType()!=null && opdPatientDetails.getMlcType().equalsIgnoreCase("y") && opdPatientDetails.getVisit().getId().equals(visitId)) {%>
							<input type="checkbox" checked="checked" class="checkboxMargin"	id="mlc" name="mlc" onchange=""/>
							<%}else{
							 if(opdPatientDetails!=null && opdPatientDetails.getMlcStatus()!=null && opdPatientDetails.getMlcStatus().equalsIgnoreCase("y") && opdPatientDetails.getVisit().getId().equals(visitId)) {
								%>
							<input	type="checkbox" class="checkboxMargin" id="mlc" name="mlc" disabled="disabled"/><%}else{ %>
							<input	type="checkbox" class="checkboxMargin" id="mlc" name="mlc" onclick="displayMLCRefered();"/>
							<%} }%>
						</label>	
			
			
								
		<div id="mlcRefered_div" style="display: none;">
		<label class="auto">MLC Refered</label>
		<select name="mlcRefered" id="mlcRefered" onchange="getTextBox(this)">
		
		
		<%-- <%for(MlcRefered mlcRefered:listOfMlcRefered){
			%>
			<option value="<%=mlcRefered.getId()%>" selected="selected"><%=mlcRefered.getMlcReferedFrom()%></option>
			<%} %> --%>
		
		<%-- <c:forEach items="${listOfMlcRefered}" var="listOfMlcRefered">
			<option value="{listOfMlcRefered}">${listOfMlcRefered.getMlcReferedFrom}</option>
		</c:forEach> --%>
			...
			<%
				...
				for(Object[] patientHistory :previousDetailList){
					...
					if(patientHistory[10] != null && ((Integer)patientHistory[10])!=0){
						findings+="Vitals: Height-"+(Integer)patientHistory[10];
					}
...
function addRowForAllergy() {
	...
	var e1 = document.createElement('input');
	e1.type = 'text';
	e1.name = 'allergyRemarks' + iteration;
	e1.id = 'allergyRemarks' + iteration;
...
.tabContWidth_diagnosis label{width:265px; background:none;box-shadow:none; margin:0px 0px 0px 4px; background:#ffd966;}
.tabContWidth_diagnosis textarea { width:265px ;height:35px; margin-left:4px;overflow-x:hidden;resize:vertical;}
