...
<form name="relievingWaiting" method="post" action="">
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
...
<div id="searchtable" tabindex=2></div>
	<script type="text/javascript" language="javascript">
		formFields = [
						...
						,[5,"curr_inst"],[6,"trans_inst"],[7,"status"]
					];
				statusTd =7;
	</script>
...
<script type="text/javascript" language="javascript">
	...
	data_header[5][0] = "Current Institution"
	data_header[5][1] = "data";
	data_header[5][2] = "5%";
	data_header[5][3] ="curr_inst"

	data_header[6] = new Array;
	data_header[6][0] = "Transfer Institution"
	...
	data_header[6][3] ="trans_inst"
		...
		data_header[8][0] = "Joining_Date"
		data_header[8][1] = "hide";
		data_header[8][2] = "5%";
		data_header[8][3] ="Joining_Date" */
	...
	Iterator iterator=hrTransferApplicationMList.iterator();
	
	while(iterator.hasNext())
	{
		HrTransferApplicationM htam= (HrTransferApplicationM) iterator.next();
		...
		{
		%>
		data_arr[<%= i%>][2] = "<%=htam.getEmployee().getEmployeeName()%>"
		<%
		}else{
		%>
		data_arr[<%= i%>][2] = ""
		...
		{
		%>
		data_arr[<%= i%>][5] =  "<%=htam.getEmployee()!= null && htam.getEmployee().getEmployeeDepartment().getEmpDeptName()!= null && htam.getEmployee().getEmployeeDepartment().getEmpDeptName()!= null?htam.getEmployee().getEmployeeDepartment().getEmpDeptName():""%>"
		<%
		}else{
		%>
		data_arr[<%= i%>][5] = ""
		...
		{
		%>
		data_arr[<%= i%>][6] = "<%=htam.getEmployee().getHospital().getHospitalName()%>"
		<%
		}else{
		%>
		data_arr[<%= i%>][6] = ""
		...
		%>
		data_arr[<%= i%>][7] ="<%=approved.getTransferInstitute().getHospitalName()%>"
		<%}}
		}else{
		%>
		data_arr[<%= i%>][7] = ""
		...
		data_arr[<%= i%>][8] = "Active"
		<%}else if(htam.getStatus().equalsIgnoreCase("a")){%>
		...
		<%-- <% if(htam.getEmployee() != null)
		{
			for(HrTransferApproved approved:hrTransferApprovedList){
					if(htam.getId() == approved.getTransferApp().getId()){
		%>
		data_arr[<%= i%>][9] ="<%=HMSUtil.getDateFormat(approved.getJoiningDate(),"dd/MM/yyyy")%>"
		<%}}
		}else{
		%>
		data_arr[<%= i%>][9] = ""
