...
<table  width="100%" border="0" id="accId" cellspacing="0" cellpadding="0">
	...
	%>
		<tr>
			<td>
			<input type="hidden" name="<%= CHEQUE_DATE+i %>" value=""/>
			</td>
			
			<td><input type="hidden" id="vType<%=i  %>" name="<%=VOUCHER_TYPE+i  %>" value=""/>
			<input type="hidden" name="<%=VOUCHER_HD_ID+i %>" value=""/>
			...
			<select name="<%=CHEQUE_NO+i %>" id="chequeNoId<%=i %>" disabled="disabled" onblur="getDetailsForCheckNo(this.value,<%=i%>);">
			...
			<td><input type="text"  name="clearingDate<%=i %>" value="" validate="Clearing Date,string,no" class="calDate" MAXLENGTH="30" readonly="readonly" />
