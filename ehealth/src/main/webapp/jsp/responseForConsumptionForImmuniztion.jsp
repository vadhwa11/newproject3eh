...
<%if(storeReservationMList.size()>0){
	for(StoreReservationM reservationM :storeReservationMList){
		...
		if(reservationM.getId().equals(reservationNoList.get(0).getId())){	
		
	%>
	<option value="<%=reservationM.getId()%>" selected="selected"><%=reservationM.getGroup().getGroupName()%></option>
<%}}else{ %>
<option value="<%=reservationM.getId()%>" selected="selected" ><%=reservationM.getGroup().getGroupName()%></option>
<%}} }%>
</select>
<input type="hidden" name="storeReservationMId" id ="storeReservationMId" value="<%=reservationNoList.get(0).getId()%>">
<div class="clear"></div>
</div>
<div class="clear"></div>
...
<div class="cmntable">	
		...
		<input type="hidden" value="<%=storeReservationT.getItem()!= null?storeReservationT.getItem().getId():"" %>" name="itemId" id="itemId<%=i %>" />
		<input type="hidden" value="<%=storeReservationT.getItem()!= null && storeReservationT.getItem().getExpiryDays() != null?storeReservationT.getItem().getExpiryDays():"" %>" name="expiryDays" id="expiryDays<%=i %>" /></td>
	...
	<td><input type="text" readonly="readonly" value="<%=storeReservationT.getItem()!= null?storeReservationT.getItem().getNomenclature():"" %>" tabindex="1" name="nomenclature" size="20" id="nomenclature<%=i %>" onblur="checkForDefectiveDrugs(this.value, 'nomenclature','<%=i %>');;"  />
		...
		<td><input type="text" size="6" tabindex="1" disabled value="<%=storeReservationT.getItem()!= null && storeReservationT.getItem().getItemConversion()!= null?storeReservationT.getItem().getItemConversion().getItemUnitName():"" %>" name="au" id="au<%=i %>" class="readOnly" readonly="readonly" />
		<input type="text" size="6" tabindex="1"  value="<%=storeReservationT.getItem()!= null && storeReservationT.getItem().getItemConversion()!= null?storeReservationT.getItem().getItemConversion().getId():"" %>" name="auId" id="auId<%=i %>" class="readOnly" readonly="readonly" /></td>
		...
		<td><input type="text" size="7" tabindex=1 readonly="readonly" value="<%=(storeReservationT.getReservedStock()!= null ?storeReservationT.getReservedStock().intValue()*10:"") %>" name="reservedStock" id="reservedStock<%=i%>"   />
		</td>
		
		<td><input type="text" size="7" tabindex=1 value="" maxlength="10" name="consumedQty" id="consumedQty<%=i%>" onblur="calculateReturnQty(<%=i %>);"  />
