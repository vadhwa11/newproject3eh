...
<select name="tempId" id="tempId"  multiple="multiple" size="3" class="listBig3">
 <%	if(mappingsList.size() >0){%>
				 ...
				 for (LocationParameterMapping parlia : mappingsList) {
					if(parlia.getParliyament()!=null){
				  %>
				  <option value="<%=parlia.getParliyament().getId()%>"><%=parlia.getParliyament().getName()%></option>
				  <%
					}	}
				   %>
               <% }%>
...
<input type="hidden" name="basicSec"  value="parliyament"/>
