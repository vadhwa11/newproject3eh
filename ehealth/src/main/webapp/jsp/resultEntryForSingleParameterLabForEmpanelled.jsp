...
%>
	...
	<div class="Block">
		...
		<input type="hidden" name="empanelledId" value="<%=masEmpaneled.getId() %>" id="resultEnteredById"/>
		<%-- <select
	...
	subCounter++;
		  ...
		  //break;
			...
			<tr>
				...
				<%}%>
				<td>
				<input type="text" name="<%=RESULT_SINGLE_VALUE %>"
					id="<%=RESULT_SINGLE_VALUE %><%=srNoCounter%>" value="" validate="Result,string,no"
					maxlength="10" />
	...
	<input type="button" name="Submit11" id="submitForDisable" tabindex="1"
		...
		onclick="submitForm('sampleCollection','investigation?method=submitResultEntryForAllInvestigationTypeForEmpanelled');"
		align="right" /> <input name="Button" type="reset"
		class="buttonHighlight" value="Reset" onclick="resetResult();" /> 
		
		<input
		name="Button" type="button" class="button" value="Case Sheet"
		onclick="printCaseSheet(<%=visitId %>,<%=inpatientId %>,'<%=hinNo %>');" />
