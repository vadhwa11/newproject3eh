...
function resetResult(){
    ...
    for(DgResultEntryHeader dgResultEntryHeader2 : resultList){
		...
		//	for(DgResultEntryDetail dgDetail : dgDtSet){
	    	if(dgDetail.getInvestigation().getInvestigationType().equals("s")){
				...
				<tr>
					<td><%=index+1 %>
					<input type="hidden" name="resultEntryDetailId" value="<%=dgDetail.getId()%>">
					</td> 
					<td width="7%">
						<%if(dgDetail.getInvestigation() !=null){
					...
					<%if(RequestConstants.UHID_FOR_QUALITY_TESTING.equalsIgnoreCase(dgResultEntryHeader.getHin().getHinNo())){%>
					<td><input type="text" name="<%=QC_RESULT_SINGLE_VALUE %><%=dgDetail.getId()%>"
		...
		System.out.println("*************** line number for 928 for multiple test");
		%>
				
				<jsp:include page="resultEntryTableForMultipleTestTypePostQC.jsp"
					flush="true">
 		...
 		index++;
				...
				<input type="hidden" name="counter" id="counter" value="<%=index%>" />
		</table>
	</div>
	...
	<input type="button" class="button" value="Submit"
		onclick="if(inputValidate()){submitForm('sampleCollection','investigation?method=submitPostResultEntryForAllInvestigationTypeQC')};"
