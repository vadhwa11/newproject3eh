 ...
 function chkLength(field,maxLimit)
{
			...
			for (var i=0;i < c;i++) {
				...
				var objForMultiple=document.getElementById('checkId'+i);
	  			if(obj !=null && obj.disabled== false){
	 				...
	 				//added by govind 30-07-2017
	 				document.getElementById('checkIdSingleValue'+i).value="y";
	  			    document.getElementById('checkIdSingleValueRetest'+i).value="n";
		  			document.getElementById('checkIdSingleValueRecollect'+i).value="n";
		  		   //added by govind 30-07-2017 end
	  			} 
	  				...
	  				document.getElementById("recollectAllButton").checked=false;
	  				//added by govind 30-07-2017
	  				document.getElementById('checkId'+i).value="y";
	  			    document.getElementById('checkIdRetest'+i).value="n";
		  			document.getElementById('checkIdRecollect'+i).value="n";
		  			//added by govind 30-07-2017
	  			}
			...
			for (var i=0;i< c;i++) {
				...
				var objForMultiple=document.getElementById('checkIdRetest'+i);
	  			if(obj !=null && obj.disabled== false){
	  				...
	  				document.getElementById('checkIdSingleValueRecollect'+i).checked=false; 
	  				//added by govind 30-07-2017
	  		 	    document.getElementById('checkIdSingleValueRetest'+i).value="y";
	  			    document.getElementById('checkIdSingleValue'+i).value="n";
		  			document.getElementById('checkIdSingleValueRecollect'+i).value="n";
	  				...
	  				document.getElementById('checkIdRecollect'+i).checked=false; 
	  				//added by govind 30-07-2017
	  				document.getElementById('checkIdRetest'+i).value="y";
		  			document.getElementById('checkId'+i).value="n";
		  			document.getElementById('checkIdRecollect'+i).value="n";
			...
			for (var i=0;i < c;i++) {
				...
				var objForMultiple=document.getElementById('checkIdRecollect'+i);
	  			if(obj !=null && obj.disabled== false){
	 				...
	 				document.getElementById("retestAllButton").checked=false;
	 			    //added by govind 30-07-2017
	 				document.getElementById('checkIdSingleValueRecollect'+i).value="y";
	  			    document.getElementById('checkIdSingleValue'+i).value="n";
		  			document.getElementById('checkIdSingleValueRetest'+i).value="n";
	  			}else if(objForMultiple !=null && objForMultiple.disabled== false){ 
	  				...
	  				document.getElementById('checkIdRetest'+i).checked=false;
	  				//added by govind 30-07-2017
	  				document.getElementById('checkIdRecollect'+i).value="y";
		  			document.getElementById('checkId'+i).value="n";
		  			document.getElementById('checkIdRetest'+i).value="n";
  ...
  function  accepted(chkObj, inc)
 {
 	 ...
 	 if(chkObj.id == "checkIdSingleValue"+inc){ 
 	 	...
 	 	//added by govind 30-07-2017
 	   document.getElementById('checkIdSingleValue'+inc).value="y";
		 document.getElementById('checkIdSingleValueRetest'+inc).value="n";
		document.getElementById('checkIdSingleValueRecollect'+inc).value="n";
 		...
 		document.getElementById('checkIdRetest'+inc).checked=false; 
 	 	...
 	 	//added by govind 30-07-2017
 	   document.getElementById('checkId'+inc).value="y";
		 document.getElementById('checkIdRetest'+inc).value="n";
		document.getElementById('checkIdRecollect'+inc).value="n";
  ...
  function  retest(chkObj, inc)
 	{
 		 if(chkObj.checked){
 		  if(chkObj.id == "checkIdSingleValueRetest"+inc){ 
 		   ...
 		   document.getElementById('checkIdSingleValueRecollect'+inc).checked=false;
 		   //added by govind 30-07-2017
 		  document.getElementById('checkIdSingleValueRetest'+inc).value="y";
 		 document.getElementById('checkIdSingleValue'+inc).value="n";
 		document.getElementById('checkIdSingleValueRecollect'+inc).value="n";
 		 ...
 		 document.getElementById('checkId'+inc).checked=false; 
		   document.getElementById('checkIdRecollect'+inc).checked=false;
		   //added by govind 30-07-2017
		   document.getElementById('checkIdRetest'+inc).value="y";
	 		 document.getElementById('checkId'+inc).value="n";
	 		document.getElementById('checkIdRecollect'+inc).value="n";
 	  }
 	 } 
  }  
  function  recollect(chkObj, inc)
	{
		 if(chkObj.checked){
		  if(chkObj.id == "checkIdSingleValueRecollect"+inc){ 
		    ...
		    document.getElementById('checkIdSingleValueRetest'+inc).checked=false;
		    //added by govind 30-07-2017
		    document.getElementById('checkIdSingleValueRecollect'+inc).value="y";
	 		 document.getElementById('checkIdSingleValue'+inc).value="n";
	 		document.getElementById('checkIdSingleValueRetest'+inc).value="n";
		  ...
		  document.getElementById('checkIdRetest'+inc).checked=false;
		  //added by govind 30-07-2017
		  document.getElementById('checkIdRecollect'+inc).value="y";
	 		 document.getElementById('checkIdRetest'+inc).value="n";
	 		document.getElementById('checkId'+inc).value="n";
	  }
	 }   
...
<%
	...
	try {
	 ...
	 //govind added by 26-10-2016
	 int myIndex=0;
	 if(map.get("index") != null){
		 myIndex =Integer.parseInt(""+map.get("index"));
...
%>
	...
	if(dgResultEntryHeader.getHin()!= null){
		...
		<div class="clear"></div>
		<%if(dgResultEntryHeader.getHin()!= null){ %>
		<label>Patient Age</label> <label class="value"><%= dgResultEntryHeader.getHin().getAge()%></label>
		<%}else{ 
		if(age==null){
			age="0";
		}
		%>
		<label>Patient Age</label> <label class="value"><%=age%></label>
		<%} %>
		...
		<%
		String bloodGroup="-";
		if(dgResultEntryHeader.getHin().getConfirmedStatus()!=null && dgResultEntryHeader.getHin().getConfirmedStatus().equalsIgnoreCase("y") && dgResultEntryHeader.getHin().getBloodGroup()!=null &&
				dgResultEntryHeader.getHin().getBloodGroupValue()!=null){
			bloodGroup=dgResultEntryHeader.getHin().getBloodGroupValue();
		}
		%>
		...
		<%  
			...
			List<DgSampleCollectionDetails> collDetList=new ArrayList<DgSampleCollectionDetails>();
				collDetList.addAll(dgResultEntryHeader.getSampleCollectionHeader().getDgSampleCollectionDetails());
				...
				for(DgSampleCollectionDetails col:collDetList){
					...
					if(dgResultEntryHeader.getHin().getId()==col.getSampleCollectionHeader().getHin().getId()){
						if(SubChargeId==col.getSubcharge().getId()){//added by govind 07-07-2017
		...
		<%--//commented by govind 21-06-2017 end <label>Sample Id</label> <label class="value"><%=new ArrayList<DgResultEntryDetail>(dgResultEntryHeader.getDgResultEntryDetails()).get(0).getSampleCollectionDetails().getSubcharge().getSubChargecodeCode()
		+"/"+new ArrayList<DgResultEntryDetail>(dgResultEntryHeader.getDgResultEntryDetails()).get(0).getSampleCollectionDetails().getDiagNo()%></label>
		--%>
		<label>Sample Id</label> <label class="value"><%=sampleId%></label>
...
function inputValidate(){
	...
	if(validatedCheckBoxCountSingle > 0 || validatedCheckBoxCountMultiple > 0){
		...
		for(var j = 0 ;j<validatedCheckBoxCountMultiple ;j++){
			...
			if(document.getElementById('checkId'+j)!=null){ //added by govind
...
function resetResult(){
	...
	for(var i =0; i<=(counter-1); i++){
		...
		document.getElementById('smsButton'+i).checked = false;
	}
	document.getElementById('addbutton').checked = false;
	document.getElementById('retestAllButton').checked = false;
	document.getElementById('recollectAllButton').checked = false;	
	...
	<div class="Block">
		...
		<input type="hidden" name="<%=RESULT_VALIDATED_BY %>" id="<%=RESULT_VALIDATED_BY %>"
			value="<%=empId%>" />
    ...
    <!-- added by govind 26-10-2016-->
    <%if(dgResultEntryHeader2.getResultType() != null
				&& dgResultEntryHeader2.getResultType().equalsIgnoreCase("v")){ 
    	DgResultEntryDetail dgDetail = dgResultDtSet.iterator().next();	
    	String resultid="",investName="",sampid="",sampDes="";
    	request.setAttribute("dgDetailForMultipleTestType",
				dgDetail);
    	myIndex=myIndex+1;
    	indexForMultiple=myIndex;
				%>
					<jsp:include page="resultValidationForMultipleTestType23.jsp"
					flush="true">
					<jsp:param name="resultEntryIndex" value="<%=myIndex%>" />
		...
		//	for(DgResultEntryDetail dgDetail : dgDtSet){
	    	if(dgDetail.getInvestigation().getInvestigationType().equals("s")){
				...
				<tr>
					...
					<td width="7%">
					<input name="collectionId<%=indexForSingle%>" value="<%=dgDetail.getId() %>" type="hidden">
						<%if(dgDetail.getInvestigation() !=null){
				    ...
				    try{
					    ...
					    <input name="collectionId<%=indexForSingle%>" value="<%=dgDetail.getId() %>" type="hidden">	<!-- added by govind 24-06-2017 -->
					    		<input
						name="<%=RESULT_SINGLE_VALUE %>" tabindex="1"
						onkeyup="submitAllExceptEnter(this,event,'','<%=index+1 %>');"
						id="<%=RESULT_SINGLE_VALUE %><%=index+1 %>" value="<%=result %>" readonly="readonly"/>
						<% }else{ %>
						<input name="collectionId<%=indexForSingle%>" value="<%=dgDetail.getId() %>" type="hidden"><!-- added by govind 24-06-2017 -->	
						 <input name="<%=RESULT_SINGLE_VALUE %>" tabindex="1"
						onkeyup="submitAllExceptEnter(this,event,'','<%=index+1 %>');"
						...
						value="<%=dgDetail.getResult()%>" readonly="readonly"/> <%}
					       }else{ %>
					       <input name="collectionId<%=indexForSingle%>" value="<%=dgDetail.getId() %>" type="hidden">	<!-- added by govind 24-06-2017 -->
					        <input name="<%=RESULT_SINGLE_VALUE %>"
						tabindex="1"
						onkeyup="submitAllExceptEnter(this,event,'','<%=index+1 %>');"
						id="<%=RESULT_SINGLE_VALUE %><%=index+1 %>"
						value="<%=dgDetail.getResult()%>" readonly="readonly"/> <% }
					...
					class="radioCheck" value="yes" id="smsButton<%=indexForSingle%>" 
					align="right" /></td>
		...
		%>
				<tr>
					...
					<td><input name="resultIdTemplate"
						...
						<input name="dgresultDetailIdTemplate"
						id="dgresultDetailIdTemplate<%=index%>" type="hidden"
						value="<%=dgDetail.getId()%>"> </input> 
		...
		System.out.println("*************** line number for 928 for multiple test"+index);
		      ...
		      if(multEnter==0){
				if(indexForSingle>0){
					indexForMultiple=indexForSingle;
				}
		       }multEnter++;
				System.out.println("indexForMultiple "+indexForMultiple);//added by govind 30-07-2017end
		%>
	...
	<input type="button" class="button" value="Submit"
		onclick="if(inputValidate()){submitForm('sampleCollection','investigation?method=submitResultValidationForAllInvestigationTypeLab')};"
		align="right" /> <input name="Button" type="reset"
		class="buttonHighlight" value="Reset" /> <input
...
function submitAllExceptEnter(myfield,e,url,inc)
{
	...
	//		return true;
		//}else{
			// document.getElementById('<%=RESULT_SINGLE_VALUE %>'+inc).value = ''; //commented by govind 14-06-2017
			return false;
		//}
	//}
}
...
function goBack(){
 	window.location.href='/hms/hms/investigation?method=showPendingForResultValidationLabJsp&selectedAppId=A1689&childAppId=A353';
 }
