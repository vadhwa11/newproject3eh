...
<tr>
 ...
 %> <input
		name="<%=SUBTEST_NAME%>" type="text" size="2" value="" readonly /> <%
 	}
 %>
	</td>
			...
			try {
						if (dgDetail.getNormal() != null
							...
							if (dgDetail.getResult() != null
								...
								Float result1 = Float.parseFloat(dgDetail
										.getResult());
 ...
 <input name="collectionId<%=resultEntryIndexForMultiple%>" value="<%=sampleId %>" type="hidden">
  <input name="<%=RESULT%>"
		...
		onkeyup="submitAllExceptEnterForMultiple(this,event,'','<%=resultEntryIndexForMultiple%>','<%=resultEntryIndex + 1%>','<%=subCounter%>');"
		class="highlight" value="<%=result%>"  readonly="readonly"/> <%
 	}
 					} else {
 %> 
 <input name="collectionId<%=resultEntryIndexForMultiple%>" value="<%=sampleId %>" type="hidden">
 <input name="<%=RESULT%>"
		...
		value="" /> <%
 					...
 					String data="";
 					if(dgDetail.getResult()!=null){
 						data=dgDetail.getResult().toString();
 ...
 <input name="<%=RESULT%>"
		id="<%=RESULT%><%=resultEntryIndexForMultiple%>" tabindex="1"
		onkeyup="submitAllExceptEnterForMultiple(this,event,'','<%=resultEntryIndexForMultiple%>','<%=resultEntryIndex + 1%>','<%=subCounter%>');"
		value="<%=data%>" readonly="readonly"/><!-- changed by govind 25-10-2016 --> <%
 	}
 ...
 %> <input name="normalId" type="hidden"
 ...
 %> <input
		name="<%=ADDITIONAL_REMARKS%>" type="text" value="" /> <%
 	}
 %> <input
