...
<%
  ...
  List <BudVoucherHeader> budVoucherHeader =new ArrayList();
  List <BudVoucherHeader>   budVoucherHeaderFull =new ArrayList();
  ...
  if(budVoucherHeader!=null){
	  budVoucherHeader = (List <BudVoucherHeader>)map.get("budVoucherHeader");
  }
  if(budVoucherHeaderFull!=null){
	  budVoucherHeaderFull = (List <BudVoucherHeader>)map.get("budVoucherHeaderFull");
  }
...
<select name="<%= CODE%>"
	...
	<%for (BudVoucherHeader voucherHeader :budVoucherHeaderFull ) {
		String toDeptName="";
		if(voucherHeader.getBillNo()!=null){
			toDeptName=" [ "+voucherHeader.getBillNo()+" ]";
		}

	%>
	<option value=<%=voucherHeader.getId()%>><%=voucherHeader.getBillNo()%></option>
	<%}%>
...
<div class="Block">
<label><span>*</span> Receipt No.</label>
<%for (BudVoucherHeader estimate :budVoucherHeader ) {
%>
<input type="hidden" name="<%=COMMON_ID %>" value="<%=estimate.getId() %>" />
<label class="value"><%=estimate.getBillNo() %></label>
<input type="hidden" name="code" value="<%=estimate.getBillNo() %>" />
<label><span>*</span>Receipt Date</label>
<label class="value"><%=estimate.getBilldate() %></label>
...
<label>Amount</label>
<input type="text" name="<%=SEARCH_NAME %>" value="<%= estimate.getTotalEstimatedAmount() %>" validate="Amount,String,yes" onkeypress="return isNumberKey(event)" tabindex="1" />
</div>
<div class="clear" > </div>
...
<input type="button" class="button" value="Print" onclick="submitForm('estimateentry','generalMaster?method=generateReportForGeneralMasters');">
