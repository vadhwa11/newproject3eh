...
<%
	...
	if(map.get("hrMedicalFitnessFirstFulList")!=null)
	{
		hrMedicalFitnessFirstFulList = (List<HrMedicalFitnessFirst>)map.get("hrMedicalFitnessFirstFulList");
	}
...
<select name="<%= CODE%>"
	...
	<%for (HrMedicalFitnessFirst estimate :hrMedicalFitnessFirstFulList ) {
		String toDeptName="";
...
<label ><span>*</span> Date</label>
 <input type="text" class="date" tabindex="1" validate="Date,string,yes"
	...
	MAXLENGTH="30" value="<%=HMSUtil.convertDateToStringWithoutTime(estimate.getDate()) %>"/> 
...
<input type="text" name="hinNo" id="hinNo" value="<%= estimate.getRegNo() %>"/>
	<div class="clear"></div>
...
<input type="text" id="name" name="Name" validate="Name,string,yes" MAXLENGTH="50" value="<%=estimate.getName() %>" readonly="readonly"/> 

<label ><span>*</span> Constitutional Weakness at Bodily Informative Except</label>
<input type="text" id="weakAtBody" name="weakAtBody" validate="Weakness,string,no" value="<%=estimate.getConstWeakness() %>"/>

<div class="clear"></div>
<label ><span>*</span> For Employment In The Office Of The</label>
<input type="text" id="employIn" name="employIn" validate="EmployIn,string,yes" value="<%=estimate.getForEmployInOffice() %>"/>

</div>
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
<input type="button" class="button" value="Update" onclick="submitForm('MedicalFitnessFirstForm','training?method=editMedicalCertificateFitnessFirstEntry');"/>
<input type="button" class="button" value="Print" onclick="submitForm('MedicalFitnessFirstForm','training?method=printPatientFitnessFirstEntry');" />

<input type="hidden" name="<%=CHANGED_BY%>" value="<%=userName%>" /> 
<input type="hidden" name="<%=CHANGED_DATE %>" value="<%=currentDate%>" /> 
<input type="hidden" name="<%=CHANGED_TIME %>" value="<%=time%>" />
<input type="hidden" name="<%=TRAINING_ID%>" value="<%= hrMedicalFitnessFirstList.get(0).getId()%>" />	
...
{
	 document.PatientSicknessForm.action="training?method=printTrainingServiceCertificate"
