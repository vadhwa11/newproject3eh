<%@page import="jkt.hms.masters.business.PrqQuotationVendorDetils"%>
...
List<PrqQuotationVendorDetils> vendorLists=new ArrayList<PrqQuotationVendorDetils>();
    if(map.get("vendorList")!=null){
    	vendorLists=(List<PrqQuotationVendorDetils>)map.get("vendorList");
}
%>
...
<table  colspan="7" id="vendorDetails" border="0" cellpadding="0" cellspacing="0">
	...
	<tbody>
	 <% if(vendorLists.size()>0){
	        	int i1=1;
				for (PrqQuotationVendorDetils ms : vendorLists) {
					
	        	 %>
	          <tr>
		   <td><%=i1 %></td>
			<td width="5%"><%=ms.getVendor().getSupplierName() %> </td>
			<td width="5%"><%=ms.getVendor().getAddress1() %></td>
		
<td><input name="selectCheckBox" type="checkbox" value="<%=ms.getId()%>" /></td>
