...
function checkk()
{
var SDate = document.smoving.<%= FROM_DATE%>.value;
var EDate = document.smoving.<%= TO_DATE %>.value;
...
<%
	...
	if(map.get("masHos") !=null){
		masHos=(List<MasHospital>)map.get("masHos");
	}
...
<form name="smoving" method="post" action="">
<div class="clear"></div>
<div class="Block">

<label>Institute</label><select name="institute" id="institute"  >
<%	if(hospitalId != 0){%>
			
				<option value="0">Select</option>
	
				 <%
				 for (MasHospital dis : masHos) {
				 if (dis.getId ()==hospitalId){
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.smoving.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.smoving.<%=TO_DATE%>,event);" />
<div class="clear"></div>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('smoving','stores?method=generateSlowMovingItems');"
