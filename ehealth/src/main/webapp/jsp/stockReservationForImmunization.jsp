...
<%@page import="jkt.hms.masters.business.StoreReservationPartialStock"%>
...
<%
	...
	if(map.get("partialStockList") != null){
		partialStockList = (List<StoreReservationPartialStock>)map.get("partialStockList");
	}
	if(map.get("campGroupList") != null){
		campGroupList = (List<StoreReservationCampGroup>)map.get("campGroupList");
	}

	
%>
<form name="stockReservationForImmunization" method="post">
<div class="titleBg">
...
<tr>
						...
						<th>Batch No.</th>
					 	<th>DOE(Open Vial)</th>
					  	<th>Qty(vial)</th>
					  	<th>Reserved Qty(Vial)</th>
</tr>
<%
	

for(StoreReservationPartialStock storeReservationPartialStock : partialStockList){
					
					%>
<tr>
<td><input type="checkbox" name="srNo" id="srNoId" value="y"></td>
<td><%=storeReservationPartialStock.getItem().getPvmsNo() %>
<input type="hidden" name="partialItemId" id="partialItemId" value="<%=storeReservationPartialStock.getItem().getId()%>"></td>
<td><%=storeReservationPartialStock.getItem().getNomenclature() %></td>
<td><%=storeReservationPartialStock.getItem().getItemConversion().getItemUnitName() %></td>
<td><%=storeReservationPartialStock.getBatchNo()%>
<input type="hidden" name="partialBatchNo" id="partialBatchNoId" value="<%=storeReservationPartialStock.getBatchNo()%>"></td>
<td><%=storeReservationPartialStock.getOpenBottleExpiryDate()!= null?HMSUtil.convertDateToStringWithoutTime(storeReservationPartialStock.getOpenBottleExpiryDate()):"" %></td>
<td><%=storeReservationPartialStock.getQty()%></td>
<td><input type="text" name="partialReserved" id="partialReserved" value="<%=storeReservationPartialStock.getQty()%>">
<input type="hidden" name="partialstockId" id="partialstockId" value="<%=storeReservationPartialStock.getId()%>"></td>
</tr>
<%} %>
</table>
<%} %>
...
<div class="clear"></div>
<input	type="button" name="Reserved" type="submit" value="Reserved" class="button" onclick="submitForm('stockReservationForImmunization','pubHealth?method=submitResrvedStockForImmunization');" />
...
function addRow(){
	 ...
	 // new Ajax.Autocompleter('nomenclature'+iteration,'ac2updates'+iteration,'stores?method=getItemListForIndent',{minChars:3,parameters:'requiredField=nomenclature'+iteration+'&counter='+iteration});
	  ...
	  e31.type = 'text';
	  e31.name='auId';
	  e31.id='auId'+iteration
	  e31.size = '8';
	  e31.className = 'readOnly';
	  e31.id='auId'+iteration;
...
function checkForDefectiveDrugs(val,a,inc)
{
		...
		ajaxFunctionForAutoCompleteInDefectiveDrugs('stockReservationForImmunization','pubHealth?method=fillItemsForDefectiveDrugs&pvmsNo=' + pvms , inc);
	    }
		...
		//	obj1 =document.getElementById(brandId);
			...
			//obj1.length =1;
	      	...
	      	for (loop = 0; loop < items.childNodes.length; loop++) {
		        ...
		        var auId  = item.getElementsByTagName("auId")[0];
		      ...
		      //  var brandLength  = item.getElementsByTagName("brands")[0];
	        	...
	        	document.getElementById('auId'+rowVal).value = auId.childNodes[0].nodeValue
	...
	var itemId = document.getElementById('itemId'+rowVal).value; 
	  ...
	  try {
	    ...
	    xmlHttp.onreadystatechange=function()
	    {
	     ...
	     url="pubHealth?method=getExpiryDateInAjax&batchName="+batchId+"&itemId="+itemId;
 	 	url = url+'&'+csrfTokenName+'='+csrfTokenValue; // added by amit das on 07-07-2016
	    xmlHttp.open("GET",url,true);
	    xmlHttp.setRequestHeader("Content-Type", "text/xml");
	    xmlHttp.send(null);
