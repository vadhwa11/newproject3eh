...
function checkk()
{
var SDate = document.substock.<%= FROM_DATE%>.value;
var EDate = document.substock.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
<h2>Sub Stock Report</h2>
</div>
<form name="substock" method="post" action="">
<div class="clear"></div>
<div class="Block">
...
<input 	type="text" name="<%=FROM_DATE%>" value="<%=currentDate%>"
	...
	onclick="setdate('<%=currentDate%>',document.substock.<%=FROM_DATE%>,event);" />
<label><span>*</span> To Date </label> <input type="text"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onclick="setdate('<%=currentDate%>',document.substock.<%=TO_DATE%>,event);" />
<div class="clear"></div>
...
<input type="button" name="add" id="addbutton" value="Generate Report"
	class="buttonBig"
	onClick="submitForm('substock','stores?method=generateSubStockRegister');"
