...
%> 
<h2>Systemic Examination Templates</h2>
...
<table colspan="7" id="componentDetails" cellpadding="0"
	...
	for(OpdTemplateTreatment treaTemplateTreatment :templateSysTreatmentList){
		...
		if(treaTemplateTreatment.getSystemicExamination() !=null){
			if(treaTemplateTreatment.getTemplate() !=null &&  treaTemplateTreatment.getSystemicExamination() !=null && !treaTemplateTreatment.getSystemicExamination().equals("") && treaTemplateTreatment.getSystemicExamination().length()>0){
		 		systemicExamination=treaTemplateTreatment.getSystemicExamination();
		 if(!systemicExamination.equals("")){
 	%>	   
		<tr>
			...
			<td>
				<input type="text" value="<%= systemicExamination%>" name="systemicExamination<%=i %>" id="systemicExamination<%=i %>" size="40"/>
			</td>
		</tr>
		<%i++;
    	}} %>
...
function setSystemicExamTempalte(){
	 ...
	 window.opener.document.getElementById('systemicExamination').value=generalExam;
	 window.opener.document.getElementById('systemicExamination').focus();
		 window.close();
	}
