...
<div class="titleBg">
<h2>Total Admissions in CHAF Bangalore (AFMSF-40)</h2>
</div>
<div class="Block">
<%
			 	String userName = "";
...
<input type="hidden" name="OutType" id="OutType" value="Pdf" /> <label
	...
	type="text" id="fromDateId" name="<%=FROM_DATE %>" class="date"
...
<label class="bodytextB"><font id="error">*</font> To Date:</label> <input
	type="text" id="ToDateId" name="<%=TO_DATE %>" class="date"
	readonly="readonly" MAXLENGTH="30" /> <img id="calendar"
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender"
	onClick="setdate('<%=currenDate%>',document.totalAdmission.<%=TO_DATE%>,event)" />

<label>Service Type:</label> <select id="serviceType" name="<%=SERVICE_TYPE_NAME %>"
	...
	<%
					...
					%>
</select> <input type="hidden" name="stName" id="serviceTypeName" /> 
<div class="clear"></div>
