...
<input type="text" id="empName" name="empName" value="<%=empList.get(0).getEmployeeName() %>"  readonly class="medium3" validate="Employee Name,string,yes"  MAXLENGTH="128"/>	
<input type="hidden" id="empid" name="empid" value="<%=empList.get(0).getId() %>"  readonly class="medium3"  MAXLENGTH="128"/>
...
<input type="text" id="department" name="department" value="<%=empList.get(0).getDepartment() != null ? empList.get(0).getDepartment() .getDepartmentName() :"" %>" MAXLENGTH="12" class="medium3" validate="Department,String,no" />

<label > Designation </label>
<input type="text" id="designation" name="designation" value="<%=empList.get(0).getRank().getRankName()%>" MAXLENGTH="12" class="medium3" validate="Designation,String,no" />

<label > Current Institute </label>
<input type="text" id="cur_institute" name="cur_institute" value="<%=empList.get(0).getHospital().getHospitalName() %>" MAXLENGTH="12" class="medium3" validate="Current Institute,String,no" />
<input type="hidden" id="cur_instituteId" name="cur_instituteId" value="<%=empList.get(0).getHospital().getId() %>" MAXLENGTH="12" class="medium3" validate="Current Institute,String,no" />
<div class="clear"></div>

<label > With in Days </label>
<input type="text" id="withInDays" name="withInDays" value="" MAXLENGTH="12" class="medium3" validate="With in Days,String,no" />
...
<table border="0" align="center" cellpadding="0" cellspacing="0"
	id="itemDetails">
		...
		<tr>
			<td>
			<input type="text" name="priority<%=i  %>" value="Priority <%=i %>"	tabindex=1  id="priority<%=i  %>"  size="20"/>
			<input type="hidden" name="employee_id<%=i  %>" value=""	tabindex=1  id="id<%=i  %>"  size="20"/>
			</td>
			<td>
			<select  name="district<%=i  %>" 	tabindex=1  id="district<%=i  %>" 	  validate="District,string,no" onChange="populateInstitute(this.value,<%=i%>)"/>
			<option value="">Select</option>
						<%
							for(MasDistrict md: distList) {
								if(md.getState().getId() == 18){
						%>
...
<input name="button"  type="button"	value="Update" class="button"	 onClick="submitForm('itemGrid','/hms/hrms/training?method=saveTransferApplication')"  />
<%} %>
