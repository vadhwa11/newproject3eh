...
function jsImport()
{
	...
	submitForm('attachFile','opd?method=uploadAndViewDocuments&hinId='+hinId+'&filename='+filename+'&'+csrfTokenName+'='+csrfTokenValue);
...
<form name="attachFiles" method="post" action="">
...
<td><%=!uploadDocs.getDescription().equals( "") ? uploadDocs.getDescription(): " - "%></td>                       
 <td><input name="Delete" type="button"  class="button" value="Delete" onClick="submitFormForButton('attachFiles','maintenance?method=deleteAttachFiles');" /></td>
<td><input type="hidden" id="documentId" name="documentId" value="<%=uploadDocs.getId()%>"></td>
</tr>																 
<%i++;
	}}
%> 
