...
function checkk() {
	...
	var SDate = document.vector.<%= FROM_DATE%>.value;
	var EDate = document.vector.<%= TO_DATE %>.value;
...
%>
<div class="titleBg">
	<h2>Vector Survey Report</h2>
</div>
<form name="vector" method="post" action="">
	<div class="clear"></div>
	<div class="Block">
		...
		<div id="divEnchashment" style="display: none;">
			<label>CHC</label><input type="radio" name="center" id="center" value="chc"
				onclick="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveychclist&district='+this.value,'tDiv');">
			<label>PHC</label><input type="radio" name="center" id="center" value="phc"
				onclick="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveychclist&district='+this.value,'tDiv');">
		</div>
		...
		<div id="tDiv">
			...
			<label>CHC </label> <select name="chcphc" id="" class="" onblur="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveyBasicCenterListChcPhc&chcphc='+this.value,'testDiv');" >
				<option value="0">Select</option>
			...
			<label>PHC </label> <select name="chcphc" id="" onblur="submitProtoAjaxWithDivName('vector','/hms/hms/pubHealth?method=getVectorSurveyBasicCenterListChcPhc&chcphc='+this.value,'testDiv');">
				<option value="0">Select</option>
				<option value=<%=map.get("hospitalId") %>><%=map.get("hospitalName") %></option>
			</select>
		...
		<label><span>*</span> From Date </label> <input type="text"
			...
			onclick="setdate('<%=currentDate%>',document.vector.<%=FROM_DATE%>,event);" />
		<label><span>*</span> To Date </label> <input type="text" class="date"
			...
			src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
			validate="Pick a date"
			onclick="setdate('<%=currentDate%>',document.vector.<%=TO_DATE%>,event);" />
		<div class="clear"></div>
		<div class="clear"></div>
		<div class="clear"></div>
		<input type="button" name="add" id="addbutton" value="Generate Report"
			class="buttonBig"
			onClick="submitForm('vector','pubHealth?method=generateVectorSurveyReport','checkk()');"
